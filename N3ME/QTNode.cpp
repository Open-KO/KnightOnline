...
void CQTNode::RenderMaxLevel()
{
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        if (m_pRefTerrain->m_ppIsLightMap[cx][cz])
        {
            ...
            pLightMapVertices[VBIndx].Set(
                (float)(cx * TERRAIN_CELL_SIZE),
                m_pRefTerrain->m_ppMapData[cx][cz].fHeight,
                (float)(cz * TERRAIN_CELL_SIZE),
                0.0f,
                1.0f,
                0.0f,
                lsuv,
                leuv);
            pLightMapVertices[VBIndx + 1].Set(
                (float)(cx * TERRAIN_CELL_SIZE),
                m_pRefTerrain->m_ppMapData[cx][cz + 1].fHeight,
                (float)((cz + 1) * TERRAIN_CELL_SIZE),
                0.0f,
                1.0f,
                0.0f,
                lsuv,
                lsuv);
            pLightMapVertices[VBIndx + 2].Set(
                (float)((cx + 1) * TERRAIN_CELL_SIZE),
                ...
                leuv,
                lsuv);
            pLightMapVertices[VBIndx + 3].Set(
                (float)((cx + 1) * TERRAIN_CELL_SIZE),
        ...
        else
        {
            ...
            else // 순수하게 타일만 찍는 경우..
            {
                ...
                pVertices[idx].Set(
                    ...
                    1.0f,
                    0.0f,
                    u1 + m_pRefTerrain->m_fTileDirU[dir1][2],
    ...
    */
    ...
    for (i = 0; i < 4; i++)
    {
        ...
        else
        {
            ...
            // int ctx = (tx * TERRAIN_CELL_SIZE) / (m_pRefTerrain->m_iColorMapTexSize *
            // m_pRefTerrain->m_iColorMapPixelPerUnitDistance); int ctz = (tz * TERRAIN_CELL_SIZE) /
            // (m_pRefTerrain->m_iColorMapTexSize * m_pRefTerrain->m_iColorMapPixelPerUnitDistance);
        ...
        else
        {
                ...
                && m_pRefTerrain->m_ppMapData[tx][tz].DTexInfo1.Attr.Attr < DTEX_MAX)
            {
                ...
                int ctx = (tx * TERRAIN_CELL_SIZE) / (m_pRefTerrain->m_iColorMapTexSize *
            m_pRefTerrain->m_iColorMapPixelPerUnitDistance); int ctz = (tz * TERRAIN_CELL_SIZE) /
            (m_pRefTerrain->m_iColorMapTexSize * m_pRefTerrain->m_iColorMapPixelPerUnitDistance); hr =
            m_pRefTerrain->s_lpD3DDev->SetTexture( 1, m_pRefTerrain->m_pColorTexture[ctx][ctz].Get() );
            }
            */
        }
...
void CQTNode::RenderMaxLevel()
{
    ...
    for(i=0;i<4;i++)
    {
        ...
        {
            int ctx = (tx * TERRAIN_CELL_SIZE) / (m_pRefTerrain->m_iColorMapTexSize *
m_pRefTerrain->m_iColorMapPixelPerUnitDistance); int ctz = (tz * TERRAIN_CELL_SIZE) / (m_pRefTerrain->m_iColorMapTexSize
* m_pRefTerrain->m_iColorMapPixelPerUnitDistance); hr = m_pRefTerrain->s_lpD3DDev->SetTexture( 0,
m_pRefTerrain->m_pColorTexture[ctx][ctz].Get() );
        }
...
void CQTNode::RenderNormalLevel()
{
    ...
    pVertices[VC].Set(
        (float)(m_CenterX * TERRAIN_CELL_SIZE),
        m_pRefTerrain->m_ppMapData[m_CenterX][m_CenterZ].fHeight,
        (float)(m_CenterZ * TERRAIN_CELL_SIZE),
        0.0f,
        1.0f,
        0.0f,
    ...
    VC++;
    pVertices[VC].Set(
        (float)((m_CenterX - half) * TERRAIN_CELL_SIZE),
        m_pRefTerrain->m_ppMapData[m_CenterX - half][m_CenterZ - half].fHeight,
        (float)((m_CenterZ - half) * TERRAIN_CELL_SIZE),
        0.0f,
        1.0f,
        0.0f,
