...
void CSowSeedMng::Random_Grass(void)
{
    ...
    for (int i = 0, Num = 0; i < MAX_BRUSH_SIZE; ++i)
    {
        for (int j = 0; j < MAX_BRUSH_SIZE; ++j)
        {
            ...
            //			if( (pFrame->m_pDlgSowSeed->Sow_Array[i][j] == 2) || (pFrame->m_pDlgSowSeed->Sow_Array[i][j] ==
            //1))
            {
        ...
        if (Render_Grass == TRUE)
        {
            ...
            for (i = 0, Num = 0; i < MAX_BRUSH_SIZE; ++i)
            {
                for (j = 0; j < MAX_BRUSH_SIZE; ++j)
                {
                    ...
                    //				if( (pFrame->m_pDlgSowSeed->Sow_Array[i][j] == 2) ||
                    //(pFrame->m_pDlgSowSeed->Sow_Array[i][j] == 1) )
                    {
                        if (Grass != nullptr)
                        {
                ...
                if ((CN3Shape *)pShape)
                {
                    ...
                    if (pShape->m_bDontRender == false)
                    {
                        ...
                        if (check_id >= 0)
                        {
                            CreateBox(
                                __Vector3(
                                    group->b_size * TERRAIN_CELL_SIZE / 2, 1.0f, group->b_size * TERRAIN_CELL_SIZE / 2),
                                __Vector3(
