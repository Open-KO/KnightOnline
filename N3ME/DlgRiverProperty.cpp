    ...
    if ((void *)wParam == &m_LPRiver)
    {
        if (pSelRiver)
        {
            ...
            if (pItem->m_propName == "River ID")
            {
                ...
                pSelRiver->SetAlphaFactor(strtoul(pItem->m_curValue, nullptr, 16));
            ...
            {
                // 에니메이션 되는 텍스쳐 지정 (갯수는 파일 이름으로부터 알아낸다. 따라서 맨 마지막번호파일을
                // 지정해야함)
...
void CDlgRiverProperty::OnOK()
{
    if (m_IsModalDialog)
    {
        ...
        {
            if (m_pRiverMng->GetRiverMesh(pSelRiver->GetRiverID()) == nullptr)
                CDialog::OnOK();
            else
