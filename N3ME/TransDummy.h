...
class CTransDummy : public CN3Transform
{
  ...
  protected:
    ...
    __Vector3 *m_vPrevScaleArray; // 이번 마우스 드래그 이전에 확대/축소값들 (선택된 객체들 모두의 값을 가지고 있어야
                                  // 하기 때문에 배열)
  ...
  public:
    ...
    void GetPickRay(
        POINT point, __Vector3 &vDir, __Vector3 &vOrig); // 마우스 포인터가 가리키는 쪽으로 뻗어나가는 선 얻는 함수
  protected:
    ...
    virtual void TransDiff(
        __Vector3 *pvDiffPos,
        __Quaternion *pvDiffRot,
