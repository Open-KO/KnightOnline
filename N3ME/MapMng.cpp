...
// 생성자
CMapMng::CMapMng(CMainFrame *pMainFrm) : m_RiverMng(pMainFrm), m_PondMng(pMainFrm)
{
...
void CMapMng::FocusAll()
{
    ASSERT(m_pSceneOutput);
    CN3Camera *pCamera = m_pSceneOutput->CameraGetActive();
    if (m_pTerrain == nullptr || pCamera == nullptr)
        return;
...
BOOL CMapMng::CameraMove(LPMSG pMsg)
{
    ...
    else if (pMsg->message == WM_KEYDOWN)
    {
        ...
        case '2':	// '1'과 같은 내용임 (함수로 만들지 않은 이유는 static변수 때문에..)
            {	static BOOL bSet = FALSE;
                if (pCamera) { static __Vector3 vEye(0, 0, 0);	static __Vector3 vAt(0, 0, 1); static __Vector3 vUp(0,
1, 0); if (GetAsyncKeyState(VK_CONTROL) & 0xff00) { vEye = pCamera->EyePos();	vAt = pCamera->AtPos();	vUp =
pCamera->UpVector(); pCamera->Apply(); bSet = TRUE; return FALSE; } else if (bSet) { pCamera->EyePosSet(vEye);
pCamera->AtPosSet(vAt);	pCamera->UpVectorSet(vUp); pCamera->Apply(); return TRUE;} else return FALSE;} } break; case
'3':	// '1'과 같은 내용임 (함수로 만들지 않은 이유는 static변수 때문에..) {	static BOOL bSet = FALSE; if (pCamera) {
static __Vector3 vEye(0, 0, 0);	static __Vector3 vAt(0, 0, 1); static __Vector3 vUp(0, 1, 0); if
(GetAsyncKeyState(VK_CONTROL) & 0xff00) { vEye = pCamera->EyePos();	vAt = pCamera->AtPos();	vUp = pCamera->UpVector();
pCamera->Apply(); bSet = TRUE; return FALSE; } else if (bSet) { pCamera->EyePosSet(vEye); pCamera->AtPosSet(vAt);
pCamera->UpVectorSet(vUp); pCamera->Apply(); return TRUE;} else return FALSE;} } break; case '4':	// '1'과 같은 내용임
(함수로 만들지 않은 이유는 static변수 때문에..) {	static BOOL bSet = FALSE; if (pCamera) { static __Vector3 vEye(0, 0,
0);	static __Vector3 vAt(0, 0, 1); static __Vector3 vUp(0, 1, 0); if (GetAsyncKeyState(VK_CONTROL) & 0xff00) { vEye =
pCamera->EyePos();	vAt = pCamera->AtPos();	vUp = pCamera->UpVector(); pCamera->Apply(); bSet = TRUE; return FALSE; }
                    else if (bSet) { pCamera->EyePosSet(vEye); pCamera->AtPosSet(vAt);	pCamera->UpVectorSet(vUp);
pCamera->Apply(); return TRUE;} else return FALSE;} } break; case '5':	// '1'과 같은 내용임 (함수로 만들지 않은 이유는
static변수 때문에..) {	static BOOL bSet = FALSE; if (pCamera) { static __Vector3 vEye(0, 0, 0);	static __Vector3
vAt(0, 0, 1); static __Vector3 vUp(0, 1, 0); if (GetAsyncKeyState(VK_CONTROL) & 0xff00) { vEye = pCamera->EyePos();	vAt
= pCamera->AtPos();	vUp = pCamera->UpVector(); pCamera->Apply(); bSet = TRUE; return FALSE; } else if (bSet) {
pCamera->EyePosSet(vEye); pCamera->AtPosSet(vAt);	pCamera->UpVectorSet(vUp); pCamera->Apply(); return TRUE;} else
return FALSE;} } break;
        }
    }
    else if (iButtonDownCount == 0 && !(GetAsyncKeyState(VK_MENU) & 0xff00) ) return FALSE;	// alt 키가 안눌렸을경우는
카메라 움직임이 아니다.
...
BOOL CMapMng::MouseMsgFilter(LPMSG pMsg)
{
    ...
    case WM_KEYDOWN: {
        ...
        case 0x20: // space bar...
        {
            ...
            if (!m_pTerrain->Pick(point.x, point.y, &vec, nullptr))
                break;
    ...
    case WM_LBUTTONDOWN: // 객체 선택
    {
        ...
        if (m_CursorMode == CM_OBJECT_BRUSH && m_pSelSourceObj)
        {
            ...
            m_CursorMode == CM_SELECT || m_CursorMode == CM_POS || m_CursorMode == CM_ROTATE ||
...
void CMapMng::OnSelChanged()
{
    ...
    if (pSelObj != nullptr)
    {
        ...
        if (iCount == 1 && m_pDlgOutputList != nullptr && m_pDlgOutputList->GetSafeHwnd() != nullptr)
...
{
    if (nullptr == m_pTerrain || nullptr == m_pSceneOutput)
        return;
    if (lstrlen(lpszPathName) == 0)
        return;
    ...
    //	{
    //		int idYesNo = ::MessageBox(::GetActiveWindow(), "중복된 오브젝트들을 삭제 하시겠습니까?", "중복된 오브젝트
    //처리", MB_YESNO); 		if(IDYES == idYesNo)
    //		{
    ...
    ShapeMgr.Create(
        (m_pTerrain->m_iHeightMapSize - 1) * TERRAIN_CELL_SIZE, (m_pTerrain->m_iHeightMapSize - 1) * TERRAIN_CELL_SIZE);
    int nSC = m_pSceneOutput->ShapeCount();
    for (int i = 0; i < nSC; i++)
    {
    ...
    _makepath(
        szObjPosting, szDrive, szDir, szFName, ".opd"); // "Object Posting Data" - Shape Manager file 이름을 정하고..
...
void CMapMng::SaveObjectPostData(LPCTSTR lpszFileName)
{
    ...
    */
    ...
    for (i = 0; i < iSC; ++i)
    {
        ...
        fprintf(
            stream,
            "FileName[ %s ] PartCount[ %d ] Position[ %f %f %f] Rotation[ %f %f %f %f ] Scale[ %f %f %f ] Belong [ %d "
            "] Attribute [ %d %d %d %d ]\n",
            szSFN,
            iSPC,
            vPos.x,
        ...
        for (int j = 0; j < iSPC; j++)
        {
            ...
            fprintf(
                ...
                pPart->m_Mtl.Diffuse.r,
                pPart->m_Mtl.Diffuse.g,
                pPart->m_Mtl.Diffuse.b,
                pPart->m_Mtl.Ambient.a,
                pPart->m_Mtl.Ambient.r,
                pPart->m_Mtl.Ambient.g,
                pPart->m_Mtl.Ambient.b);
        }
    }
...
CStatusBar *CMapMng::GetStatusBar()
{
    ...
    if (pFrame == nullptr || pFrame->IsKindOf(RUNTIME_CLASS(CFrameWnd)) == FALSE)
        return nullptr;

    CStatusBar *pBar = (CStatusBar *)pFrame->GetMessageBar();
    if (pBar == nullptr || pBar->IsKindOf(RUNTIME_CLASS(CStatusBar)) == FALSE)
        return nullptr;
