...
void CNPCPathMgr::MakeServerDataFile(const char *FullFileName)
{
    ...
    for (itPath = m_pPaths.begin(); itPath != m_pPaths.end(); itPath++)
    {
        ...
        SetRect(
            &rt,
            (int)pPath->m_LTStartVertex.x,
            (int)pPath->m_LTStartVertex.z,
            (int)pPath->m_RBStartVertex.x,
            (int)pPath->m_RBStartVertex.z);

        RECT LimitRC;
        SetRect(
            &LimitRC,
            (int)pPath->m_LTActVertex.x,
            (int)pPath->m_LTActVertex.z,
            (int)pPath->m_RBActVertex.x,
