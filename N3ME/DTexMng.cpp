    ...
    if (iCount < 0) // new version...
    {
        ...
        for (i = 0; i < iCount; i++)
        {
            ...
            if (version == 1)
            {
                ...
                _splitpath(szDTexFileName, nullptr, szDir, szFName, nullptr);
                wsprintf(
                    szDTexInfoFileName, "%s%s%s.dif", s_szPath.c_str(), szDir, szFName); // Texture Information file
...
void CDTexMng::SaveToFile(CString RealFileName)
{
    ...
    for (it = m_pDTex.begin(); it != m_pDTex.end(); it++)
    {
        ...
        //	그에 관한 타일 정보들을 읽고..
        HANDLE hFile = CreateFile(szDTexInfoFileName, GENERIC_WRITE, 0, nullptr, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL,
        nullptr);
