...
class CAPISocket
{
  ...
  public:
    ...
    //	static	void	MP_AddString(BYTE *dest, int &iOffset, BYTE *sor, int len) { CopyMemory(dest+iOffset, sor, len);
    //iOffset += len; }
    ...
    //	static	void	Parse_GetString(const BYTE* buf, int &iOffset, char *dest, int len) { if (len) {
    //CopyMemory(dest, buf+iOffset, len); iOffset += len; } dest[len] = 0; }
