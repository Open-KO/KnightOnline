...
BOOL CLauncherDlg::OnInitDialog()
{
    ...
    lStatus = RegQueryValueEx(
        m_hRegistryKey, _T("SERVICE"), nullptr, &dwType, (BYTE *)m_strServiceName, &dwBytes); // 서비스 이름..
    if (ERROR_SUCCESS != lStatus)
    {
        CString szErr;
...
void CLauncherDlg::DownloadProcess()
{
    ...
    for (int i = 0; i < m_nGetFileNum; i++)
    {
        ...
        if (bDownloadSuccess)
        {
            szLocalFName.Format(_T("%s\\%hs"), m_szInstalledPath.GetString(), m_szGetFileNames[i].c_str());
...
{
    m_hInetSession = InternetOpen(_T("3DOnline"), INTERNET_OPEN_TYPE_PRECONFIG, nullptr, nullptr, 0);
    if (!m_hInetSession)
    {
        CString szErr, szMsg;
...
BOOL CLauncherDlg::GetDownloadFile(const std::string &szFtpUrl, const std::string &szFileName)
{
    ...
    // CInternetFile *pFile;
    hFile = FtpOpenFileA(m_hFtpConnection, szFtpUrl.c_str(), GENERIC_READ, FTP_TRANSFER_TYPE_BINARY, 0);
...
bool CLauncherDlg::ArchiveOpen(CString OpenFileName)
{
    ...
    do
    {
        ...
        {
            if (MessageBox(
                    _T("The central directory was not found. If you're opening a multi-disk archive, make sure you ")
                    _T("have inserted the last disk. Retry?"),
                    _T(""),
