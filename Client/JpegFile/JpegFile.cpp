...
BOOL CJpegFile::RGBToJpegFile(std::string fileName, BYTE *dataBuf, UINT widthPix, UINT height, BOOL color, int quality)
{
    if (dataBuf == nullptr)
        return FALSE;
    if (widthPix == 0)
...
BYTE *CJpegFile::MakeDwordAlignedBuf(
    BYTE *dataBuf,
...
BOOL CJpegFile::DibToSamps(
    HANDLE hDib, int nSampsPerRow, jpeg_compress_struct cinfo, JSAMPARRAY jsmpPixels, const char **pcsMsg)
{
    // Sanity...
    ...
    const char **pcsMsg) // Error msg to return
{
    ...
    BOOL convertedSamples = DibToSamps(hDib, nSampsPerRow, cinfo, jsmpArray, pcsMsg);
    if (convertedSamples)
    {
