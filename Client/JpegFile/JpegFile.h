...
class CJpegFile
{
  public:
    ...
    BYTE *RGBFromDWORDAligned(
        BYTE *inBuf,     // input buf
    ...
    BOOL DibToSamps(
        HANDLE hDib, int nSampsPerRow, struct jpeg_compress_struct cinfo, JSAMPARRAY jsmpPixels, const char *pcsMsg);
    BOOL JpegFromDib(
        HANDLE hDib,         // Handle to DIB
        int nQuality,        // JPEG quality (0-100)
        std::string csJpeg,  // Pathname to jpeg file
        const char *pcsMsg); // Error msg to return
    virtual BOOL EncryptJPEG(
        HANDLE hDib,         // Handle to DIB
        int nQuality,        // JPEG quality (0-100)
        std::string csJpeg,  // Pathname to jpeg file
        const char *pcsMsg); // Error msg to return
