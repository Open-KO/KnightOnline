...
void CZipFileHeader::WriteLocal(CZipStorage &storage)
{
    ...
    // it was only local information, use CZipArchive::SetExtraField to set the file extra field in the central
    // directory
...
bool CZipFileHeader::PrepareData(int iLevel, bool bExtraHeader, bool bEncrypted)
{
    ...
    return !(
        m_pszComment.GetSize() > USHRT_MAX || m_pszFileName.GetSize() > USHRT_MAX ||
