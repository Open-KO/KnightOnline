...
bool CN3Chr::Load(File &file)
{
    ...
    for (int i = 0; i < 2; i++)
        file.Read(
            &m_nJointPartStarts[i],
            sizeof(int)); // 조인트의 일부분이 따로 에니메이션 되야 한다면.. 조인트 인덱스 시작 번호

    for (int i = 0; i < 2; i++)
        file.Read(
            &m_nJointPartEnds[i], sizeof(int)); // 조인트의 일부분이 따로 에니메이션 되야 한다면.. 조인트 인덱스 끝 번호
...
void CN3Chr::Tick(float fFrm)
{
    ...
    for (int i = 0; i < MAX_CHR_LOD; i++)
    {
        if (fLOD < CHR_LOD_CALCULATION_VALUES[0][i]) // 일단 가장 큰값을 기준으로 하고 LOD 값을 정한 다음..... Render
                                                     // 에서 변경한다....
        {
            m_nLOD = i;
            break;
    ...
    if (m_FrmCtrlUpper.pAniData == nullptr)
    {
        for (size_t i = 0; i < m_JointRefs.size(); i++)
        {
            if (m_FrmCtrl.fBlendTime > 0)
            {
                ...
                m_JointRefs[i]->ReCalcMatrixBlended(
                    m_FrmCtrl.fFrmCur, m_FrmCtrl.fBlendFrm, fBlendFactor); // Joint Animation Blending...
            }
            else
            {
        ...
        for (int i = 0; i < MAX_CHR_ANI_PART; i++)
        {
            ...
            if (0 == i)
            {
                if (pFrmCtrls[i]->fBlendTime > 0) // Motion Blending 이 필요하면...
                {
                    ...
                    m_pRootJointRef->ReCalcMatrixBlended(
                        pFrmCtrls[i]->fFrmCur,
            ...
            if (m_nJointPartStarts[i] > 0)
            {
                ...
                for (int j = m_nJointPartStarts[i]; j <= m_nJointPartEnds[i]; j++)
                {
                    if (pFrmCtrls[i]->fBlendTime > 0)
                        m_JointRefs[j]->ReCalcMatrixBlended(
                            pFrmCtrls[i]->fFrmCur,
                            pFrmCtrls[i]->fBlendFrm,
...
void CN3Chr::Render()
{
    ...
    if (s_iLODDelta > 0)
    {
        ...
        for (int i = 0; i < MAX_CHR_LOD; i++)
        {
            if (fLOD < CHR_LOD_CALCULATION_VALUES[s_iLODDelta][i]) // 일단 가장 큰값을 기준으로 하고 LOD 값을 정한
                                                                   // 다음..... Render 에서 변경한다....
            {
                m_nLOD = i;
                break;
...
int CN3Chr::AniCurSet(
    int iAni,                 // Animation 번호,
    ...
    bool bStopUpperAnimation) // 상체 에니메이션이 있으면.. 멈추도록
{
    ...
    //	if(iAni == m_FrmCtrl.iAni) { m_FrmCtrl.iAniLoop = 0; return -1; } // 같은 에니메이션이면 루핑 참조 카운트만
    //초기화하고 돌아간다.
...
int CN3Chr::CheckCollisionPrecisely(int ixScreen, int iyScreen, __Vector3 *pvPick)
{
    ...
    ::_Convert2D_To_3DCoordinate(
        ixScreen,
        iyScreen,
        s_CameraData.mtxView,
        s_CameraData.mtxProjection,
        s_CameraData.vp.Width,
