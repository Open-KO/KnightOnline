...
float CN3PMeshCreate::GetTriCollapsesLoss(uint16_t *pTriIndex, bool bArea)
{
    ...
    __Vector3 pts[3] = {m_pVertices[pTriIndex[0]], m_pVertices[pTriIndex[1]], m_pVertices[pTriIndex[2]]};
...
void CN3PMeshCreate::TryEdge(int pt_a, int pt_b, float &be_val, uint16_t &be_to, uint16_t &be_from, bool &IsOver)
{
...
CN3PMesh *CN3PMeshCreate::CreateRendererMesh()
{
    ...
    if (m_iTotalIndexChanges > 0)
    {
        ...
        __ASSERT(
            pPMesh->m_pAllIndexChanges, "Index change buffer is NULL"); // again, you should check for malloc failure
...
void CN3PMeshCreate::SwapToEnd(
    uint16_t swapper,
    __PMCEdgeCollapse *collapses,
    __PMCEdgeCollapse *collapses_end,
    uint16_t &pt_to,
    uint16_t &pt_from)
{
