...
HRESULT CDFont::SetText(const std::string &szText, uint32_t dwFlags)
{
    ...
    if (nullptr == m_pTexture)
    {
        ...
        hr = m_pd3dDevice->CreateTexture(
            m_dwTexWidth, m_dwTexHeight, iMipMapCount, 0, D3DFMT_A4R4G4B4, D3DPOOL_MANAGED, &m_pTexture, nullptr);
        if (FAILED(hr))
            return hr;
    }
...
HRESULT CDFont::DrawText(FLOAT sx, FLOAT sy, uint32_t dwColor, uint32_t dwFlags, FLOAT fZ)
{
    ...
    //	}
    //	else if ( D3DZB_TRUE != dwZEnable) m_pd3dDevice->SetRenderState( D3DRS_ZENABLE, D3DZB_TRUE );	// fZ가 1.0이
    //아니면 z 버퍼 켜고 그린다.
    if (FALSE != dwFog)
