...
struct __EXPORT_OPTION
{
    ...
    BOOL bExportDummy; // 도우미 오브젝트(??? - 실제 겜에서는 필요없고 개발시에만 필요한 오브젝트를 말한다)데이터를 갖고
                       // 있다.
    BOOL bExportCharacter; // 도우미 오브젝트(??? - 실제 겜에서는 필요없고 개발시에만 필요한 오브젝트를 말한다)데이터를
                           // 갖고 있다.
    ...
    BOOL bGenerateHalfSizeTexture; // 텍스처 파일을 자동으로 최적화 시켜서 생성 Direct3D 의 포맷에 맞게 2의 제곱수
                                   // 단위로 맞추어서 "OBM" 비트맵 파일로 저장.
