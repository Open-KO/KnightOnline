...
bool CN3VMesh::CheckCollision(
    const __Matrix44 &MtxWorld, const __Vector3 &v0, const __Vector3 &v1, __Vector3 *pVCol, __Vector3 *pVNormal)
{
    if (m_nVC <= 0)
        return false;
    ...
    for (int i = 0; i < nFC; i++)
    {
        ...
        if (false == ::_IntersectTriangle(
                         vPos0, vDir, m_pVertices[nCI0], m_pVertices[nCI1], m_pVertices[nCI2], fT, fU, fV, &vColTmp))
            continue; // 첫째 벡터가 걸치면..
        if (false == ::_IntersectTriangle(
                         vPos1, vDir, m_pVertices[nCI0], m_pVertices[nCI1], m_pVertices[nCI2])) // 둘째는 안 걸치면..
        {
    ...
    for (int i = 0; i < nFC; i++)
    {
        ...
        d = -(tmpNormal.x * m_pVertices[nCI0].x) - (tmpNormal.y * m_pVertices[nCI0].y) -
...
bool CN3VMesh::Pick(
    const __Matrix44 &MtxWorld, const __Vector3 &vPos, const __Vector3 &vDir, __Vector3 *pVCol, __Vector3 *pVNormal)
{
    if (m_nVC <= 0)
        return false;
    ...
    for (int i = 0; i < nFC; i++)
    {
        ...
        if (pVCol)
        {
            float fT, fU, fV;
            ::_IntersectTriangle(
                vPos2, vDir2, m_pVertices[nCI0], m_pVertices[nCI1], m_pVertices[nCI2], fT, fU, fV, pVCol);
            (*pVCol) *= MtxWorld;
        }
