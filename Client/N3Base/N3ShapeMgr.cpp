...
bool CN3ShapeMgr::Load(File &file)
{
    ...
    if (iSC > 0)
    {
        ...
        for (int i = 0; i < iSC; i++)
        {
            ...
            // pShape->m_iEventType; 0-바인드포인트 1-성문(좌우열림) 2-성문(상하열림) 3-레버(상하당김) 4-깃발(보임,
            // 안보임) pShape->m_iNPC_ID; 조종할 Object ID pShape->m_iNPC_Status; toggle 0, 1
        ...
        for (int j = 0; j < 3; j++)
        {
            ...
            for (int z = zz1; z <= zz2; z++)
            {
                ...
                for (int x = xx1; x <= xx2; x++)
                {
                    ...
                    {
                        // 위의 변과의 교차점을 계산하고..
                        float fXCross = vEdge[j][0].x + (fZMax - vEdge[j][0].z) * (vEdge[j][1].x - vEdge[j][0].x) /
                        ...
                        for (float fZ = zz3; fZ <= zz4 && !bWriteID; fZ += 0.25f)
                        {
                            for (float fX = xx3; fX <= xx4 && !bWriteID; fX += 0.25f)
                            {
                                ...
                                if (::_IntersectTriangle(
                                        ...
                                        vDir,
                                        m_pvCollisions[i * 3],
                                        m_pvCollisions[i * 3 + 1],
                                        m_pvCollisions[i * 3 + 2]))
                    ...
                    //						memcpy(pSubCell->pdwCCVertIndices, pwBack, nCCPC * 3 * 4); // 점세개가
                    //하나의 폴리곤이고 워드 인덱스이므로.. 						delete [] pwBack;
                    //					}
    ...
    for (int i = 0; i < iSubCellCount; i++)
    {
        ...
        for (int j = 0; j < ppCells[i]->nCCPolyCount; j++)
        {
            ...
            if (!::_IntersectTriangle(
                    vPos,
                    vDir,
                    m_pvCollisions[nIndex0],
                    m_pvCollisions[nIndex1],
            ...
            if (::_IntersectTriangle(
                    vPosNext, vDir, m_pvCollisions[nIndex0], m_pvCollisions[nIndex1], m_pvCollisions[nIndex2]))
                continue;
