...
void CN3AlphaPrimitiveManager::Render()
{
    ...
    for (int i = 0; i < m_nToDrawCount; i++)
    {
        ...
        if (pBuffs[i]->pwIndices && pBuffs[i]->nPrimitiveCount > 0) // Index 가 있으면..
        {
            if (pBuffs[i]->bUseVB)
            {
                CN3Base::s_lpD3DDev->SetStreamSource(
                    0, (LPDIRECT3DVERTEXBUFFER9)pBuffs[i]->pVertices, 0, pBuffs[i]->dwPrimitiveSize);
                CN3Base::s_lpD3DDev->SetIndices((LPDIRECT3DINDEXBUFFER9)pBuffs[i]->pwIndices);
                CN3Base::s_lpD3DDev->DrawIndexedPrimitive(
                    pBuffs[i]->ePrimitiveType, 0, 0, pBuffs[i]->nVertexCount, 0, pBuffs[i]->nPrimitiveCount);
            }
            else
            {
                CN3Base::s_lpD3DDev->DrawIndexedPrimitiveUP(
                    pBuffs[i]->ePrimitiveType,
                    0,
    ...
    /*
        ...
        qsort(pBuffs, m_nToDrawCount, sizeof(__AlphaPrimitive*), SortByCameraDistance); // 버퍼에 쌓인 프리미티브대로
    정렬하고..
        ...
        if(TRUE != RS_current.dwAlpha) {RS_current.dwAlpha = TRUE;
    CN3Base::s_lpD3DDev->SetRenderState(D3DRS_ALPHABLENDENABLE, TRUE);} static __Material mtl; mtl.Init();
        CN3Base::s_lpD3DDev->SetMaterial(&mtl);
        ...
        for(i = 0; i < m_nToDrawCount; i++)
        {
            if(pBuffs[i]->nRenderFlags & RF_NOTUSEFOG)
            {
                if (FALSE != RS_current.dwFog) { RS_current.dwFog = FALSE;
    CN3Base::s_lpD3DDev->SetRenderState(D3DRS_FOGENABLE, FALSE);} } // Fog 무시.. else
            {
                if (TRUE != RS_current.dwFog) {	RS_current.dwFog = TRUE;
    CN3Base::s_lpD3DDev->SetRenderState(D3DRS_FOGENABLE, TRUE);	}
            }

            if(pBuffs[i]->nRenderFlags & RF_DOUBLESIDED)
            {
                if (D3DCULL_NONE != RS_current.dwCull) { RS_current.dwCull = D3DCULL_NONE;
    CN3Base::s_lpD3DDev->SetRenderState(D3DRS_CULLMODE, D3DCULL_NONE);} } // Render Flags - else
            {
                if (D3DCULL_CCW != RS_current.dwCull) {	RS_current.dwCull = D3DCULL_CCW;
    CN3Base::s_lpD3DDev->SetRenderState(D3DRS_CULLMODE, D3DCULL_CCW);}
            }

            if(pBuffs[i]->nRenderFlags & RF_NOTUSELIGHT)
            {
                if (FALSE != RS_current.dwLgt) { RS_current.dwLgt = FALSE;
    CN3Base::s_lpD3DDev->SetRenderState(D3DRS_LIGHTING, FALSE);	} } // Render Flags - else
            {
                if (TRUE != RS_current.dwLgt) {	RS_current.dwLgt = TRUE;
    CN3Base::s_lpD3DDev->SetRenderState(D3DRS_LIGHTING, TRUE);}
            }

            if(pBuffs[i]->nRenderFlags & RF_NOTZWRITE)
            {
                if (FALSE != RS_current.dwZWrite) {	RS_current.dwZWrite = FALSE;
    CN3Base::s_lpD3DDev->SetRenderState(D3DRS_ZWRITEENABLE, FALSE);}
            }
            else
            {
                if (TRUE != RS_current.dwZWrite) {	RS_current.dwZWrite = TRUE;
    CN3Base::s_lpD3DDev->SetRenderState(D3DRS_ZWRITEENABLE, TRUE);}
            ...
            if(pBuffs[i]->nRenderFlags & RF_DIFFUSEALPHA)
            {
                if(pBuffs[i]->lpTex)
                {
                    if (D3DTOP_MODULATE != RS_current.dwAO)	{ RS_current.dwAO = D3DTOP_MODULATE;
    CN3Base::s_lpD3DDev->SetTextureStageState( 0, D3DTSS_ALPHAOP,   D3DTOP_MODULATE );	} if (D3DTA_DIFFUSE !=
    RS_current.dwAA1)	{ RS_current.dwAA1 = D3DTA_DIFFUSE; CN3Base::s_lpD3DDev->SetTextureStageState( 0,
    D3DTSS_ALPHAARG1, D3DTA_DIFFUSE);} if (D3DTA_TEXTURE != RS_current.dwAA2)	{ RS_current.dwAA2 = D3DTA_TEXTURE;
    CN3Base::s_lpD3DDev->SetTextureStageState( 0, D3DTSS_ALPHAARG2, D3DTA_TEXTURE);}
                }
                else
                {
                    if (D3DTOP_SELECTARG1 != RS_current.dwAO) {	RS_current.dwAO = D3DTOP_SELECTARG1;
    CN3Base::s_lpD3DDev->SetTextureStageState( 0, D3DTSS_ALPHAOP,   D3DTOP_SELECTARG1 );} if (D3DTA_DIFFUSE !=
    RS_current.dwAA1) { RS_current.dwAA1 = D3DTA_DIFFUSE;	CN3Base::s_lpD3DDev->SetTextureStageState( 0,
    D3DTSS_ALPHAARG1, D3DTA_DIFFUSE);}
            ...
            if(pBuffs[i]->nRenderFlags & RF_POINTSAMPLING)
            {
                if ( D3DTEXF_POINT != RS_current.dwPointSampling) { RS_current.dwPointSampling = D3DTEXF_POINT;
    CN3Base::s_lpD3DDev->SetTextureStageState(0, D3DTSS_MIPFILTER, D3DTEXF_POINT); }// Render Flags -
            }
            else
            {
                if ( D3DTEXF_NONE != RS_current.dwPointSampling) { RS_current.dwPointSampling = D3DTEXF_NONE;
    CN3Base::s_lpD3DDev->SetTextureStageState(0, D3DTSS_MIPFILTER, D3DTEXF_NONE); }// Render Flags -
            }

            if (pBuffs[i]->dwBlendSrc != RS_current.dwSrcBlend)	{ RS_current.dwSrcBlend = pBuffs[i]->dwBlendSrc;
    CN3Base::s_lpD3DDev->SetRenderState(D3DRS_SRCBLEND,   pBuffs[i]->dwBlendSrc);} if (pBuffs[i]->dwBlendDest !=
    RS_current.dwDestBlend) {	RS_current.dwDestBlend = pBuffs[i]->dwBlendDest;
    CN3Base::s_lpD3DDev->SetRenderState(D3DRS_DESTBLEND,  pBuffs[i]->dwBlendDest);}
            ...
            if(pBuffs[i]->lpTex)
            {
                if (D3DTOP_MODULATE != RS_current.dwCO)	{ RS_current.dwCO = D3DTOP_MODULATE;
    CN3Base::s_lpD3DDev->SetTextureStageState(0, D3DTSS_COLOROP, D3DTOP_MODULATE);	} if (D3DTA_DIFFUSE !=
    RS_current.dwCA1) { RS_current.dwCA1 = D3DTA_DIFFUSE; CN3Base::s_lpD3DDev->SetTextureStageState(0, D3DTSS_COLORARG1,
    D3DTA_DIFFUSE);} if (D3DTA_TEXTURE != RS_current.dwCA2) { RS_current.dwCA2 = D3DTA_TEXTURE;
    CN3Base::s_lpD3DDev->SetTextureStageState(0, D3DTSS_COLORARG2, D3DTA_TEXTURE);}
            }
            else
            {
                if (D3DTOP_SELECTARG1 != RS_current.dwCO) {	RS_current.dwCO = D3DTOP_SELECTARG1;
    CN3Base::s_lpD3DDev->SetTextureStageState(0, D3DTSS_COLOROP, D3DTOP_SELECTARG1);} if (D3DTA_DIFFUSE !=
    RS_current.dwCA1) { RS_current.dwCA1 = D3DTA_DIFFUSE;	CN3Base::s_lpD3DDev->SetTextureStageState(0,
    D3DTSS_COLORARG1, D3DTA_DIFFUSE);}
            ...
            if(pBuffs[i]->pwIndices && pBuffs[i]->nPrimitiveCount > 0) // Index 가 있으면..
            {
                if (pBuffs[i]->bUseVB)
                {
                    CN3Base::s_lpD3DDev->SetStreamSource( 0, (LPDIRECT3DVERTEXBUFFER8)pBuffs[i]->pVertices,
    pBuffs[i]->dwPrimitiveSize ); CN3Base::s_lpD3DDev->SetIndices((LPDIRECT3DINDEXBUFFER8)pBuffs[i]->pwIndices, 0);
            ...
            else if(pBuffs[i]->nPrimitiveCount > 0)
            {
                ...
                {
                    CN3Base::s_lpD3DDev->SetStreamSource( 0, (LPDIRECT3DVERTEXBUFFER8)pBuffs[i]->pVertices,
    pBuffs[i]->dwPrimitiveSize ); CN3Base::s_lpD3DDev->DrawPrimitive(	pBuffs[i]->ePrimitiveType, 0,
                                            pBuffs[i]->nPrimitiveCount );
                }
                else
                {
        ...
        // restore
        if(RS_old.dwAlpha != RS_current.dwAlpha) CN3Base::s_lpD3DDev->SetRenderState(D3DRS_ALPHABLENDENABLE,
    RS_old.dwAlpha); if(RS_old.dwFog != RS_current.dwFog) CN3Base::s_lpD3DDev->SetRenderState(D3DRS_FOGENABLE,
    RS_old.dwFog); if(RS_old.dwCull != RS_current.dwCull) CN3Base::s_lpD3DDev->SetRenderState(D3DRS_CULLMODE,
    RS_old.dwCull); if(RS_old.dwLgt != RS_current.dwLgt) CN3Base::s_lpD3DDev->SetRenderState(D3DRS_LIGHTING,
    RS_old.dwLgt); if(RS_old.dwZWrite != RS_current.dwZWrite) CN3Base::s_lpD3DDev->SetRenderState(D3DRS_ZWRITEENABLE,
    RS_old.dwZWrite); if(RS_old.dwSrcBlend != RS_current.dwSrcBlend) CN3Base::s_lpD3DDev->SetRenderState(D3DRS_SRCBLEND,
    RS_old.dwSrcBlend ); if(RS_old.dwDestBlend != RS_current.dwDestBlend)
    CN3Base::s_lpD3DDev->SetRenderState(D3DRS_DESTBLEND, RS_old.dwDestBlend); if(RS_old.dwAO != RS_current.dwAO)
    CN3Base::s_lpD3DDev->SetTextureStageState( 0, D3DTSS_ALPHAOP,   RS_old.dwAO); if(RS_old.dwAA1 != RS_current.dwAA1)
    CN3Base::s_lpD3DDev->SetTextureStageState( 0, D3DTSS_ALPHAARG1, RS_old.dwAA1); if(RS_old.dwAA2 != RS_current.dwAA2)
    CN3Base::s_lpD3DDev->SetTextureStageState( 0, D3DTSS_ALPHAARG2, RS_old.dwAA2); if(RS_old.dwCO != RS_current.dwCO)
    CN3Base::s_lpD3DDev->SetTextureStageState( 0, D3DTSS_COLOROP,   RS_old.dwCO); if(RS_old.dwCA1 != RS_current.dwCA1)
    CN3Base::s_lpD3DDev->SetTextureStageState( 0, D3DTSS_COLORARG1, RS_old.dwCA1); if(RS_old.dwCA2 != RS_current.dwCA2)
    CN3Base::s_lpD3DDev->SetTextureStageState( 0, D3DTSS_COLORARG2, RS_old.dwCA2); if(RS_old.dwPointSampling !=
    RS_current.dwPointSampling) CN3Base::s_lpD3DDev->SetTextureStageState( 0, D3DTSS_MIPFILTER, RS_old.dwPointSampling);
