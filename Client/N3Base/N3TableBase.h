...
template <typename Type> class CN3TableBase : public CN3TableBaseImpl
{
  public:
...
template <class Type> bool CN3TableBase<Type>::Load(File &file)
{
    Release();
    ...
    if (iDataTypeCount > 0)
    {
        ...
        int iSize = offsets[iDataTypeCount]; // MakeOffstTable 함수에서 리턴되는 값중 m_iDataTypeCount번째에 이 함수의
                                             // 실제 사이즈가 들어있다.
...
template <class Type> bool CN3TableBase<Type>::MakeOffsetTable(std::vector<int> &offsets)
{
    ...
    for (int i = 1; i < iDataTypeCount; i++)
    {
        ...
        if (0 == modulo)
        {
            ...
            if (0 == modulo)
                ...
                offsets[i] = ((int)(iPreviousOffset + iPrevDataSize + (StructAlignment - 1)) / StructAlignment) *
                             StructAlignment;
        }
        else if (1 == modulo)
        {
    ...
    offsets[iDataTypeCount] =
        ((int)(offsets[iDataTypeCount - 1] + iPrevDataSize + (StructAlignment - 1)) / StructAlignment) *
        StructAlignment;
    return true;
}
