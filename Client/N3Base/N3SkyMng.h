...
struct __SKY_DAYCHANGE
{
    ...
    void Init(
        const std::string &szName_Arg,
        eSKY_DAYCHANGE eSDC_Arg,
        uint32_t dwWhen_Arg,
        uint32_t dwParam1_Arg,
        uint32_t dwParam2_Arg,
        float fHowLong_Arg)
    {
        szName = szName_Arg;
...
class CN3SkyMng : public CN3BaseFileAccess
{
  ...
  protected:
    void SetCheckGameTime(uint32_t dwCheckGameTime); //	CheckGameTime을 정해주고 현재시간을 다시 세팅한다.(특정 시간으로
                                                     //강제적으로 만들때 호출한다.)
    int GetLatestChange(
        eSKY_DAYCHANGE eSDC,
