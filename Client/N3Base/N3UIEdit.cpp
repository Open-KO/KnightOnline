...
BOOL CN3UIEdit::CreateEditWindow(HWND hParent, RECT rect)
{
    ...
    s_hWndEdit = CreateWindow(
        "EDIT",
        "EditWindow",
        WS_CHILD | WS_TABSTOP | ES_LEFT | ES_WANTRETURN,
        rect.left,
        rect.top,
    ...
    if (szString.size() > m_iMaxStrLen)
    {
        ...
        if (IsHangulMiddleByte(szString.c_str(), static_cast<int>(m_iMaxStrLen)))
        {
            szNewBuff = szString.substr(
                0, m_iMaxStrLen - 1); // -1은 한글이므로 하나 덜 카피하기 위해 +1은 맨 마지막에 nullptr 넣기 위해
            if (UISTYLE_EDIT_PASSWORD & m_dwStyle)
            {
...
BOOL CN3UIEdit::MoveOffset(
    int iOffsetX, int iOffsetY) // 위치 지정(chilren의 위치도 같이 바꾸어준다. caret위치도 같이 바꾸어줌.)
{
