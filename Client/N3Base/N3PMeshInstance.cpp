...
#else
    // value는 distance * FOV이다.
    if (m_pCollapseUpTo == nullptr || m_pPMesh == nullptr)
        return;
...
bool CN3PMeshInstance::SplitOne()
{
    ...
    // 이렇게 해도 괜찮을 수 있도록 방어코드를 넣었다. m_pPMesh->m_pCollapses 를 할당할때 1개 더 할당하고 마지막
    // 데이터를 초기값으로 넣었다.
    ...
    if (m_iNumIndices > 3)
    {
        ...
        for (i = 0; i < iLC; ++i)
        {
            s_lpD3DDev->DrawIndexedPrimitiveUP(
                ...
                m_iNumVertices,
                iPCToRender,
                m_pIndices + i * iPCToRender * 3,
                D3DFMT_INDEX16,
                m_pPMesh->m_pVertices,
                sizeof(__VertexT1));
        }
    ...
    /*	if(m_iNumIndices > 3)
        {
            ...
            for (i=0; i<iLC; ++i)
            {
                s_lpD3DDev->DrawIndexedPrimitiveUP(D3DPT_TRIANGLELIST, 0, m_iNumVertices, iPCToRender, m_pIndices +
       i*iPCToRender*3, D3DFMT_INDEX16, m_pPMesh->m_pVertices, sizeof(__VertexT1));
            }

            int iRPC = iPC%iPCToRender;
            if(iRPC > 0) s_lpD3DDev->DrawIndexedPrimitiveUP(D3DPT_TRIANGLELIST, 0, m_iNumVertices, iRPC, m_pIndices +
       i*iPCToRender*3, D3DFMT_INDEX16, m_pPMesh->m_pVertices, sizeof(__VertexT1));
        }
    */
