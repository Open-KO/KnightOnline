...
bool CN3FXParticle::Tick()
{
    ...
    if (m_pRefParent->m_bChangeColor && m_fCurrLife >= m_fLife)
        return false;
    if (!m_pRefParent->m_bChangeColor && m_fCurrLife >= (m_pRefParent->m_fFadeIn + m_fLife + m_pRefParent->m_fFadeOut))
        return false;
    ...
    __Vector3 vScale;
    vScale.Set(
        m_fSize + m_pRefParent->m_fScaleVelX * m_fCurrLife,
        m_fSize + m_pRefParent->m_fScaleVelY * m_fCurrLife,
        m_fSize);
    if (vScale.x < 0.0f)
        vScale.x = 0.0f;
    if (vScale.y < 0.0f)
    ...
    else
    {
        ...
        {
            uint32_t Alpha =
                (uint32_t)(255.0f * ((m_pRefParent->m_fFadeIn + m_fLife + m_pRefParent->m_fFadeOut) - m_fCurrLife) /
