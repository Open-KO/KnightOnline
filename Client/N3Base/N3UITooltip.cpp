...
void CN3UITooltip::Render()
{
    ...
    else
    {
        ...
        pVB[0].Set(
            static_cast<float>(m_rcRegion.left),
            static_cast<float>(m_rcRegion.top),
            UI_DEFAULT_Z,
            UI_DEFAULT_RHW,
            BkColor);
        pVB[1].Set(
            static_cast<float>(m_rcRegion.right),
            static_cast<float>(m_rcRegion.top),
            UI_DEFAULT_Z,
            UI_DEFAULT_RHW,
            BkColor);
        pVB[2].Set(
            static_cast<float>(m_rcRegion.right),
            static_cast<float>(m_rcRegion.bottom),
            UI_DEFAULT_Z,
            UI_DEFAULT_RHW,
            BkColor);
        pVB[3].Set(
            ...
            BkColor);
        pVB[4].Set(
            static_cast<float>(m_rcRegion.left) + 1,
            static_cast<float>(m_rcRegion.top) + 1,
            UI_DEFAULT_Z,
            UI_DEFAULT_RHW,
            BorderColorIn);
        pVB[5].Set(
            static_cast<float>(m_rcRegion.right) - 1,
            static_cast<float>(m_rcRegion.top) + 1,
            UI_DEFAULT_Z,
            UI_DEFAULT_RHW,
            BorderColorIn);
        pVB[6].Set(
            static_cast<float>(m_rcRegion.right) - 1,
            static_cast<float>(m_rcRegion.bottom) - 1,
            UI_DEFAULT_Z,
            UI_DEFAULT_RHW,
            BorderColorIn);
        pVB[7].Set(
        ...
        s_lpD3DDev->DrawIndexedPrimitiveUP(
            D3DPT_LINELIST, 0, 8, 8, pIB, D3DFMT_INDEX16, pVB, sizeof(__VertexTransformedColor)); // 테두리 칠하기
