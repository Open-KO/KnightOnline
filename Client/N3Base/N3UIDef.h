...
enum eUI_STATE
{
    ...
    UI_STATE_BUTTON_DOWN_2CHECKDOWN, // 버튼이 임시적으로 눌린 상태(체크 버튼시 사용, 진짜로 눌린 상태가 아니다) 다음에
                                     // down상태로
    UI_STATE_BUTTON_DOWN_2CHECKUP, // 버튼이 임시적으로 눌린 상태(체크 버튼시 사용, 진짜로 눌린 상태가 아니다) 다음에
                                   // normal상태로
...
// const uint32_t UIMSG_AREA_DOWN_FIRST	= 0x00100000;		// Area에 왼쪽 마우스 버튼 처음으로 다운.. - intended for
// the inventory only, but it's unused. Use UIMSG_BUTTON_CLICK instead.
...
const uint32_t UI_MOUSEPROC_CHILDDONESOMETHING =
