...
// 5. 3D공간상에 글씨를 찍을때 가운데 정렬해서 찍고 싶으면 SetText함수에 두번째 파라미터에
// (D3DFONT_3D|D3DFONT_CENTERED)를 넣어준다.
//////////////////////////////////////////////////////////////////////
...
class CDFont : public CN3Base
{
  ...
  public:
    ...
    HRESULT DrawText(
        FLOAT sx, FLOAT sy, uint32_t dwColor, uint32_t dwFlags, FLOAT fZ = 1.0f); // 버퍼에 저장된 글씨를 그린다.(2d)
  ...
  protected:
    void Make2DVertex(
        const int iFontHeight, const std::string &szText); // 입력 받은 문자를 적절하게 배치된 2d 폴리곤으로 만든다.
    void Make3DVertex(
        const int iFontHeight,
        const std::string &szText,
        uint32_t dwFlags); // 입력 받은 문자를 적절하게 배치된 3d 폴리곤을 만든다.
};
