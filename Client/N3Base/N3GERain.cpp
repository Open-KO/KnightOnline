...
void CN3GERain::Create(
    float fDensity, float fWidth, float fHeight, float fRainLength, const __Vector3 &vVelocity, float fTimeToFade)
...
// vVelocity : 빗줄기의 속도
{
    ...
    for (i = 0; i < iRainCount; ++i)
    {
        pVertices[i * 2 + 0].Set(
            ...
            dwColorA);
        pVertices[i * 2 + 1].Set(
            pVertices[i * 2 + 0].x + vAdd.x,
            pVertices[i * 2 + 0].y + vAdd.y,
            pVertices[i * 2 + 0].z + vAdd.z,
            dwColorB);
    }
    m_pVB->Unlock();
}
