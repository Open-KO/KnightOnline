    ...
    for (int i = 0; i < nMC; i++)
    {
        if (m_DeviceInfo.pModes[i].Format == s_DevParam.BackBufferFormat)
        {
            FindDepthStencilFormat(
                0, m_DeviceInfo.DevType, m_DeviceInfo.pModes[i].Format, &s_DevParam.AutoDepthStencilFormat);
...
bool CN3Eng::Init(BOOL bWindowed, HWND hWnd, uint32_t dwWidth, uint32_t dwHeight, uint32_t dwBPP, BOOL bUseHW)
{
...
//		{ for(int iii = 0; iii < 2; iii++) Beep(2000, 200); Sleep(300); } // 여러번 삑~
    ...
    s_DevParam.PresentationInterval = s_Options.bVSyncEnabled ? D3DPRESENT_INTERVAL_ONE : D3DPRESENT_INTERVAL_IMMEDIATE;
    ...
    for (int i = 0; i < nMC; i++)
    {
        ...
        {
            this->FindDepthStencilFormat(
                0,
                m_DeviceInfo.DevType,
                m_DeviceInfo.pModes[i].Format,
    ...
    HRESULT rval =
        m_lpD3D->CreateDevice(0, DevType, s_hWndBase, D3DCREATE_HARDWARE_VERTEXPROCESSING, &s_DevParam, &s_lpD3DDev);
    if (rval != D3D_OK)
    {
    ...
    {
        MessageBox(
            s_hWndBase, "Can't support this graphic card : Texture size is too small", "Initialization error", MB_OK);
#ifdef _N3GAME
    ...
    UINT iAdapter, D3DDEVTYPE DeviceType, D3DFORMAT TargetFormat, D3DFORMAT *pDepthStencilFormat)
{
    ...
    {
        rval = m_lpD3D->CheckDeviceFormat(
            iAdapter, DeviceType, TargetFormat, D3DUSAGE_DEPTHSTENCIL, D3DRTYPE_SURFACE, DepthFmts[i]);
        if (D3D_OK == rval)
        {
...
void CN3Eng::ClearAuto(RECT *pRC)
{
    ...
    else
    {
        ...
        if (bEnable)
        {
            ...
            dwFillColor = D3DCOLOR_ARGB(
                (uint8_t)(Lgt.Diffuse.a * 255.0f),
                (uint8_t)(Lgt.Diffuse.r * 255.0f),
                (uint8_t)(Lgt.Diffuse.g * 255.0f),
