...
void CN3SkyMng::Tick()
{
    ...
    uint32_t dwCurGameTime =
        m_dwCheckGameTime + (uint32_t)((dwCurTickCount - m_dwCheckTick) * TIME_REAL_PER_GAME * 0.001f);
    if (!m_DayChanges.empty())
    {
        ...
        {
            while (m_iWeatherChangeCurPos >= 0 && m_iWeatherChangeCurPos < static_cast<int>(m_WeatherChanges.size()) &&
    ...
    if (m_fCellSize > 0.0f)
    {
        ...
        if (NewCellPos.x != m_CurCellPos.x || NewCellPos.y != m_CurCellPos.y)
        {
            ...
            for (i = 0; i < 3; ++i)
                for (j = 0; j < 3; ++j)
                    m_vPos[j * 3 + i].Set(
                        (m_CurCellPos.x + i - 0.5f) * m_fCellSize, 0, (m_CurCellPos.y + j - 0.5f) * m_fCellSize);
        }
...
bool CN3SkyMng::DayChangeParse(FILE *fp, __SKY_DAYCHANGE *pDayChange)
{
    ...
    sscanf(
        szLine1,
        "Type : [%d], Time : [%d], Parameter1 : [%d], Parameter2 : [%d], Elapse : [%f]",
        &(pDayChange->eSkyDayChange),
        &pDayChange->dwWhen,
        &pDayChange->dwParam1,
        &pDayChange->dwParam2,
        &pDayChange->fHowLong);
            ...
            for (int i = 0; i < iSDCC; i++)
            {
                if (false == this->DayChangeParse(fp, &(m_DayChanges[i])))
                {
                    ...
                    MessageBoxA(
                        CN3Base::s_hWndBase,
                        szErrLine.c_str(),
                        "하늘 환경설정 데이터 Parsing 실패",
                        MB_OK); // CN3Base::s_hWndBase
                    this->Release();
                    return false;
                }
            }
...
void CN3SkyMng::InitToDefaultHardCoding()
{
    ...
    tmpDayChange.Init(
        "MidNight - SunColor", SDC_SUNCOLOR, dwTime, D3DCOLOR_ARGB(255, 30, 36, 85), 0, 150.0f); // 푸르스름
    m_DayChanges.push_back(tmpDayChange);

    tmpDayChange.Init(
        "MidNight - GlowColor", SDC_GLOWCOLOR, dwTime, D3DCOLOR_ARGB(255, 15, 18, 49), 0, 150.0f); // 하늘과 동
    m_DayChanges.push_back(tmpDayChange);

    tmpDayChange.Init(
        "MidNight - FlareColor", SDC_FLARECOLOR, dwTime, D3DCOLOR_ARGB(255, 15, 18, 49), 0, 150.0f); // 하늘과 동
    m_DayChanges.push_back(tmpDayChange);
...
void CN3SkyMng::SetWeather(eSKY_WEATHER eWeather, int iPercentage)
{
    ...
    else if (SW_RAINY == m_eWeather || SW_SNOW == m_eWeather) // 비, 눈 옴
    {
        ...
        tmpWeatherChange.Init(
            "Sun", SDC_SUNCOLOR, dwWhen, 0xff000000, (uint32_t)(this->GetSunRatio() * 1000), fHowLong);
        m_WeatherChanges.push_back(tmpWeatherChange);

        // 해의 flare
        tmpWeatherChange.Init(
            "Flare", SDC_FLARECOLOR, dwWhen, 0xff000000, (uint32_t)(this->GetFlareRatio() * 1000), fHowLong);
        m_WeatherChanges.push_back(tmpWeatherChange);

        // 해의 glow
        tmpWeatherChange.Init(
            "Glow", SDC_GLOWCOLOR, dwWhen, 0xff000000, (uint32_t)(this->GetGlowRatio() * 1000), fHowLong);
        m_WeatherChanges.push_back(tmpWeatherChange);
...
int CN3SkyMng::GetDayChangePos_AfterNSec(uint32_t dwCurGameTime, float fSec)
{
    ...
    // n초후의 queue의 위치 찾기
    while (iCheckDayChangeCurPos >= 0 && iCheckDayChangeCurPos < static_cast<int>(m_DayChanges.size()) &&
           m_DayChanges[m_iDayChangeCurPos].dwWhen < dwCheckGameTime)
...
/*
void CN3SkyMng::ColorDeltaSet(int iPercentage, float fHowLong) // 현재 하늘, 안개 색을 퍼센트 단위로 변화시킨다.. 비, 눈
올때 쓴다..
{
