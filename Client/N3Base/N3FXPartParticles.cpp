...
bool CN3FXPartParticles::ParseScript(char *szCommand, char *szBuff0, char *szBuff1, char *szBuff2, char *szBuff3)
{
    ...
    if (lstrcmpi(szCommand, "<start_range_min>") == 0)
    {
        m_MinCreateRange.Set(
            static_cast<float>(atof(szBuff0)), static_cast<float>(atof(szBuff1)), static_cast<float>(atof(szBuff2)));
        return true;
    }
...
void CN3FXPartParticles::Render()
{
    ...
    for (; it != m_pVBList_Alive.end(); it++)
    {
        ...
        // hr = CN3Base::s_lpD3DDev->DrawIndexedPrimitiveUP(D3DPT_TRIANGLELIST, 0, 4, 2, m_pIB, D3DFMT_INDEX16, m_pVB,
...
void CN3FXPartParticles::Render()
{
    ...
    for(int TexIdx=0;TexIdx<m_iNumTex;TexIdx++)
    {
        ...
        HRESULT hr;
        hr = CN3Base::s_lpD3DDev->DrawPrimitiveUP( D3DPT_TRIANGLELIST, m_pNumTex[TexIdx], &(m_pVB[VBIdx]),
sizeof(__VertexXyzColorT1)); VBIdx += (m_pNumTex[TexIdx]*NUM_VERTEX_PARTICLE); m_pNumTex[TexIdx] = 0;
    }
