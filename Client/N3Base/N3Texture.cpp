...
bool CN3Texture::LoadFromFile(const std::string &szFileName, uint32_t iVer)
{
    ...
    else
    {
        ...
        HRESULT rval = D3DXCreateTextureFromFileEx(
            s_lpD3DDev,
            szFullPath.c_str(),
            D3DX_DEFAULT,
            D3DX_DEFAULT,
...
bool CN3Texture::Save(File &file)
{
        ...
        D3DFMT_DXT5 == sd.Format)
    {
        ...
        for (int i = 0; i < nMMC2; i++)
        {
            ...
            D3DXLoadSurfaceFromSurface(
                lpSurfDest, nullptr, nullptr, lpSurfSrc, nullptr, nullptr, D3DX_FILTER_TRIANGLE, 0); // 서피스 복사.
            int nPixelSize = 2;
    ...
    else // 일반적인 포맷이면.
    {
        ...
        if (nMMC == 1 && m_Header.nWidth >= 512) // 부두를 위해 256 * 256 짜리 하나 더 저장해준다..
        {
            ...
            HRESULT rval = D3DXLoadSurfaceFromSurface(
                lpSurfDest, nullptr, nullptr, lpSurfSrc, nullptr, nullptr, D3DX_FILTER_TRIANGLE, 0); // 서피스 복사.
            lpSurfDest->LockRect(&LR, nullptr, 0);
            for (int y = 0; y < nH; y++)
            {
...
bool CN3Texture::Convert(D3DFORMAT Format, int nWidth, int nHeight, BOOL bGenerateMipMap)
{
    ...
    else
    {
        ...
        lpTexOld->GetSurfaceLevel(0, &lpTSOld);
        D3DXLoadSurfaceFromSurface(
            lpTSNew, nullptr, nullptr, lpTSOld, nullptr, nullptr, D3DX_FILTER_NONE, 0); // 첫번재 레벨 서피스 복사.
        lpTSOld->Release();
        lpTSNew->Release();
    }
...
bool CN3Texture::GenerateMipMap(LPDIRECT3DSURFACE9 lpSurfSrc)
{
    ...
    else // MipMap 이 있으면 그냥 표면만 복사
    {
        if (false == bNeedReleaseSurf) // 다른 서피스에서 복사해야 되는 거면 0 레벨도 복사..
        {
            ...
            HRESULT rval = D3DXLoadSurfaceFromSurface(
                lpSurfDest, nullptr, nullptr, lpSurfSrc, nullptr, nullptr, dwFilter, 0); // 작은 맵 체인에 서피스 이미지
                                                                                         // 축소 복사
            lpSurfDest->Release();
            lpSurfDest = nullptr;
        }
        ...
        for (int i = 1; i < nMMC2; i++)
        {
            ...
            uint32_t dwFilter = D3DX_FILTER_TRIANGLE; // 기본 필터는 없다..
            HRESULT rval = D3DXLoadSurfaceFromSurface(
                lpSurfDest, nullptr, nullptr, lpSurfUp, nullptr, nullptr, dwFilter, 0); // 작은 맵 체인에 서피스 이미지
                                                                                        // 축소 복사
            lpSurfDest->Release();
            lpSurfUp->Release();
        }
