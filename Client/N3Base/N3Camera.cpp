...
BOOL CN3Camera::MoveByWindowMessage(MSG *pMsg)
{
    ...
    else if (pMsg->message == WM_KEYDOWN)
    {
        ...
        case '8':
        case '9': {
            static __Vector3 vEyes[10] = {
                m_vPos, m_vPos, m_vPos, m_vPos, m_vPos, m_vPos, m_vPos, m_vPos, m_vPos, m_vPos};
            static __Vector3 vAts[10] = {m_vAt, m_vAt, m_vAt, m_vAt, m_vAt, m_vAt, m_vAt, m_vAt, m_vAt, m_vAt};
            static __Vector3 vUps[10] = {
                m_vScale, m_vScale, m_vScale, m_vScale, m_vScale, m_vScale, m_vScale, m_vScale, m_vScale, m_vScale};
...
void CN3Camera::Apply()
{
    ...
    // Range Fog : 장점 - 거리기반으로 fog가 적용된다. 단점 - poligon단위로 같은 fog값이 적용된다.(큰 폴리곤이 있을경우
    // 어색한 fog가 될 수 있다.) range fog = FALSE로 했을때는 depth버퍼 기반으로 fog가 계산되어 적용된다.
...
void CN3Camera::Tick(float fFrm)
{
    ...
    if (m_bOrtho)
    {
        ...
        m_Data.mtxProjection.OrthoLH(
            fL, fL / m_Data.fAspect, m_Data.fNP * (1.0f + fL / 1000.0f), m_Data.fFP); // Projection Matrix Setting
