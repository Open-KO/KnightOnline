...
bool CN3FXPMeshInstance::SplitOne()
{
    ...
    // 이렇게 해도 괜찮을 수 있도록 방어코드를 넣었다. m_pFXPMesh->m_pCollapses 를 할당할때 1개 더 할당하고 마지막
    // 데이터를 초기값으로 넣었다.
    //	if (m_pCollapseUpTo >= m_pFXPMesh->m_pCollapses + m_pFXPMesh->m_iNumCollapses - 1) return false; // 이게
    //정상이다..
