...
void CN3GESnow::Tick()
{
    ...
    for (i = 0; i < iActiveCount; ++i)
    {
        ...
        if (fDiff < 0) // 높이 범위를 벗어났을 경우
        {
            ...
            pParticle->vOffset2.Set(
                cosf(fRadian) * m_fSnowSize / 2, -sqrt3 * m_fSnowSize / 6.f, sinf(fRadian) * m_fSnowSize / 2);
            pParticle->vOffset3.Set(
                -cosf(fRadian) * m_fSnowSize / 2, -sqrt3 * m_fSnowSize / 6.f, -sinf(fRadian) * m_fSnowSize / 2);
        }
        else
        {
...
void CN3GESnow::Create(
    float fDensity, float fWidth, float fHeight, float fSnowSize, const __Vector3 &vVelocity, float fTimeToFade)
{
    ...
    HRESULT hr = s_lpD3DDev->CreateVertexBuffer(
        m_iVC * sizeof(__VertexXyzT1), D3DUSAGE_DYNAMIC, FVF_XYZT1, D3DPOOL_DEFAULT, &m_pVB, nullptr);
    if (FAILED(hr))
        return;
    __VertexXyzT1 *pVertices;
    ...
    for (i = 0; i < iSnowCount; ++i)
    {
        ...
        //		m_pSnowParticle[i].vOffset2.Set(cosf(fRadian)*fSnowSize/2, -sqrt3*fSnowSize/6.f,
        //sinf(fRadian)*fSnowSize/2); 		m_pSnowParticle[i].vOffset3.Set(-cosf(fRadian)*fSnowSize/2, -sqrt3*fSnowSize/6.f,
        //-sinf(fRadian)*fSnowSize/2);

        //		이등변 삼각형(밑변의 길이 fSnowSize, 높이 fSnowSize)
        m_pSnowParticle[i].vOffset1.Set(0, fSnowSize / 2.f, 0);
        m_pSnowParticle[i].vOffset2.Set(
            cosf(fRadian) * fSnowSize / 2.f, -fSnowSize / 2.f, sinf(fRadian) * fSnowSize / 2.f);
        m_pSnowParticle[i].vOffset3.Set(
            -cosf(fRadian) * fSnowSize / 2.f, -fSnowSize / 2.f, -sinf(fRadian) * fSnowSize / 2.f);
