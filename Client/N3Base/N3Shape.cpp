...
void CN3SPart::Tick(
    const __Matrix44 &mtxParent,
    const __Quaternion &qRot,
    float fScale) // timeGetTime 으로 얻은 값을 넣으면 Texture Animation 을 컨트롤 한다..
{
    ...
    if (m_Mtl.nRenderFlags & RF_WINDY) // 바람에 살짝 날려야 하면..
    {
        ...
        //		else if(m_vWindFactorToReach != m_vWindFactorCur)
        {
            ...
            //			if(std::abs(m_vWindFactorToReach.x - m_vWindFactorCur.x) < fFactor) m_vWindFactorCur.x =
            //m_vWindFactorToReach.x; 			if(std::abs(m_vWindFactorToReach.y - m_vWindFactorCur.y) < fFactor)
            //m_vWindFactorCur.y = m_vWindFactorToReach.y; 			if(std::abs(m_vWindFactorToReach.z - m_vWindFactorCur.z) <
            //fFactor) m_vWindFactorCur.z = m_vWindFactorToReach.z;
            if (std::abs(m_fWindFactorToReach - m_fWindFactorCur) < fFactor)
...
bool CN3SPart::Save(File &file)
{
    ...
    if (pPMesh != nullptr)
        ...
        MessageBox(
            s_hWndBase,
            "Progressive mesh pointer is NULL! : object가 제대로 보이지 않을 수 있습니다.(리소스 파일이 Load되지 "
            "않았을 가능성이 큼)",
            "warning",
            MB_OK);
    ...
    if (nL > 0)
    {
        ...
        //		if(-1 == pPMesh->FileName().find("object\\")) // 임시로 경로를 바꾸려고 넣었다.. 나중에 필요없음
        //지운다..
        //		{
    ...
    for (int j = 0; j < iTC; j++) // Texture File 이름 쓰기...
    {
        ...
        if (nL > 0)
        {
            ...
            //			if(-1 == m_TexRefs[j]->FileName().find("object\\")) // 임시로 경로를 바꾸려고 넣었다.. 나중에
            //필요없음 지운다..
            //			{
            //				// 폴더 이름을 분리하고..
...
{
    if (iPart < 0 || iPart >= static_cast<int>(m_Parts.size()))
        return;
...
int CN3Shape::CheckCollisionPrecisely(
    bool bIgnoreBoxCheck, int ixScreen, int iyScreen, __Vector3 *pVCol, __Vector3 *pVNormal)
{
    ...
    vTmp.y =
        -(((2.0f * iyScreen) / (CN3Base::s_CameraData.vp.Height)) - 1) / CN3Base::s_CameraData.mtxProjection.m[1][1];
    vTmp.z = 1.0f;
...
int CN3Shape::CheckCollisionPrecisely(
    bool bIgnoreBoxCheck, const __Vector3 &vPos, const __Vector3 &vDir, __Vector3 *pVCol, __Vector3 *pVNormal)
{
    // 박스 체크 먼저한다..
    if (!bIgnoreBoxCheck && !_CheckCollisionByBox(vPos, vDir, m_vMin * m_Matrix, m_vMax * m_Matrix))
        return -1;
...
{
    if (iPartIndex < 0 || iPartIndex >= static_cast<int>(m_Parts.size()))
        return;
