...
void CN3Transform::GenerateSelectBox(__Vector3 &vMin, __Vector3 &vMax)
{
    ...
    pVC[4] =  pVC[2] =  pVC[0] =  __VertexColor(vMin.x, vMax.y, vMin.z, 0xff7f7f7f);  // 정면 - 아래와 같은 선 리스트를
만든다. pVC[10] = pVC[8] =  pVC[6] =  __VertexColor(vMax.x, vMax.y, vMin.z, 0xff7f7f7f);  //  /          / pVC[16] =
pVC[14] = pVC[12] = __VertexColor(vMax.x, vMin.y, vMin.z, 0xff7f7f7f);  // +--      --+ pVC[22] = pVC[20] = pVC[18] =
__VertexColor(vMin.x, vMin.y, vMin.z, 0xff7f7f7f);  // |          | pVC[28] = pVC[26] = pVC[24] = __VertexColor(vMax.x,
vMax.y, vMax.z, 0xff7f7f7f);  // pVC[34] = pVC[32] = pVC[30] = __VertexColor(vMin.x, vMax.y, vMax.z, 0xff7f7f7f);  // |/
|/ pVC[40] = pVC[38] = pVC[36] = __VertexColor(vMin.x, vMin.y, vMax.z, 0xff7f7f7f);  // +--      --+ pVC[46] = pVC[44] =
pVC[42] = __VertexColor(vMax.x, vMin.y, vMax.z, 0xff7f7f7f);  //
