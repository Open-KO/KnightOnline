...
void AudioDecoderThread::decode_impl_pcm(StreamedAudioHandle *handle)
{
    ...
    for (size_t i = 0; i < ChunksToDecode; i++)
    {
        ...
        const size_t bytesRemaining = fileSize > fileReaderHandle.Offset ? fileSize - fileReaderHandle.Offset : 0;
        ...
        std::memcpy(
            &decodedChunk.Data[0], static_cast<const uint8_t *>(file->Memory()) + fileReaderHandle.Offset, bytesToRead);
