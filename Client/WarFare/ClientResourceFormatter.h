...
template <typename... Args> inline std::string format_text_resource(uint32_t resourceId, Args &&...args)
{
    std::string fmtStr;
...
#if defined(_N3GAME)
            CLogWriter::Write("format_text_resource({}) failed - invalid args for format string.", resourceId);
#endif
        }
