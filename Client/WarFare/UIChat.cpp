...
void CUIChat::CreateLines()
{
    ...
    for (i = 0; i < m_iChatLineCount; ++i)
    {
        ...
        SetRect(
            &rc,
            m_rcChatOutRegion.left,
            m_rcChatOutRegion.top + (i * size.cy),
            m_rcChatOutRegion.right,
...
void CUIChat::AddChatMsg(e_ChatMode eCM, const std::string &szString, D3DCOLOR color)
{
    ...
    //	N3_CHAT_NORMAL = 1, N3_CHAT_PRIVATE, N3_CHAT_PARTY, N3_CHAT_FORCE, N3_CHAT_SHOUT, N3_CHAT_KNIGHTS,
    // N3_CHAT_PUBLIC 	e_ChatBuffer eCB = CHAT_BUFFER_NORMAL; // Chatting Mode Index 0 - Normal, shout, notice | 1 -
    // private | 2 - party force | 3 - Knights 	switch(eCM)
    //	{
    //	case N3_CHAT_NORMAL:
    //	case N3_CHAT_SHOUT:
    //	case N3_CHAT_PUBLIC:
...
void CUIChat::AdjustScroll()
{
    ...
    //	while (m_LineBuffers[m_eChatBuffer].size() > MAX_CHAT_LINES && 0 < iCurLinePos)	// MAX_CHAT_LINES은 최대 line의
    // 수 (단 스크롤바가 0인 곳에 있으면 line을 지우지 않으므로 500개를 넘길 수 있다)
    while (m_LineBuffer.size() > MAX_CHAT_LINES &&
           0 < iCurLinePos) // MAX_CHAT_LINES은 최대 line의 수 (단 스크롤바가 0인 곳에 있으면 line을 지우지 않으므로
                            // 500개를 넘길 수 있다)
    {
        // 한줄 지우기
...
BOOL CUIChat::MoveOffset(int iOffsetX, int iOffsetY)
{
    ...
    // NOTE: (madpew) Don't stick them together, so chat and info can move freely, but sync with the folded version
    // instead
