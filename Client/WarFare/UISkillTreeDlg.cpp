...
CUISkillTreeDlg::~CUISkillTreeDlg()
{
    ...
    /*	int i, j, k;
        ...
        if ( (CN3UIWndBase::s_sSkillSelectInfo.pSkillDoneInfo != nullptr) && (CN3UIWndBase::s_sSkillSelectInfo.UIWnd ==
       UIWND_SKILL_TREE) )
        {
...
void CUISkillTreeDlg::Release()
{
    ...
    if ((CN3UIWndBase::s_sSkillSelectInfo.pSkillDoneInfo != nullptr) &&
...
void CUISkillTreeDlg::Render()
{
    ...
    for (UIListReverseItor itor = m_Children.rbegin(); m_Children.rend() != itor; ++itor)
    {
        ...
        if ((GetState() == UI_STATE_ICON_MOVING) && (pChild->UIType() == UI_TYPE_ICON) &&
            (CN3UIWndBase::s_sSkillSelectInfo.pSkillDoneInfo) &&
...
void CUISkillTreeDlg::PageButtonInitialize()
{
    ...
    // 서버에게 받은 값으로 세팅.. m_iSkillInfo[MAX_SKILL_FROM_SERVER];										// 서버로
    // 받는 슬롯 정보..
...
void CUISkillTreeDlg::SetPageInIconRegion(int iKindOf, int iPageNum) // 아이콘 역역에서 현재 페이지 설정..
{
    ...
    // 아이콘 설명 문자열 업데이트.. 현재 스킬 종류와 현재 스킬 페이지중 아이콘이 보이면 String보이게.. 아니면
    // 안보이게..
