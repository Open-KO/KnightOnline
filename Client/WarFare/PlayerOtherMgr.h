...
class CPlayerOtherMgr : public CGameBase
{
  ...
  public:
    ...
    //	CPlayerOther*		UPCGetByName(const char* szID);					// User Player Character 와 NPC 를 조사해서
    //포인터를 가져온다.
    CPlayerOther *UPCGetByID(
        int iID, bool bFromAliveOnly); // User Player Character 와 NPC 를 조사해서 포인터를 가져온다.
    //	CPlayerNPC*			NPCGetByName(const char* szID);					// User Player Character 와 NPC 를 조사해서
    //포인터를 가져온다.
