...
/*
    ...
    for(int i = 0; i < 4; i++)
        m_vShadows[i].y = s_pTerrain->GetHeight(pAP->MtxWorld.m[3][0] + m_vShadows[i].x, pAP->MtxWorld.m[3][2] +
m_vShadows[i].z);
...
void CPlayerBase::RenderChrInRect(CN3Chr *pChr, const RECT &Rect)
{
    ...
    float fViewVolumeWidth =
        fChrHeight * vp.Width / iHeight; // 가로는 pRect의 가로 세로 비율에 맞게 (클리핑 될 경우 클리핑 되는 비율에 맞게
                                         // 좁혀준다.) 원래는 이거 : fChrHeight * iWidth / iHeight * vp.Width / iWidth;
    ...
    if (rcViewport.left != Rect.left)
        fCameraMoveX = -fCameraMoveX; // 왼쪽 영역이 짤리게 그려야 하므로 카메라를 오른쪽(카메라가 -Z축을 바라보기
                                      // 때문에 카메라의 오른쪽은 -X쪽이다.)으로 이동
    if (rcViewport.top != Rect.top)
        fCameraMoveY = -fCameraMoveY; // 위쪽 영역이 짤리게 그려야 하므로 카메라를 아래쪽으로 이동

    //	mtxView.LookAtLH(	{ 0.0f + fCameraMoveX, fVCenter+2.0f + fCameraMoveY, 10.0f },	// 여기서 View matrix는
    //카메라 각도와 상관있다. 거리는 원근에 아무 영향을 미치지 않는다. 						{ 0.0f + fCameraMoveX, fVCenter + fCameraMoveY,
    //0.0f },	// fVCenter: 캐릭터 키의 중간을 바라보기 						{ 0.0f, 1.0f, 0.0f } );
...
bool CPlayerBase::Action(e_StateAction eState, bool bLooping, CPlayerBase *pTarget, bool bForceSet)
{
    ...
    case PSA_DEATH:
        ...
        this->RegenerateCollisionMesh(); // 최대 최소값을 다시 찾고 충돌메시를 다시 만든다.. -> 죽은 넘 피킹을 정확하게
                                         // 하기위해서이다.
        return true; // 돌아간다!!!
...
bool CPlayerBase::ProcessAttack(CPlayerBase *pTarget)
{
    ...
    if (pTarget->m_fTimeAfterDeath > 0 && !pTarget->IsDead())
    {
        ...
        if (ITEM_CLASS_SWORD_2H == eICR || ITEM_CLASS_AXE_2H == eICR || ITEM_CLASS_MACE_2H == eICR ||
            ...
            ITEM_CLASS_SWORD == eICR || ITEM_CLASS_AXE == eICR || ITEM_CLASS_MACE == eICR || ITEM_CLASS_SPEAR == eICR)
...
bool CPlayerBase::TryWeaponElementEffect(
    e_PlugPosition plugPosition, const CPlayerBase &target, __Vector3 collisionPosition)
{
    bool affected = false;
    ...
    if ((m_pItemPlugExts[plugPosition]->byMagicOrRare == ITEM_ATTRIB_UNIQUE &&
        ...
        (m_pItemPlugExts[plugPosition]->byMagicOrRare == ITEM_ATTRIB_UNIQUE &&
...
e_Ani CPlayerBase::JudgeAnimationBreath()
{
    ...
    else // 플레이어 일경우..
    {
        ...
        if (pTarget != nullptr && IsHostileTarget(pTarget))
        {
            ...
            //			if(m_pItemPlugBasics[PLUG_POS_LEFTHAND]) fIWL = m_pItemPlugBasics[PLUG_POS_LEFTHAND]->siWeight
            /// 10.f;
...
e_Ani CPlayerBase::JudgeAnimationWalk() // 걷기 모드판단하기.. 가진 아이템과 타겟이 있는냐에 따라 다른 에니메이션
                                        // 인덱스를 리턴.
{
    e_Ani eAni = ANI_WALK;
...
e_Ani CPlayerBase::JudgeAnimationRun() // 걷기 모드판단하기.. 가진 아이템과 타겟이 있는냐에 따라 다른 에니메이션
                                       // 인덱스를 리턴.
{
    e_Ani eAni = ANI_RUN;
...
e_Ani CPlayerBase::JudgeAnimationWalkBackward() // 걷기 모드판단하기.. 가진 아이템과 타겟이 있는냐에 따라 다른
                                                // 에니메이션 인덱스를 리턴.
{
...
bool CPlayerBase::CheckCollisionToTargetByPlug(CPlayerBase *pTarget, int nPlug, __Vector3 *pVCol)
{
    ...
    //	if(pPlugNormal->m_fTrace0 >= pPlugNormal->m_fTrace1) return false; // 무기의 길이 정보가 없거나 이상하면
    //충돌체크 하지 않는다.
    ...
    e_PlugPosition ePos, const std::string &szFN, __TABLE_ITEM_BASIC *pItemBasic, __TABLE_ITEM_EXT *pItemExt)
{
    ...
    if (pItemExt)
    {
            ...
            (pItemExt->byDamageFire >= LIMIT_FX_DAMAGE)) // 17 추가데미지 - 불
        {
            ...
            (pItemExt->byMagicOrRare == ITEM_ATTRIB_UNIQUE && pItemExt->byDamageIce > 0) ||
            (pItemExt->byDamageIce >= LIMIT_FX_DAMAGE)) // 18 추가데미지 - 얼음
        {
            ...
            (pItemExt->byMagicOrRare == ITEM_ATTRIB_UNIQUE && pItemExt->byDamageThuner > 0) ||
            (pItemExt->byDamageThuner >= LIMIT_FX_DAMAGE)) // 19 추가데미지 - 전격
        {
            ...
            (pItemExt->byMagicOrRare == ITEM_ATTRIB_UNIQUE && pItemExt->byDamagePoison > 0) ||
...
CN3CPart *CPlayerBase::PartSet(
    e_PartPosition ePos, const std::string &szFN, __TABLE_ITEM_BASIC *pItemBasic, __TABLE_ITEM_EXT *pItemExt)
{
        ...
        if (pItemBasic) // 입히는 경우
        {
                ...
                m_Chr.Part(PART_POS_LOWER)) // 로브 타입의 통짜 윗옷이고 아래에 뭔가 입고 있으면..
            {
                this->PartSet(
                    PART_POS_LOWER,
                    "",
                    m_pItemPartBasics[PART_POS_LOWER],
            ...
            if (m_pItemPartBasics[ePos] && m_pItemPartBasics[ePos]->byIsRobeType) // 전에 착용했던게 로브면..
            {
                if (m_pItemPartBasics[PART_POS_LOWER]) // 하체에 아이템이 입혀있으면..
                {
                    ...
                    MakeResrcFileNameForUPC(
                        m_pItemPartBasics[PART_POS_LOWER],
                        m_pItemPartExts[PART_POS_LOWER],
                        &szFN2,
                        nullptr,
                        ePartPos2,
                        ePlugPos2,
                ...
                else // 하체에 입고 있었던 아이템이 없다면..
                {
                    ...
                    this->PartSet(
                        PART_POS_LOWER, pLooks->szPartFNs[PART_POS_LOWER], nullptr, nullptr); // 하체에 기본옷을 입힌다.
                }
            }
        }
    }
...
CPlayerBase *CPlayerBase::TargetPointerCheck(bool bMustAlive)
{
    ...
    //	if(pTarget && (PSA_DEATH == pTarget->State())) pTarget = nullptr; //픽킹에 의해서 선택된 캐릭을 중간의
    //임의적으로 수정 못하게 막음
...
void CPlayerBase::RenderShadow(float fAngle)
{
    ...
    DWORD dwAlpha, dwFog, dwCull, dwColorVertex, dwMaterial, dwZWrite, dwColorOp0, dwColorArg01, dwColorArg02,
    ...
    */
    ...
    CN3Base::s_lpD3DDev->DrawIndexedPrimitiveUP(
        D3DPT_TRIANGLELIST, 0, 4, 2, m_pIndex, D3DFMT_INDEX16, m_vTVertex, sizeof(__VertexT1));
...
void CPlayerBase::CalcPart(CN3CPart *pPart, int nLOD, __Vector3 vDir)
{
    ...
    for (int i = 0; i < iTotalCount; i++)
    {
        if (pVDest)
        {
            vec = pVDest[i];
            vec *= m_Chr.m_Matrix;
            // vec.x = vec.x*m_Chr.m_Matrix.m[0][0] + vec.y*m_Chr.m_Matrix.m[1][0] + vec.z*m_Chr.m_Matrix.m[2][0] +
            // m_Chr.m_Matrix.m[3][0]; vec.y = vec.x*m_Chr.m_Matrix.m[0][1] + vec.y*m_Chr.m_Matrix.m[1][1] +
            // vec.z*m_Chr.m_Matrix.m[2][1] + m_Chr.m_Matrix.m[3][1]; vec.z = vec.x*m_Chr.m_Matrix.m[0][2] +
            // vec.y*m_Chr.m_Matrix.m[1][2] + vec.z*m_Chr.m_Matrix.m[2][2] + m_Chr.m_Matrix.m[3][2];
...
void CPlayerBase::CalcPlug(CN3CPlugBase *pPlug, const __Matrix44 *pmtxJoint, __Matrix44 &mtxMV, __Vector3 vDir)
{
    ...
    CN3Base::s_RenderInfo.nChr_Plug_Polygon +=
        pPlug->PMeshInst()->GetNumIndices() / 3; // Rendering Information Update...
#endif
