...
bool CN3Terrain::CheckRenderablePatch()
{
    ...
    for (x = m_pat_BoundRect.left; x <= m_pat_BoundRect.right; x++)
    {
        for (z = m_pat_BoundRect.top; z <= m_pat_BoundRect.bottom; z++)
        {
            ...
            CenterPoint.Set(
                (float)((m_ppPatch[x][z].m_ti_LBPoint.x + (PATCH_TILE_SIZE >> 1)) * TILE_SIZE),
...
BOOL CN3Terrain::Pick(int x, int y, __Vector3 &vPick)
{
    ...
    if ((ix + iz) % 2 == 0)
    {
        ...
        B.Set(
            (float)(ix + 1) * TILE_SIZE,
            GetHeight((ix + 1) * TILE_SIZE, (iz + 1) * TILE_SIZE),
    ...
    while ((vPosCur.x >= 0.0f) && (vPosCur.z >= 0.0f) && (IsInTerrain(vPosCur.x, vPosCur.z)))
    {
        ...
        for (int i = 0; i < 10; i++)
        {
            ...
            if ((ix + iz) % 2 == 1) // 당근.. 왼손 바인딩...
            {
                ...
                C.Set(
                    (float)(ix + 1) * TILE_SIZE,
                    GetHeight((ix + 1) * TILE_SIZE, iz * TILE_SIZE),
                    (float)iz * TILE_SIZE);
                B.Set(
                    (float)ix * TILE_SIZE,
                    GetHeight(ix * TILE_SIZE, (iz + 1) * TILE_SIZE),
                    (float)(iz + 1) * TILE_SIZE);
