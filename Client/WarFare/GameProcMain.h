...
class CGameProcMain : public CGameProcedure
{
  ...
  protected:
    ...
    bool MsgRecv_UserInAndRequest(Packet &pkt); // 주위 영역의 모든 User 아이디를 카운트만큼 받는다... 글구.. 업데이트가
                                                // 필요한 것만 서버에게 요청..
    ...
    bool MsgRecv_NPCInAndRequest(Packet &pkt); // 주위 영역의 모든 NPC 아이디를 카운트만큼 받는다... 글구.. 업데이트가
                                               // 필요한 것만 서버에게 요청..
    ...
    //	void	MsgRecv_ServerCheckAndRequestConcurrentUserCount(Packet& pkt);			// 서버 IP 와 포트를 받아
    //동접자를 체크해 본다..
  ...
  public:
    ...
    const __InfoPartyOrForce *PartyOrForceConditionGet(
    ...
    void MsgSend_Move(
        bool bMove, bool bContinous); // 서버에게 움직임 패킷을 날린다.. // 움직이는가 ? 주기적으로 움직이는 건가?
    ...
    void MsgSend_RequestTargetHP(
        int16_t siIDTarget, uint8_t byUpdateImmediately); // 0x00 - 점차 늘어나게끔.. 0x01 - 즉시 업데이트..
