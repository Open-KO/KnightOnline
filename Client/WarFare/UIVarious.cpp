...
{
    __ASSERT(
        iVal >= 0 && iVal < 10000 && iValMax >= 0 && iValMax < 10000,
        "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
...
void CUIKnights::RemoveButtonHandler() // TODO: @Demircivi, maybe add confirmation dialog in further development? just
                                       // saying.
{
    ...
    for (i = 0; i < 10; i++)
    {
        ...
        if (KMI.iConnected)
        {
            ...
            int index = m_pList_CharGrades->AddString(
                szDuty); // TODO: @Demircivi, Char Grade is not loading from language files.
...
void CUIFriends::MsgSend_MemberInfo(bool bDisableInterval)
{
    ...
    CAPISocket::MP_AddByte(
        &(buffers[0]), iOffset, WIZ_FRIEND_PROCESS); // 친구 정보.. Send s1(이름길이), str1(유저이름) | Receive
                                                     // s1(이름길이), str1(유저이름), s1(ID), b2(접속, 파티)
...
void CUIFriends::MsgSend_MemberInfo(const std::string &szID)
{
    ...
    CAPISocket::MP_AddByte(
        byBuff, iOffset, WIZ_FRIEND_PROCESS); // 친구 정보.. Send s1(이름길이), str1(유저이름) | Receive s1(이름길이),
                                              // str1(유저이름), s1(ID), b2(접속, 파티)
...
void CUIFriends::MsgRecv_MemberInfo(Packet &pkt)
{
    ...
    for (int i = 0; i < iFC; i++)
    {
        iLen = pkt.read<int16_t>(); // 친구 정보.. Send s1(이름길이), str1(유저이름) | Receive s1(이름길이),
                                    // str1(유저이름), s1(ID), b2(접속, 파티)
...
void CUIVarious::UpdateAllStates(const __InfoPlayerBase *pInfoBase, const __InfoPlayerMySelf *pInfoExt)
{
    ...
    // 기사단 관련 정보 업데이트...
    m_pPageState->UpdateRealmPoint(
        pInfoExt->iRealmPoint, pInfoExt->iRealmPointMonthly); // Edited by @Demircivi while integrating monthly np
                                                              // system.  // 국가 기여도는 10을 나누어서 표시
