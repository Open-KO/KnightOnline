...
// This is the maximum time we must wait after sending the WIZ_VERSION_CHECK packet on the login scene, before we're
// allowed to attempt to re-establish a connection to the game server. We'll use 5 seconds here as it's a more than
// reasonable enough time for it to receive a packet, even with lag, while not being too excessive. If the client
// disconnects in this time, this timer will be reset, so there's no need to account for this. Officially it Sleep()s
// for 1 second prior to even sending WIZ_VERSION_CHECK packet we need to wait for, so there's not really a comparable
// official limit. All this accomplishes is reducing the number of connection attempts, not preventing overlaps.
...
enum e_StateDying
{
    PSD_DISJOINT = 0, // Dies with a twisting/rolling death animation. NOTE: The original comment indicated the body
                      // physically breaking apart, but this is misleading -- the actual animations for players and NPCs
...
struct __TABLE_ITEM_BASIC
{
    uint32_t dwID; // 01 Encoded item number: first 2 digits = item type, next 2 digits = equip position (used to
                   // determine Plugs or Parts), last 4 digits = item index
    ...
    uint8_t byAttachPoint; // 13 Equip position — identifies the specific slot on the character's body where this item
                           // is equipped
...
struct __TABLE_ITEM_EXT
{
    ...
    int16_t
        siDefenseRateBlow; // 18 Defense against blunt weapons [maces/clubs] (percentage modifier: 20% = 120% defense)
    ...
    uint8_t bySoulBind; // 30 Soul bind — percentage chance of dropping this item upon death in one-on-one combat; not
                        // currently in use.
...
struct __TABLE_UPC_SKILL_TYPE_6
{
    ...
    uint32_t
        dwSkillSuccessRate; // 24 NOTE: These columns may be shuffled slightly, the naming is based on the server data
...
constexpr int MAX_ITEM_SLOT_OPC = 8; // Max equipment slots for other players (including NPCs): 0-4 = upper body, lower
                                     // body, helmet, arms, legs; 5 = cloak; 6 = right hand; 7 = left hand
...
enum e_Behavior
{
    ...
    BEHAVIOR_PERSONAL_TRADE_FMT_WAIT, // Private trade: Wait for other player to accept our trade request [does
                                      // nothing].
