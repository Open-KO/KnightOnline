...
void CGameProcedure::StaticMemberInit(HINSTANCE hInstance, HWND hWndMain)
{
    ...
    s_pEng = new CGameEng();
    if (!s_pEng->Init(
            s_bWindowed, hWndMain, s_Options.iViewWidth, s_Options.iViewHeight, s_Options.iViewColorDepth, TRUE))
        exit(-1);
...
void CGameProcedure::StaticMemberRelease()
{
    ...
    s_pProcOption = nullptr; // 게임 옵션 프로시져
...
void CGameProcedure::Tick()
{
    ...
    if (s_pLocalInput->IsKeyPress(DIK_NUMPADMINUS)) // 키패드의 마이너스 키를 누르면..
    {
        ...
        std::string szFN =
            fmt::format("{}_{}_{}_{}.{}.{}.ksc", st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute, st.wSecond);
    ...
    */
    ...
    /*
        ...
        if(lpDDSBack)
        {
            ...
            if(lpDDSTmp)
            {
                HRESULT rval = D3DXLoadSurfaceFromSurface(lpDDSTmp, nullptr, nullptr, lpDDSBack, nullptr, nullptr,
    D3DX_FILTER_NONE, 0);
...
std::string CGameProcedure::MessageBoxPost(
    const std::string &szMsg, const std::string &szTitle, int iStyle, e_Behavior eBehavior)
{
...
void CGameProcedure::ReportServerConnectionClosed(bool bNeedQuitGame)
{
    ...
    if (s_pPlayer != nullptr)
    {
        ...
        CLogWriter::Write(
            "Socket Closed... Zone({}) Pos({:.1f}, {:.1f}, {:.1f}) Exp({})",
            s_pPlayer->m_InfoExt.iZoneCur,
            vPos.x,
            vPos.y,
            vPos.z,
            s_pPlayer->m_InfoExt.iExp);
    }
    else
    {
...
void CGameProcedure::MsgSend_CharacterSelect() // virtual
{
    ...
    CLogWriter::Write(
        "MsgSend_CharacterSelect - name({}) zone({})",
        s_pPlayer->IDString(),
...
void CGameProcedure::MsgRecv_CompressedPacket(Packet &pkt) // 압축된 데이터 이다... 한번 더 파싱해야 한다!!!
{
    ...
    uint32_t decompressedLength =
        lzf_decompress(pkt.contents() + pkt.rpos(), compressedLength, &decompressedBuffer[0], originalLength);
...
bool CGameProcedure::MsgRecv_CharacterSelect(Packet &pkt) // virtual
{
    ...
    if (1 == iResult)                  // 성공..
    {
        ...
        CLogWriter::Write(
            "MsgRecv_CharacterSelect - name({}) zone({} -> {})", s_pPlayer->m_InfoBase.szID, iZonePrev, iZoneCur);
        return true;
    }
    else // 실패
    {
