...
void CPlayerOtherMgr::Tick(const __Vector3 &vPosPlayer)
{
    ...
    for (; it2 != itEnd2;)
    {
        ...
        else if (pNPC->m_InfoBase.eNation == NATION_KARUS || pNPC->m_InfoBase.eNation == NATION_ELMORAD)
        {
            ...
            float fDist = pNPC->Distance(vPosPlayer);
            if (fDist < SOUND_RANGE_TO_SET)
                pNPC->SetSoundAndInitFont(D3DFONT_BOLD); // SOUND_RANGE 안에 있으면.
            else if (fDist > SOUND_RANGE_TO_RELEASE)
                pNPC->ReleaseSoundAndFont();
            it2++;
        }
...
CPlayerNPC *CPlayerOtherMgr::CorpseGetNearstNPC(
    bool bMustHaveItem, e_Nation eNation, const __Vector3 &vPosPlayer) // 가장 가까운 적 시체 가져오기..
{
...
int CPlayerOtherMgr::SortByCameraDistance(const void *pArg1, const void *pArg2)
{
    ...
    if (pPlayer1)
        fDist1 = (CN3Base::s_CameraData.vEye - pPlayer1->Position()).Magnitude();
    if (pPlayer2)
        fDist2 = (CN3Base::s_CameraData.vEye - pPlayer2->Position()).Magnitude();
