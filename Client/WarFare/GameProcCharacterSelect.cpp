...
void CGameProcCharacterSelect::Tick()
{
    ...
    if (s_pUIMgr->m_bDoneSomething == false && s_pUIMgr->EnableOperation()) // 패킷을 받기 전에 아무짓도 못하게 한다.
    {
        ...
        if (nMFlags & MOUSE_LBCLICK)                 // 누르는 순간..
        {
            ...
            RECT rc = {
                (int)(vp.Width * 0.36f), (int)(vp.Height * 0.44f), (int)(vp.Width * 0.64f), (int)(vp.Height * 0.86f)};
...
void CGameProcCharacterSelect::AddChr(e_ChrPos eCP, __CharacterSelectInfo *pInfo)
{
    ...
    if (pItemUpper && pItemUpper->byIsRobeType)
        ...
        this->AddChrPart(
            iPosIndex, pLooks, PART_POS_LOWER, pInfo->dwItemLower, pInfo->iItemLowerDurability); // 아님 입히고...
    // 팔
...
void CGameProcCharacterSelect::AddChrPart(
    int iPosIndex, const __TABLE_PLAYER_LOOKS *pLooks, e_PartPosition ePartPos, uint32_t dwItemID, int iItemDurability)
{
    ...
    MakeResrcFileNameForUPC(
        pItem, pItemExt, &szResrcFN, nullptr, ePartPosTmp, ePlugPosTmp, m_InfoChrs[iPosIndex].eRace);
    if (szResrcFN.empty())
...
void CGameProcCharacterSelect::ProcessOnReturn()
{
    ...
    if (m_eCurProcess != PROCESS_ROTATEING)
    {
        ...
        CGameProcedure::ProcActiveSet(
            (CGameProcedure *)s_pProcMain); // 캐릭터 고르기에 성공하면.. 메인으로 가자!!
...
void CGameProcCharacterSelect::FadeOutRender()
{
    ...
    pVertices[1].Set(
        (float)s_CameraData.vp.Width, 0.0f, 0.000002f, 0.99f, D3DCOLOR_ARGB((uint8_t)m_fFadeOut, 0x00, 0x00, 0x00));
    pVertices[2].Set(
        (float)s_CameraData.vp.Width,
        (float)s_CameraData.vp.Height,
        0.000002f,
        0.99f,
        D3DCOLOR_ARGB((uint8_t)m_fFadeOut, 0x00, 0x00, 0x00));
    pVertices[3].Set(
        0.0f, (float)s_CameraData.vp.Height, 0.000002f, 0.99f, D3DCOLOR_ARGB((uint8_t)m_fFadeOut, 0x00, 0x00, 0x00));
