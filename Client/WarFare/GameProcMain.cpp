...
bool CGameProcMain::ProcessPacket(Packet &pkt)
{
    ...
    case WIZ_REGENE: {
        ...
        std::string szMsg =
            "Press OK to teleport back to the re-spawn point."; // IDS_REGENERATION (3701) in 1.298 client.
...
bool CGameProcMain::MsgRecv_MyInfo_All(Packet &pkt)
{
    ...
    {
        CLogWriter::Write(
            "CGameProcMain::MsgRecv_MyInfo_All : failed find character resource data (Race : {})",
    ...
    // 기본값 읽기..
    int iRun = 1;
    //	if(false == CGameProcedure::RegGetSetting("UserRun", &iRun, 4)) iRun = 1; // 걷고 뛰는 상태를 레지스트리에서
    //읽고.. 기본값은 뛰는 상태이다.. 	if(1 == iRun)
...
bool CGameProcMain::MsgRecv_UserIn(Packet &pkt, bool bWithFX)
{
    ...
    float fYObject = ACT_WORLD->GetHeightNearstPosWithShape(
        __Vector3(fXPos, fYPos, fZPos), 1.0f); // 오브젝트에서 가장 가까운 높이값 얻기..
    if (fYObject > fYTerrain)
        fYPos = fYObject;
    else
        fYPos = fYTerrain;
    ...
    uint32_t dwItemIDs[MAX_ITEM_SLOT_OPC]; // 착용 아이템 - 다른 플레이어(NPC 포함) 0 ~ 4 상체,하체,헬멧,팔,발 5 망토 6
                                           // 오른손 7 왼손
    ...
    {
        CLogWriter::Write(
            "User In - Duplicated ID ({}, {}) Pos({:.2f},{:.2f},{:.2f})", iID, szName, fXPos, fYPos, fZPos);
...
bool CGameProcMain::MsgRecv_NPCIn(Packet &pkt)
{
    ...
    uint32_t dwStatus =
        pkt.read<uint32_t>();              // 상태... 여러가지로 or 연산해서 쓴다. 0 문 열림, 1 닫힘. 2, 4, 8, 16 ....
    ...
    {
        CLogWriter::Write(
            "NPC In - Duplicated ID ({}, {}) Pos({:.2f},{:.2f},{:.2f})", iID, szName, fXPos, fYPos, fZPos);
    ...
    if (pLooks)
    {
        ...
        std::string szPartFN;
        e_PartPosition eParts[6] = {
            PART_POS_UPPER, PART_POS_LOWER, PART_POS_FACE, PART_POS_HANDS, PART_POS_FEET, PART_POS_HAIR_HELMET};
        for (int i = 0; i < 6; i++)
        {
        ...
        if (iItemID0)
        {
            ...
            if (pItem0 && pItemExt0)
            {
                ...
                MakeResrcFileNameForUPC(
                    pItem0, pItemExt0, &szItemFN, nullptr, ePart, ePlug, s_pPlayer->m_InfoBase.eRace);
        ...
        if (iItemID1)
        {
            ...
            if (pItem1 && pItemExt1)
            {
                ...
                std::string szItemFN;
                MakeResrcFileNameForUPC(
                    pItem1, pItemExt1, &szItemFN, nullptr, ePart, ePlug, s_pPlayer->m_InfoBase.eRace);
...
bool CGameProcMain::MsgRecv_UserLookChange(Packet &pkt)
{
    ...
    if (ePartPos != PART_POS_UNKNOWN)
    {
        if (dwItemID) // 아이템이 있는 경우
        {
            std::string szItemFN;
            MakeResrcFileNameForUPC(
                pItem, pItemExt, &szItemFN, nullptr, ePartPos2, ePlugPos2, s_pPlayer->m_InfoBase.eRace);
        ...
        else
        {
            ...
            {
                CLogWriter::Write(
                    "CGameProcMain::MsgRecv_UserLookChange() - failed find table : Race ({})",
    ...
    {
        CLogWriter::Write(
            "CGameProcMain::MsgRecv_UserLookChange() - Unknown Slot({}) Item({})", static_cast<int>(eSlot), dwItemID);
        return false;
    }
...
void CGameProcMain::MsgRecv_MyInfo_RealmPoint(Packet &pkt)
{
    ...
    if (opcode == 1) // TODO: @Demircivi: LOYALTY_NATIONAL_POINTS is: 1, define it in header.
    {
        ...
        if (m_pUIVar->m_pPageState)
            m_pUIVar->m_pPageState->UpdateRealmPoint(
                s_pPlayer->m_InfoExt.iRealmPoint,
    ...
    else if (opcode == 2)
    {
        ...
        CLogWriter::Write(
            "Got manner update packet but didn't update UI since there is no manner feature. New Manner: {}.",
            iNewManner);
    }
    else
    {
...
void CGameProcMain::MsgRecv_MyInfo_PointChange(Packet &pkt)
{
    ...
    else if (0x04 == iType) // Intelligence
    {
        ...
        m_pUIVar->m_pPageState->UpdateIntelligence(
            s_pPlayer->m_InfoExt.iIntelligence, s_pPlayer->m_InfoExt.iIntelligence_Delta);
    }
    else if (0x05 == iType) // Magic Attack
    {
        s_pPlayer->m_InfoExt.iMagicAttak = iVal;
        m_pUIVar->m_pPageState->UpdateMagicAttak(
            s_pPlayer->m_InfoExt.iMagicAttak, s_pPlayer->m_InfoExt.iMagicAttak_Delta);
    }
...
void CGameProcMain::InitZone(int iZone, const __Vector3 &vPosPlayer)
{
    ...
    if (iZonePrev != iZone) // 다른 존일 경우에만 로딩..
    {
        ...
        CLogWriter::Write(
            "CGameProcMain::InitZone -> Zone Change ({} -> {}) Position({:.1f}, {:.1f}, {:.1f})",
            iZonePrev,
            iZone,
            vPosPlayer.x,
            vPosPlayer.y,
    ...
    CLogWriter::Write(
        "InitPlayerPosition() Position({:.1f}, {:.1f}, {:.1f})",
        vPosPlayer.x,
        vPosPlayer.y,
...
void CGameProcMain::CommandMove(e_MoveDirection eMD, bool bStartOrEnd)
{
    ...
    if (MD_FORWARD == eMD || MD_BACKWARD == eMD)
    {
        ...
        {
            float fSpeed = s_pPlayer->MoveSpeedCalculationAndCheckCollision(); // 속도를 구하고 그 속도로 충돌 체크를
                                                                               // 한다. 리턴값이 0 이면 충돌이다..
...
void CGameProcMain::MsgRecv_Notice(Packet &pkt)
{
    ...
    if (m_pUINotice && iNoticeCount > 0)
    {
        ...
        int x = CN3Base::s_CameraData.vp.Width -
                (rc.right - rc.left); //(CN3Base::s_CameraData.vp.Width/2) - (rc.right - rc.left)/2;
...
void CGameProcMain::MsgRecv_PartyOrForce(Packet &pkt)
{
    ...
    case N3_SP_PARTY_OR_FORCE_INSERT: // 0x03	// Send - s1(ID) | Recv - s3(ID, HPMax, HP, MPMax, MP) b2(Level, Class)
                                      // - 문자열은 ID 로 알아낸다..
    {
        ...
        if (iIDorErrorCode >= 0)
        {
            ...
            m_pUIPartyOrForce->MemberAdd(
                iIDorErrorCode, szID, iLevel, eClass, iHP, iHPMax, iMP, iMPMax); // 다른넘 파티에추가..
    ...
    case N3_SP_PARTY_OR_FORCE_REMOVE: // 0x04	// Send - s1(ID) | Recv - s1(ID) -
    {
        ...
        else
        {
            ...
            if (pInfo)
            {
                ...
                this->MsgOutput(
                    pInfo->szID + szMsg, D3DCOLOR_ARGB(255, 255, 255, 255)); // 누가 파티에서 떠났다는 메시지..
    ...
    case N3_SP_PARTY_OR_FORCE_HP_CHANGE: // 0x06	// Recv - s3(ID, HPMax, HP, iMPMax, MP) - 자기 자신이면 파티를 깨야
                                         // 한다..
    {
        int iID = pkt.read<int16_t>();
        int iHPMax = pkt.read<int16_t>();
...
void CGameProcMain::MsgRecv_ObjectEvent(Packet &pkt)
{
    ...
    if (iType == OBJECT_TYPE_BINDPOINT)
    {
        ...
        iType == OBJECT_TYPE_DOOR_LEFTRIGHT || iType == OBJECT_TYPE_DOOR_TOPDOWN ||
        iType == OBJECT_TYPE_LEVER_TOPDOWN || iType == OBJECT_TYPE_FLAG)
    {
...
void CGameProcMain::MsgRecv_Knights(Packet &pkt)
{
    ...
    /*	case N3_SP_KNIGHTS_APPOINT_CHIEF: //단장 임명 - 가입허가와 같음
        ...
        case N3_SP_KNIGHTS_APPOINT_OFFICER: // 장교임명 - 가입허가와 같음
            ...
            break;
        case N3_SP_KNIGHTS_LIST: //모든 리스트 요청 Send - | s1(Knights Count) Loop { s1(Knights ID) s1(Name Length)
       str1 (Name) }
            {
            ...
            break;
        case N3_SP_KNIGHTS_MEMBER_INFO_ALL: // 전체 멤버 Send - s1(page) | s1(Member Count) Loop { s1(Name Length) str1
       (Name) }
            {
                m_pUIVar->m_pPageKnights->MsgRecv_MemberInfo(pkt);
            }
            break;
        case N3_SP_KNIGHTS_MEMBER_INFO_ONLINE: //현재 접속 리스트 Send - s1(page) | s1(Member Count) Loop { s1(Name
       Length) str1 (Name) }
            {
                m_pUIVar->m_pPageKnights->MsgRecv_MemberInfo(pkt);
            }
    ...
    if (s_pPlayer->IsDead())
    {
        ...
        if (s_pPlayer->Nation() == NATION_KARUS)
            CGameProcedure::s_pFX->TriggerBundle(
                s_pPlayer->IDNumber(), -1, FXID_REGEN_KARUS, s_pPlayer->IDNumber(), -1);
        else if (s_pPlayer->Nation() == NATION_ELMORAD)
            CGameProcedure::s_pFX->TriggerBundle(
                s_pPlayer->IDNumber(), -1, FXID_REGEN_ELMORAD, s_pPlayer->IDNumber(), -1);
    }
...
bool CGameProcMain::OnMouseMove(POINT ptCur, POINT ptPrev)
{
    ...
    if(ptCur.x != ptPrev.x || ptCur.y != ptPrev.y)
    {
        if(m_pMagicSkillMng->m_dwRegionMagicState==1)
        {
            ...
            {
                s_pPlayer->m_pObjectTarget = ACT_WORLD->PickWithShape(ptCur.x, ptCur.y, true, &vPick); // 찍힌 위치를
저장한다.. if(nullptr == s_pPlayer->m_pObjectTarget) // 타겟도 없으면..
                {
...
bool CGameProcMain::OnMouseMove(POINT ptCur, POINT ptPrev)
{
    ...
    if (ptCur.x != ptPrev.x || ptCur.y != ptPrev.y)
    {
        ...
        if (m_pMagicSkillMng->m_dwRegionMagicState == 1)
        {
            ...
            s_pFX->SetBundlePos(
                m_pMagicSkillMng->m_iMyRegionTargetFXID, m_pMagicSkillMng->m_iMyRegionTargetFXID, m_vMouseSkillPos);
        }
    }
...
bool CGameProcMain::OnMouseLBtnPress(POINT ptCur, POINT ptPrev)
{
    ...
    _POINT ptPlayer = ::_Convert3D_To_2DCoordinate(
        s_pPlayer->Position(),
            ...
            if (pShape && pShape == s_pPlayer->m_pObjectTarget && pShape->m_iEventID) // Event 가 있으면..
            {
                ...
                else
                {
                    if (OBJECT_TYPE_BINDPOINT == pShape->m_iEventType)
                    {
                        ...
                        this->MessageBoxPost(
                            szMsg, "", MB_YESNO, BEHAVIOR_REQUEST_BINDPOINT); // 바인팅 포인트 설정 메시지 박스
                    }
