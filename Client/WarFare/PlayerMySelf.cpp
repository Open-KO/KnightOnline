...
void CPlayerMySelf::ToggleMoveMode()
{
    ...
    if (m_bMoveContinous) // 계속 움직여야 하면..
    {
        ...
        if (0 == fSpeed)                                              // 갈수 없으면...
        {
            ...
            m_eState = PSA_BASIC;
            nAni = this->JudgeAnimationBreath(); // 타겟이 있으면. 들고 있는 무기에 따라 기본 에니메이션 판다.. 없으면
                                                 // 걍 기본 에니메이션
        }
    }
    else
    {
        m_eStateMove = PSM_STOP;
        m_eState = PSA_BASIC;
        nAni = this->JudgeAnimationBreath(); // 타겟이 있으면. 들고 있는 무기에 따라 기본 에니메이션 판다.. 없으면 걍
                                             // 기본 에니메이션
    }
...
void CPlayerMySelf::RotAdd(const float fRotRadianPerSec) // y 축을 기준으로 초당 회전하는 속도를 넣어준다.
{
    ...
    if (m_fYawCur >= DegreesToRadians(180.0f) * 2)
        m_fYawCur -= DegreesToRadians(180.0f) * 2;
    if (m_fYawCur <= -DegreesToRadians(180.0f) * 2)
        m_fYawCur += DegreesToRadians(180.0f) * 2;
...
CN3CPart *CPlayerMySelf::PartSet(
...
float CPlayerMySelf::AttackableDistance(CPlayerBase *pTarget)
{
    ...
    else if (m_pItemPlugBasics[1] && ITEM_POS_TWOHANDLEFT == m_pItemPlugBasics[1]->byAttachPoint) // 왼손에
                                                                                                  // 투핸드레프트(활)
                                                                                                  // 아이템을 들고
                                                                                                  // 있으면..
...
// TODO(Gilad): It'd probably be better to refactor this one as it modifies the m_fYNext variable which is part of the
// PlayerBase class and when the packet is sent to the game server, it reads the next Y coordinates to climb the terrain
// height. Why? Because we just checking if collision detected and it should just check if collision detected and not
// modify any global members or pointers. It would be more partical to check the whole thing if authority = GM when more
// code is implemented here in the future. Another option would be to send a packet to the server validate the collision
// through the game server and let the game server check against wall hacking.
bool CPlayerMySelf::CheckCollision()
{
    float fSpeed =
        m_fMoveSpeedPerSec * CN3Base::s_fSecPerFrm; // The speed at which the current moving speed is calculated.
    if (0 == fSpeed)
        return false; // If it does not move, do not check collision.
    ...
    if (!s_pWorldMgr->IsIndoor())
        ...
        vPos2.y += 0.6f; // 캐릭터 발높이에서 0.6 미터 높이 위에서 충돌체크한다.	이 함수 내에서 쓰는 0.6은 PvsMgr의
                         // m_fVolumeOffs.. ^^
    bool bColShape = ACT_WORLD->CheckCollisionWithShape(vPos2, vDir, fSpeed, &vCol, &vNormal);
    if (s_pPlayer->m_InfoBase.iAuthority != AUTHORITY_MANAGER && bColShape)
...
float CPlayerMySelf::MoveSpeedCalculationAndCheckCollision() // 속도를 구하고 그 속도로 충돌 체크를 한다. 리턴값이 0
                                                             // 이면 충돌이다..
{
...
bool CPlayerMySelf::HasWeaponEquipped() const
{
    ...
    if (IsItemClassWeapon(ItemClass_RightHand()) || IsItemClassWeapon(ItemClass_LeftHand()))
        return true;
