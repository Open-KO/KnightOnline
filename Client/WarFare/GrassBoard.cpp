...
void CGrassBoard::Tick(CN3Terrain *pTerrain)
{
    ...
    for (int i = 0; i < m_ucTexNum; ++i)
    {
        ...
        //	카메라 방향으로 방향을 튼다
        if (vDir.x > 0.0f)
            pGrass->mtxWorld.RotationY(-atanf(vDir.z / vDir.x) - (__PI * 0.5f));
        else
            pGrass->mtxWorld.RotationY(-atanf(vDir.z / vDir.x) + (__PI * 0.5f));
...
void CGrassBoard::Render(CN3Texture **ppTex)
{
    ...
    {
        pGrass = &m_sGrassInfo[i];
        if (ppTex[pGrass->iTexIndex] == nullptr)
            continue;
...
void CGrassBoard::SetInfo(__Vector3 vBoardPosion, uint16_t usData)
{
    ...
    for (int i = 0; i < m_ucTexNum; ++i)
    {
        ...
        mRand = rand();
        pGrass->vPos.x = (mRand % 40) / 10.0f + vBoardPosion.x;
        ...
        mRand = rand();
        pGrass->vPos.z = (mRand % 40) / 10.0f + vBoardPosion.z; //	랜더값를 좀더 벌리기 위해
        pGrass->vPos.y = 0.0f;
    }
}
