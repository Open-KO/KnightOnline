...
bool CUICmdList::CreateCategoryList()
{
    ...
    // The command index needs to be reworked to behave more like official, where it's handled within CUICmdList and
    // categorised, so that entire translation can be thrown away.
    ...
    for (const CommandCategory &commandCategory : commandCategories)
        AppendToCommandMap(
            commandCategory.eCategory,
            commandCategory.eBaseCmd,
            commandCategory.iFirstResourceID,
            commandCategory.iLastResourceID);
...
void CUICmdList::AppendToCommandMap(
    e_CmdListCategory eCategory, e_ChatCmd eBaseCmd, int iFirstResourceID, int iLastResourceID)
{
    for (int iResourceID = iFirstResourceID, iRealCmdIndex = eBaseCmd; iResourceID <= iLastResourceID;
