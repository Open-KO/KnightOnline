...
// enum e_ChatBuffer { CHAT_BUFFER_NORMAL = 0, CHAT_BUFFER_PRIVATE, CHAT_BUFFER_PARTY, CHAT_BUFFER_KNIGHTS,
// CHAT_BUFFER_COUNT };
...
class CUIChat : public CN3UIBase
{
  ...
  protected:
    ...
    //	void			AddLineBuffer(e_ChatBuffer eCB, const std::string& szString, D3DCOLOR color);	// line 버퍼를
    // 만들어준다.(너무 길면 알아서 2줄로 만들어준다.)
    void AddLineBuffer(
        const std::string &szString, D3DCOLOR color); // line 버퍼를 만들어준다.(너무 길면 알아서 2줄로 만들어준다.)
  ...
  public:
    ...
    void SetRegion(const RECT &Rect)
        override; // 영역 지정(사이즈가 변할때 호출된다. 단순 이동은 호출되지 않는다.(단순이동은 MoveOffset이 호출))
