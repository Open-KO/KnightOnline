...
void CGrassMng::Tick(CGameProcedure *pProc)
{
    ...
    /*
        ...
        for(int i = 0; i < iSize; i++, it++)
        {
            ...
            uint8_t cGroundInfo = s_pTerrain->GetGrassAttr(((uint16_t)m_pCount[i].x)/TILE_SIZE,
       ((uint16_t)m_pCount[i].z)/TILE_SIZE);	//	현재 타일의 정보 if(cGroundInfo==0)	//	풀을 뿌리는 셀(영역,지역)
       아님
            {
...
{
    return (
        fPoX > fRange[GRASS_LEFT] && fPoX < fRange[GRASS_RIGHT] && fPoY > fRange[GRASS_TOP] &&
...
void CGrassMng::FineNewTile(uint16_t *Tile, int &iCount, float *fCompRange, float *fLargeRange)
{
    ...
    if (fCompRange[GRASS_LEFT] != m_fChkRange[GRASS_LEFT])
    {
        ...
        for (int i = 0; i < GRASS_TILENUM; ++i)
        {
            Tile[iCount] = (uint16_t)sx, Tile[iCount + 1] = (uint16_t)(sy += TILE_SIZE);
            iCount += 2;
        }
    }
    ...
    if (fCompRange[GRASS_TOP] != m_fChkRange[GRASS_TOP])
    {
        ...
        for (int i = si; i < ei; ++i)
        {
            Tile[iCount] = (uint16_t)(sx += TILE_SIZE), Tile[iCount + 1] = (uint16_t)sy;
            iCount += 2;
        }
    }
}
