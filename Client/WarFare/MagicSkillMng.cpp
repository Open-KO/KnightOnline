...
void CMagicSkillMng::Init()
{
    ...
    if( pInfoBase->eClass==CLASS_KA_ROGUE || pInfoBase->eClass==CLASS_KA_HUNTER ||
    pInfoBase->eClass==CLASS_KA_PENETRATOR )
    {
        m_iMyRegionTargetFXID = FXID_REGION_TARGET_KA_ROGUE;
    }
    else if( pInfoBase->eClass==CLASS_KA_WIZARD || pInfoBase->eClass==CLASS_KA_SORCERER ||
    pInfoBase->eClass==CLASS_KA_NECROMANCER )
    {
        m_iMyRegionTargetFXID = FXID_REGION_TARGET_KA_WIZARD;
    }
    else if( pInfoBase->eClass==CLASS_KA_PRIEST || pInfoBase->eClass==CLASS_KA_SHAMAN ||
    pInfoBase->eClass==CLASS_KA_DARKPRIEST )
    {
        m_iMyRegionTargetFXID = FXID_REGION_TARGET_KA_PRIEST;
    }
    else if( pInfoBase->eClass==CLASS_EL_ROGUE || pInfoBase->eClass==CLASS_EL_RANGER ||
    pInfoBase->eClass==CLASS_EL_ASSASIN )
    {
        m_iMyRegionTargetFXID = FXID_REGION_TARGET_EL_ROGUE;
    }
    else if( pInfoBase->eClass==CLASS_EL_WIZARD || pInfoBase->eClass==CLASS_EL_MAGE ||
    pInfoBase->eClass==CLASS_EL_ENCHANTER )
    {
        m_iMyRegionTargetFXID = FXID_REGION_TARGET_EL_WIZARD;
    }
    else if( pInfoBase->eClass==CLASS_EL_PRIEST || pInfoBase->eClass==CLASS_EL_CLERIC ||
    pInfoBase->eClass==CLASS_EL_DRUID )
    {
...
{
    if (s_pPlayer->State() == PSA_SPELLMAGIC || s_pPlayer->m_dwMagicID != 0xffffffff || s_pPlayer->m_bStun == true ||
        m_fDelay > 0.0f)
        return true;
    return false;
}
...
bool CMagicSkillMng::HasEquippedRequiredItemGroup(const __TABLE_UPC_SKILL *pSkill) const
{
    ...
    if (pSkill->dwNeedItem != 9 && pSkill->dwNeedItem != 0 &&
...
bool CMagicSkillMng::MsgSend_MagicProcess(int iTargetID, __TABLE_UPC_SKILL *pSkill)
{
    ...
    CGameProcedure::s_pFX->Stop(
        s_pPlayer->IDNumber(), s_pPlayer->IDNumber(), m_iMyRegionTargetFXID, m_iMyRegionTargetFXID, true);
    ...
    case SKILLMAGIC_TARGET_AREA_ALL: {
        ...
        // m_pGameProcMain->m_vMouseLBClickedPos, m_iMyRegionTargetFXID);	//전격무기...
        CGameProcedure::s_pFX->TriggerBundle(
            s_pPlayer->IDNumber(),
            0,
            m_iMyRegionTargetFXID,
            m_pGameProcMain->m_vMouseSkillPos,
            m_iMyRegionTargetFXID); // 전격무기...
...
bool CMagicSkillMng::CheckValidDistance(
    const __TABLE_UPC_SKILL *pSkill, __Vector3 vTargetPos, float fTargetRadius) const
{
...
void CMagicSkillMng::SuccessCast(__TABLE_UPC_SKILL *pSkill, CPlayerBase *pTarget)
{
    ...
    else
    {
        ...
        if (pSkill->iFlyingFX != 0)
        {
            ...
            {
                int16_t Data[6] = {
                    (int16_t)m_vTargetPos.x, (int16_t)m_vTargetPos.y, (int16_t)m_vTargetPos.z, (int16_t)idx, 0, 0};
...
void CMagicSkillMng::MsgRecv_Flying(Packet &pkt)
{
    ...
    if (pTarget == nullptr)
    {
        if (pPlayer != nullptr)
        {
            ...
            CGameProcedure::s_pFX->TriggerBundle(
                iSourceID, spart1, pSkill->iFlyingFX, vTargetPos, Data[3], FX_BUNDLE_MOVE_DIR_FIXEDTARGET);
        }
    }
    else
    {
...
void CMagicSkillMng::MsgRecv_Fail(Packet &pkt)
{
    ...
    if (Data[3] == SKILLMAGIC_FAIL_KILLFLYING) // flying효과 죽이고..그자리에 타겟효과 해라..
    {
        ...
        {
            CGameProcedure::s_pFX->TriggerBundle(
                iSourceID, pSkill->iTargetPart, pSkill->iTargetFX, iTargetID, pSkill->iTargetPart);
        }
        else
        {
...
void CMagicSkillMng::MsgRecv_BuffType(Packet &pkt)
{
    ...
    case BUFFTYPE_ABILITY:
        ...
        m_pGameProcMain->m_pUIVar->m_pPageState->UpdateIntelligence(
            pInfoExt->iIntelligence, pInfoExt->iIntelligence_Delta);
    ...
    case BUFFTYPE_RESIST:
        ...
        m_pGameProcMain->m_pUIVar->m_pPageState->UpdateRegistLight(
            pInfoExt->iRegistLight, pInfoExt->iRegistLight_Delta);
        m_pGameProcMain->m_pUIVar->m_pPageState->UpdateRegistMagic(
            pInfoExt->iRegistMagic, pInfoExt->iRegistMagic_Delta);
        m_pGameProcMain->m_pUIVar->m_pPageState->UpdateRegistCurse(
            pInfoExt->iRegistCurse, pInfoExt->iRegistCurse_Delta);
        m_pGameProcMain->m_pUIVar->m_pPageState->UpdateRegistPoison(
            pInfoExt->iRegistPoison, pInfoExt->iRegistPoison_Delta);
...
void CMagicSkillMng::FlyingType2(__TABLE_UPC_SKILL *pSkill, int iSourceID, int iTargetID, int16_t *pData)
{
    ...
    //__TABLE_UPC_SKILL_TYPE_2* pType2 = m_pTbl_Type_2->Find(pSkill->dwID);
    // if(pType2 != nullptr)	CGameProcedure::s_pFX->Trigger(iSourceID, spart1, pSkill->iFlyingFX, iTargetID,
    // pSkill->iTargetPart, pData[3], pType2->iSuccessType);
}
        ...
        if (iSourceID != s_pPlayer->IDNumber()) // 내가 스킬을 쓸때..
        {
            ...
            if (pType1 != nullptr)
            {
                ...
                if (pPlayer != nullptr)
                {
                    // 검기 색을 바꾸어 준다..
                    //					__TABLE_UPC_SKILL* pSkill = s_pTbl_Skill.Find(dwMagicID);
                    //					D3DCOLOR crTrace = TraceColorGet(pSkill); // 스킬의 종류에 따라 검기의 색을
                    // 정한다.. 					pPlayer->PlugTraceColorRemake(crTrace); // 검기 색 적용..
...
void CMagicSkillMng::EffectingType4(uint32_t dwMagicID)
{
    ...
    if (pType4 != nullptr)
    {
        ...
        case BUFFTYPE_ABILITY:
            ...
            m_pGameProcMain->m_pUIVar->m_pPageState->UpdateIntelligence(
                pInfoExt->iIntelligence, pInfoExt->iIntelligence_Delta);
            m_pGameProcMain->m_pUIVar->m_pPageState->UpdateMagicAttak(
                pInfoExt->iMagicAttak, pInfoExt->iMagicAttak_Delta);
            break;
        case BUFFTYPE_RESIST:
            pInfoExt->iRegistFire_Delta -= m_iFireR;
            pInfoExt->iRegistCold_Delta -= m_iColdR;
            ...
            m_pGameProcMain->m_pUIVar->m_pPageState->UpdateRegistFire(
                pInfoExt->iRegistFire, pInfoExt->iRegistFire_Delta);
            m_pGameProcMain->m_pUIVar->m_pPageState->UpdateRegistCold(
                pInfoExt->iRegistCold, pInfoExt->iRegistCold_Delta);
            m_pGameProcMain->m_pUIVar->m_pPageState->UpdateRegistLight(
                pInfoExt->iRegistLight, pInfoExt->iRegistLight_Delta);
            m_pGameProcMain->m_pUIVar->m_pPageState->UpdateRegistMagic(
                pInfoExt->iRegistMagic, pInfoExt->iRegistMagic_Delta);
