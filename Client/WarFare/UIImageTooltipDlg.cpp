...
CUIImageTooltipDlg::CUIImageTooltipDlg()
    : m_CYellow(D3DCOLOR_XRGB(255, 255, 0)), m_CBlue(D3DCOLOR_XRGB(128, 128, 255)),
      m_CGold(D3DCOLOR_XRGB(220, 199, 124)), m_CIvory(D3DCOLOR_XRGB(200, 124, 199)),
      m_CGreen(D3DCOLOR_XRGB(128, 255, 0)), m_CWhite(D3DCOLOR_XRGB(255, 255, 255)), m_CRed(D3DCOLOR_XRGB(255, 60, 60))
{
    m_iPosXBack = 0;
    m_iPosYBack = 0;
    m_pImg = nullptr;
...
#define ERROR_EXCEPTION                                                                                                \
    ...
    {
        m_pstdstr[iIndex] =
            fmt::format_text_resource(IDS_TOOLTIP_DAMAGE, spItem->pItemBasic->siDamage + spItem->pItemExt->siDamage);
    ...
    if (spItem->pItemBasic->siAttackInterval * (float)((float)spItem->pItemExt->siAttackIntervalPercentage / 100.0f) !=
        0)
    {
    ...
    {
        m_pstdstr[iIndex] =
            fmt::format_text_resource(IDS_TOOLTIP_ATTACKRANGE, (float)spItem->pItemBasic->siAttackRange / 10.0f);
    ...
    {
        m_pstdstr[iIndex] =
            fmt::format_text_resource(IDS_TOOLTIP_DEFENSE, spItem->pItemBasic->siDefense + spItem->pItemExt->siDefense);
    ...
    {
        m_pstdstr[iIndex] =
            fmt::format_text_resource(IDS_TOOLTIP_DEFENSE_RATE_DAGGER, spItem->pItemExt->siDefenseRateDagger);
    ...
    {
        m_pstdstr[iIndex] =
            fmt::format_text_resource(IDS_TOOLTIP_DEFENSE_RATE_SWORD, spItem->pItemExt->siDefenseRateSword);
    ...
    {
        m_pstdstr[iIndex] =
            fmt::format_text_resource(IDS_TOOLTIP_DEFENSE_RATE_BLOW, spItem->pItemExt->siDefenseRateBlow);
    ...
    {
        m_pstdstr[iIndex] =
            fmt::format_text_resource(IDS_TOOLTIP_DEFENSE_RATE_SPEAR, spItem->pItemExt->siDefenseRateSpear);
    ...
    {
        m_pstdstr[iIndex] =
            fmt::format_text_resource(IDS_TOOLTIP_DEFENSE_RATE_ARROW, spItem->pItemExt->siDefenseRateArrow);
    ...
    {
        m_pstdstr[iIndex] =
            fmt::format_text_resource(IDS_TOOLTIP_BONUSMAGICATTACK, spItem->pItemExt->siBonusMagicAttak);
    ...
    if (/*(spItem->pItemBasic->byAttachPoint == ITEM_LIMITED_EXHAUST) &&*/ spItem->pItemBasic->cNeedLevel +
            spItem->pItemExt->siNeedLevel >
        1)
    {
        m_pstdstr[iIndex] = fmt::format_text_resource(
            IDS_TOOLTIP_NEEDLEVEL, spItem->pItemBasic->cNeedLevel + spItem->pItemExt->siNeedLevel);

        if (SetTooltipTextColor(
                CGameBase::s_pPlayer->m_InfoBase.iLevel,
    ...
    {
        m_pstdstr[iIndex] = fmt::format_text_resource(
            IDS_TOOLTIP_NEEDRANK, spItem->pItemBasic->byNeedRank + spItem->pItemExt->siNeedRank);
    ...
    if (iNeedValue > 0)
    {
        ...
        if (SetTooltipTextColor(
                pInfoExt->iStrength, spItem->pItemBasic->byNeedStrength + spItem->pItemExt->siNeedStrength))
            m_pStr[iIndex]->SetColor(m_CWhite);
        else
            m_pStr[iIndex]->SetColor(m_CRed);
    ...
    if (iNeedValue > 0)
    {
        ...
        if (SetTooltipTextColor(
                pInfoExt->iStamina, spItem->pItemBasic->byNeedStamina + spItem->pItemExt->siNeedStamina))
            m_pStr[iIndex]->SetColor(m_CWhite);
        else
            m_pStr[iIndex]->SetColor(m_CRed);
    ...
    if (iNeedValue > 0)
    {
        ...
        if (SetTooltipTextColor(
                pInfoExt->iDexterity, spItem->pItemBasic->byNeedDexterity + spItem->pItemExt->siNeedDexterity))
            m_pStr[iIndex]->SetColor(m_CWhite);
        else
            m_pStr[iIndex]->SetColor(m_CRed);
    ...
    if (iNeedValue > 0)
    {
        ...
        if (SetTooltipTextColor(
                pInfoExt->iIntelligence, spItem->pItemBasic->byNeedInteli + spItem->pItemExt->siNeedInteli))
            m_pStr[iIndex]->SetColor(m_CWhite);
        else
            m_pStr[iIndex]->SetColor(m_CRed);
    ...
    if (iNeedValue > 0)
    {
        ...
        if (SetTooltipTextColor(
                pInfoExt->iMagicAttak, spItem->pItemBasic->byNeedMagicAttack + spItem->pItemExt->siNeedMagicAttack))
            m_pStr[iIndex]->SetColor(m_CWhite);
        else
            m_pStr[iIndex]->SetColor(m_CRed);
    ...
    if (spItem->pItemBasic != nullptr)
    {
        ...
        if (!szRemark.empty())
        {
            ...
            if (totalWords >= MIN_WORDS_TO_SPLIT_DESC)
            {
                ...
                // NOTE: This doesn't perfectly match official's behaviour; they rebuild the lines, always appending a
                // space. This means that there's a guaranteed space after the word here, before the '*' To replicate
                // this behaviour, we can manually add a space in the format string, but it's not really necessary.
                m_pStr[iIndex]->SetColor(m_CWhite);
                m_pstdstr[iIndex] =
                    fmt::format("{}*", std::string_view(szRemark.data() + splitPos, szRemark.size() - splitPos));
                m_pStr[iIndex]->SetStyle(UI_STR_TYPE_HALIGN, UISTYLE_STRING_ALIGNCENTER);
...
exceptions:;
    ...
    if (bPrice)
    {
        ...
        {
            m_pstdstr[iIndex] =
                fmt::format_text_resource(IDS_TOOLTIP_BUY_PRICE, CGameBase::FormatNumber(spItem->GetBuyPrice()));
        ...
        {
            m_pstdstr[iIndex] =
                fmt::format_text_resource(IDS_TOOLTIP_SELL_PRICE, CGameBase::FormatNumber(spItem->GetSellPrice()));
...
{
    if (m_dwID_Basic == spItem->pItemBasic->dwID && m_dwID_Ext == spItem->pItemExt->dwID &&
        m_iDurability == spItem->iDurability && m_iCount == spItem->iCount)
        return false;
