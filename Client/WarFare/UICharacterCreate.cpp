...
bool CUICharacterCreate::Load(File &file)
{
    ...
    std::string szBtns[MAX_CLASS_SELECT] = {
        "btn_class_warrior", "btn_class_rogue", "btn_class_mage", "btn_class_priest"};
...
{
    __InfoPlayerMySelf *pInfoExt = &(CGameBase::s_pPlayer->m_InfoExt);
    int iStats[MAX_STATS] = {
        pInfoExt->iStrength, pInfoExt->iStamina, pInfoExt->iDexterity, pInfoExt->iIntelligence, pInfoExt->iMagicAttak};
...
void CUICharacterCreate::UpdateRaceAndClassButtons(e_Race eRace) // 종족에 따라 직업 버튼 다시 설정..
{
    eUI_STATE eUIStateRaces[4] = {
        UI_STATE_BUTTON_NORMAL, // 종족 0
        UI_STATE_BUTTON_NORMAL,
        UI_STATE_BUTTON_NORMAL,
        UI_STATE_BUTTON_NORMAL};

    eUI_STATE eUIStateClasses[4] = {
        UI_STATE_BUTTON_DISABLE, // 직업 0
