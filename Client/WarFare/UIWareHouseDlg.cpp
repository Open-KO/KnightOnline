...
void CUIWareHouseDlg::Render()
{
    ...
    for (UIListReverseItor itor = m_Children.rbegin(); m_Children.rend() != itor; ++itor)
    {
        ...
        if (GetState() == UI_STATE_ICON_MOVING && pChild->UIType() == UI_TYPE_ICON &&
            s_sSelectedIconInfo.pItemSelect != nullptr && pChild == s_sSelectedIconInfo.pItemSelect->pUIIcon)
            continue;
    ...
    for (int i = 0; i < MAX_ITEM_TRADE; i++)
    {
        ...
        if (spItemWare != nullptr && (spItemWare->pItemBasic->byContable == UIITEM_TYPE_COUNTABLE ||
    ...
    for (int i = 0; i < MAX_ITEM_INVENTORY; i++)
    {
        ...
        if (spItemInv != nullptr && (spItemInv->pItemBasic->byContable == UIITEM_TYPE_COUNTABLE ||
                                     spItemInv->pItemBasic->byContable == UIITEM_TYPE_COUNTABLE_SMALL))
        {
            ...
            if (pStr != nullptr)
            {
                if ((GetState() == UI_STATE_ICON_MOVING) && (spItemInv == s_sSelectedIconInfo.pItemSelect))
                {
                    pStr->SetVisible(false);
                }
    ...
    case UIWND_DISTRICT_TRADE_MY:
        ...
        if (eUIWnd == UIWND_DISTRICT_TRADE_NPC)
        {
                    ...
                    for (int i = 0; i < MAX_ITEM_TRADE; i++)
                    {
                        if (m_pMyWare[iPage][i] != nullptr &&
                            m_pMyWare[iPage][i]->pItemBasic->dwID ==
                                s_sSelectedIconInfo.pItemSelect->pItemBasic->dwID &&
            ...
            else
            {
                ...
                m_pMyWare[s_sRecoveryJobInfo.m_iPage][iDestiOrder] =
...
void CUIWareHouseDlg::ReceiveResultFromWareMsg(uint8_t bResult)
{
    ...
    if (bResult != 0x01) // 실패..
    {
        ...
        else
        {
            ...
            if (pArea != nullptr)
            {
                m_pMyWare[m_iCurPage][s_sRecoveryJobInfo.UIWndSourceStart.iOrder]->pUIIcon->SetRegion(
                    pArea->GetRegion());
                m_pMyWare[m_iCurPage][s_sRecoveryJobInfo.UIWndSourceStart.iOrder]->pUIIcon->SetMoveRect(
                    pArea->GetRegion());
            }
