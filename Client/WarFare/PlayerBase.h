...
class CPlayerBase : public CGameBase
{
  ...
  protected:
    ...
    float m_fMoveSpeedPerSec; // 초당 움직임 값.. 이값은 기본값이고 상태(걷기, 달리기, 뒤로, 저주등) 에 따라 가감해서
                              // 쓴다..
  ...
  public:
    ...
    e_Ani JudgeAnimationBreath(); // 숨쉬기 모션 판단하기.. 가진 아이템과 타겟이 있는냐에 따라 다른 에니메이션 인덱스를
                                  // 리턴.
    ...
    e_Ani JudgeAnimationWalkBackward(); // 걷기 모드판단하기.. 가진 아이템과 타겟이 있는냐에 따라 다른 에니메이션
                                        // 인덱스를 리턴.
    ...
    float Distance(const __Vector3 &vPos)
    {
    ...
    void DurationColorSet(
        const _D3DCOLORVALUE &color, float fDurationTime); // 컬러를 정하는 시간대로 유지하면서 원래색대로 돌아간다.
