...
void CSubProcPerTrade::InitPerTradeDlg(CUIManager *pUIManager)
{
    ...
    CGameBase::MakeResrcFileNameForUPC(
        pItem, nullptr, nullptr, &szIconFN, ePart, ePlug); // 아이템에 따른 파일 이름을 만들어서
...
void CSubProcPerTrade::EnterWaitMsgFromServerStatePerTradeReq()
{
    ...
    // 메시지 박스 텍스트 표시..
    std::string szMsg =
        fmt::format_text_resource(IDS_PERSONAL_TRADE_FMT_WAIT, s_pPlayer->IDString(), pTarget->IDString());
...
void CSubProcPerTrade::EnterWaitMyDecisionToPerTrade(int iOtherID) // 내가 타인에게서 아이템 거래를 신청 받은 상태..
{
    ...
    // 메시지 박스 텍스트 표시..
    std::string szMsg =
        fmt::format_text_resource(IDS_PERSONAL_TRADE_PERMIT, s_pPlayer->IDString(), pTarget->IDString());
        ...
        if ((pItem->byContable == UIITEM_TYPE_COUNTABLE) || (pItem->byContable == UIITEM_TYPE_COUNTABLE_SMALL))
        {
            ...
            else
            {
                ...
                CGameBase::MakeResrcFileNameForUPC(
                    ...
                    ePart,
                    ePlug,
                    RACE_UNKNOWN); // 아이템에 따른 파일 이름을 만들어서
