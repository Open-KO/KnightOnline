...
void CUIDroppedItemDlg::AddToItemTable(int iItemID, int iItemCount, int iOrder)
{
    ...
    // 아이템에 따른 파일 이름을 만들어서
    e_ItemType eType =
        CGameBase::MakeResrcFileNameForUPC(pItem, pItemExt, nullptr, &szIconFN, ePart, ePlug, RACE_UNKNOWN);
...
#define FAIL_CODE                                                                                                      \
    ...
    case UIMSG_ICON_UP:
        ...
        if (pItem == nullptr)
        {
            ...
            CLogWriter::Write(
                "CUIDroppedItemDlg::ReceiveMessage - UIMSG_ICON_UP - NULL Icon : {}", spItem->pItemBasic->dwID);
            break;
        }
