...
void CN3TerrainManager::InitWorld(int iZoneID, const __Vector3 &vPosPlayer)
{
    ...
    /*if(iZoneID == 1) m_pTerrain->LoadFromFile(pZone->szTerrainFN,
    N3FORMAT_VER_1068);//N3FORMAT_VER_1298);//pZone->dwVersion); else*/
    ...
    m_pShapes->LoadFromFile(
        pZone->szObjectPostDataFN); //, N3FORMAT_VER_1298);//, pZone->dwVersion);	// 오브젝트 데이터 로드..
...
bool CN3TerrainManager::CheckCollisionWithShape(
