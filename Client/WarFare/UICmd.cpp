...
bool CUICmd::ReceiveMessage(CN3UIBase *pSender, uint32_t dwMsg)
{
    ...
    if (dwMsg == UIMSG_BUTTON_CLICK)
    {
        ...
        else if (pSender == m_pBtn_Party_Disband)
        {
            ...
            pMain->PartyOrForceConditionGet(
                bIAmLeader, bIAmMemberOfParty, iMemberIndex, pTarget); // 파티의 상황을 보고..
            ...
            if (bIAmLeader) // 내가 리더면..
            {
                if (iMemberIndex > 0)
                {
                    ...
                    szMsg = fmt::format_text_resource(IDS_PARTY_CONFIRM_DESTROY);
            }
            ...
            if (!szMsg.empty())
                CGameProcedure::MessageBoxPost(
                    szMsg, "", MB_YESNO, BEHAVIOR_PARTY_DISBAND); // 파티 해체,축출,탈퇴하기..확인
        }
