                                 ...
                                 (m_pMyTradeInv[i]->pItemBasic->byContable == UIITEM_TYPE_COUNTABLE_SMALL)))
        {
            ...
            if (pStr)
            {
                if ((GetState() == UI_STATE_ICON_MOVING) &&
                    (m_pMyTradeInv[i] == CN3UIWndBase::s_sSelectedIconInfo.pItemSelect))
                {
...
void CUITransactionDlg::InitIconWnd(e_UIWND eWnd)
{
    ...
    m_pUIMsgBoxOkCancel->SetPos(
        iX - (m_pUIMsgBoxOkCancel->GetWidth() / 2), iY - (m_pUIMsgBoxOkCancel->GetHeight() / 2) - 80);
    ...
    case UIWND_DISTRICT_TRADE_NPC: // 사는 경우..
        ...
        if (m_pMyTradeInv[CN3UIWndBase::s_sRecoveryJobInfo.UIWndSourceEnd.iOrder]) // 해당 위치에 아이콘이 있으면..
        {
            ...
            SendToServerBuyMsg(
                ...
                CN3UIWndBase::s_sRecoveryJobInfo.UIWndSourceEnd.iOrder,
                iGold);
        }
        else
        {
            ...
            pArea = CN3UIWndBase::GetChildAreaByiOrder(
                UI_AREA_TYPE_TRADE_MY, CN3UIWndBase::s_sRecoveryJobInfo.UIWndSourceEnd.iOrder);
            if (pArea)
            {
    ...
    case UIWND_DISTRICT_TRADE_NPC:
        if (eUIWnd == UIWND_DISTRICT_TRADE_MY) // 사는 경우..
        {
                ...
                for (i = 0; i < MAX_ITEM_INVENTORY; i++)
                {
                    ...
                    if ((m_pMyTradeInv[i]) &&
                        ...
                        (m_pMyTradeInv[i]->pItemExt->dwID ==
                         CN3UIWndBase::s_sSelectedIconInfo.pItemSelect->pItemExt->dwID))
                    {
                        bFound = true;
                        iDestiOrder = i;
                    }
            ...
            else
            {
                ...
                // 무게 체크..
                if ((pInfoExt->iWeight + CN3UIWndBase::s_sRecoveryJobInfo.pItemSource->pItemBasic->siWeight) >
                    pInfoExt->iWeightMax)
                {
                    std::string szMsg = fmt::format_text_resource(IDS_ITEM_WEIGHT_OVERFLOW);
                    CGameProcedure::s_pProcMain->MsgOutput(szMsg, 0xffff3b3b);
                ...
                CGameBase::MakeResrcFileNameForUPC(
                    spItemTrade->pItemBasic,
                    spItemTrade->pItemExt,
                    nullptr,
                    &szIconFN,
                    ePart,
                ...
                spItemNew->pItemBasic =
                    m_pMyTrade[m_iCurPage][CN3UIWndBase::s_sRecoveryJobInfo.UIWndSourceStart.iOrder]->pItemBasic;
                spItemNew->pItemExt =
                    m_pMyTrade[m_iCurPage][CN3UIWndBase::s_sRecoveryJobInfo.UIWndSourceStart.iOrder]->pItemExt;
    ...
    case UIWND_DISTRICT_TRADE_MY:
        ...
        else
        {
            ...
            if (m_pMyTradeInv[iDestiOrder]) // 해당 위치에 아이콘이 있으면..
            {
                ...
                pArea = nullptr;
                pArea = GetChildAreaByiOrder(
                    UI_AREA_TYPE_TRADE_MY, CN3UIWndBase::s_sRecoveryJobInfo.UIWndSourceStart.iOrder);
                if (pArea)
                {
                    spItemTarget->pUIIcon->SetRegion(pArea->GetRegion());
                    spItemTarget->pUIIcon->SetMoveRect(pArea->GetRegion());
...
void CUITransactionDlg::ReceiveResultTradeFromServer(byte bResult, byte bType, int iMoney)
{
    ...
    case UIWND_DISTRICT_TRADE_MY:
        ...
        else
        {
            ...
            if ((((CN3UIWndBase::s_sRecoveryJobInfo.pItemSource->pItemBasic->byContable == UIITEM_TYPE_COUNTABLE) ||
                  (CN3UIWndBase::s_sRecoveryJobInfo.pItemSource->pItemBasic->byContable ==
...
void CUITransactionDlg::IconRestore()
{
    ...
    case UIWND_DISTRICT_TRADE_NPC:
        ...
        {
            pArea = CN3UIWndBase::GetChildAreaByiOrder(
                UI_AREA_TYPE_TRADE_NPC, CN3UIWndBase::s_sSelectedIconInfo.UIWndSelect.iOrder);
            if (pArea)
            {
                m_pMyTrade[m_iCurPage][CN3UIWndBase::s_sSelectedIconInfo.UIWndSelect.iOrder]->pUIIcon->SetRegion(
                    pArea->GetRegion());
                m_pMyTrade[m_iCurPage][CN3UIWndBase::s_sSelectedIconInfo.UIWndSelect.iOrder]->pUIIcon->SetMoveRect(
                    pArea->GetRegion());
            }
        }
        break;
    ...
    case UIWND_DISTRICT_TRADE_MY:
        ...
        {
            pArea = CN3UIWndBase::GetChildAreaByiOrder(
                UI_AREA_TYPE_TRADE_MY, CN3UIWndBase::s_sSelectedIconInfo.UIWndSelect.iOrder);
            if (pArea)
            {
                m_pMyTradeInv[CN3UIWndBase::s_sSelectedIconInfo.UIWndSelect.iOrder]->pUIIcon->SetRegion(
                    pArea->GetRegion());
                m_pMyTradeInv[CN3UIWndBase::s_sSelectedIconInfo.UIWndSelect.iOrder]->pUIIcon->SetMoveRect(
                    pArea->GetRegion());
            }
        }
        break;
