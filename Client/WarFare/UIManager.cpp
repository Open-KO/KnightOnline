...
uint32_t CUIManager::MouseProc(uint32_t dwFlags, const POINT &ptCur, const POINT &ptOld)
{
    ...
    for (UIListItor itor = m_Children.begin(); m_Children.end() != itor;)
    {
        ...
        // 개인간 거래중이면 아이콘 매니저 윈도우만 작동.. 돈 액수나 화살 갯수등을 입력하는 중이면.. 입력 윈도우만
        // 작동..
...
void CUIManager::Render()
{
    ...
    */
    ...
    if (CGameProcedure::s_pProcMain != nullptr && CGameBase::ACT_WORLD != nullptr &&
        CGameBase::ACT_WORLD->GetSkyRef() != nullptr)
    {
        ...
        fmt::format_to(
            std::back_inserter(szDebugs[3]),
            "{:.2f}:FPS, {}/{}/{} : {}:{}",
            CN3Base::s_fFrmPerSec,
            iYear,
            iMonth,
            iDay,
            iH,
