...
void CUIEDoc::OnInsertString()
{
    ...
    {
        if (IDYES ==
            pFrm->MessageBox("Font 지정이 취소되었습니다.\n생성된 UIString을 삭제하시겠습니까?", nullptr, MB_YESNO))
            OnEditDelete();
        else
...
void CUIEDoc::OnInsertButton()
{
    ...
    {
        if (IDYES ==
            pFrm->MessageBox("텍스쳐 지정이 취소되었습니다.\n생성된 버튼을 삭제하시겠습니까?", nullptr, MB_YESNO))
            OnEditDelete();
        else
    ...
    pFrm->MessageBox(
        "1. 버튼의 위치와 영역을 지정해주세요.\n2. 네모 버튼이 아닐경우 click되는 영역을 지정해주세요.\n3. "
        "체크버튼으로 만들고 싶으면 style을 지정해주세요.");
    pFrm->GetRightPane()->SetMode(CUIEView::UIEMODE_EDIT);
    pFrm->GetRightPane()->SelectRectType(CUIEView::RT_REGION);
    UpdateAllViews(nullptr);
...
void CUIEDoc::OnInsertProgress()
{
    ...
    while (1)
    {
        ...
        {
            if (IDYES ==
                pFrm->MessageBox("텍스쳐 지정이 취소되었습니다.\nProgress를 삭제하시겠습니까?", nullptr, MB_YESNO))
                OnEditDelete();
            else
    ...
    if (IDCANCEL == dlg.DoModal())
    {
        if (IDYES ==
            pFrm->MessageBox(
                "텍스쳐 UV좌표 지정이 취소되었습니다.\n생성된 Progress를 삭제하시겠습니까?", nullptr, MB_YESNO))
            OnEditDelete();
        else
...
void CUIEDoc::OnInsertTrackbar()
{
    ...
    if (FALSE == SetTrackBarInfos(pUI))
    {
        if (IDYES ==
            pFrm->MessageBox(
                "Trackbar 정보 지정이 취소되었습니다.\n생성된 Trackbar를 삭제하시겠습니까?", nullptr, MB_YESNO))
            OnEditDelete();
        else
...
void CUIEDoc::OnInsertScrollbar()
{
    ...
    {
        if (IDYES ==
            pFrm->MessageBox("텍스쳐 지정이 취소되었습니다.\n생성된 스크롤을 삭제하시겠습니까?", nullptr, MB_YESNO))
            OnEditDelete();
        else
    ...
    if (FALSE == SetTrackBarInfos(pUITrackBar))
    {
        if (IDYES == pFrm->MessageBox(
                         "Trackbar 정보 지정이 취소되었습니다.\n생성된 ScrollBar 삭제하시겠습니까?", nullptr, MB_YESNO))
            OnEditDelete();
        else
    ...
    // style을 정하고
    pFrm->MessageBox(
        "1. 왼쪽창에서 스타일(가로/세로)을 지정해주세요.\n스크롤의 영역을 반드시 양 버튼과 가운데 트랙바가 들어가게 "
        "정해주세요");
...
//		case UI_TYPE_ICON_MANAGER:	pUIDest = new CN3UIIconManager();	*pUIDest = *((CN3UIBase*)pUISrc); break;	//
//icon manager..
#ifdef _REPENT
...
void CUIEDoc::OnEditMakeGroup()
{
    ...
    {
        MessageBox(
            AfxGetMainWnd()->m_hWnd, "두개이상의 UI 를 선택해야 그룹을 만들수 있습니다.", "그룹지정 실패", MB_OK);
        return;
    }
