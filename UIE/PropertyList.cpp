...
void CPropertyList::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
    ...
    if (nIndex != (UINT)-1)
    {
        ...
        {
            dc.FillSolidRect(
                CRect(rect.left + 2, rect.top + 2, rect.right - 4, rect.bottom - 4),
...
int CPropertyList::AddPropItem(
    const char *szPropName, const char *szVal, int nItemType, const char *szCmbItems, COLORREF crColor)
{
...
void CPropertyList::OnSelchange()
{
    ...
    if (pItem->m_nItemType == PIT_COMBO)
    {
        ...
        else
        {
            rect.bottom += 300;
            m_ComboBox.Create(
                CBS_DROPDOWNLIST | CBS_NOINTEGRALHEIGHT | WS_VISIBLE | WS_CHILD | WS_BORDER,
                rect,
                this,
                IDC_PROPCMBBOX);
...
void CPropertyList::OnSelchangeCmbBox()
{
    ...
    if (m_ComboBox)
    {
        ...
        CWnd *pWnd = GetParent();
        if (pWnd != nullptr)
            pWnd->SendMessage(WM_NOTIFY, (DWORD_PTR)this, (DWORD_PTR)pItem); // 부모 윈도우에 메시지 보내기..
    }
}
    ...
    else if (pItem->m_nItemType == PIT_FILE_MULTI)
    {
        ...
        if (IDOK == dlg.DoModal())
        {
            ...
            if (iSC > 0)
            {
                ...
                for (int i = 1; i < iSC - 1; i++) // 1 부터 시작하는 이유는 파일 대화상자에서 여러 파일을 부르면 첨과 끝
                                                  // 파일 이름이 바뀌기 때문이다.
                {
...
CPropertyItem::CPropertyItem(
    const char *szPropName, const char *szVal, int nItemType, const char *szComboItemsOrFilter, COLORREF crColor)
{
