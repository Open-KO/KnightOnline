    ...
    else if (UIEMODE_EDIT == m_eMode)
    {
        ...
        if (-1000 != m_rcSelectedRect.left && RT_NONE != m_eSelectedRectType)
        {
            strText.Format(
                ...
                strRect,
                m_rcSelectedRect.left,
                m_rcSelectedRect.top,
                m_rcSelectedRect.right,
                m_rcSelectedRect.bottom,
                m_rcSelectedRect.Width(),
                m_rcSelectedRect.Height());
        }
        else
        {
            CPoint pt;
    ...
    if (pSelectedUI && UIEMODE_EDIT == m_eMode && RT_NONE != m_eSelectedRectType && -1000 != m_rcSelectedRect.left)
    {
        ...
        case RT_REGION: {
            ...
            for (int i = 0; i < iUIC; i++)
            {
                ...
                else if (i == 0 && m_eDragType >= DRAGTYPE_LEFT && m_eDragType <= DRAGTYPE_RIGHTBOTTOM) // 마지막에
                                                                                                        // 선택한 UI
                {
