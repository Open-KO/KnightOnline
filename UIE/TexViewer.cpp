...
CTexViewer::CTexViewer() : m_WhiteDashPen(PS_DOT, 1, RGB(255, 255, 255))
{
    ...
    m_hCursorZoomIn = (HCURSOR)LoadImage(
        AfxGetResourceHandle(), MAKEINTRESOURCE(IDC_CURSOR_ZOOMIN), IMAGE_CURSOR, 0, 0, LR_DEFAULTSIZE);
    m_hCursorZoomOut = (HCURSOR)LoadImage(
        AfxGetResourceHandle(), MAKEINTRESOURCE(IDC_CURSOR_ZOOMOUT), IMAGE_CURSOR, 0, 0, LR_DEFAULTSIZE);
    m_hCursorHand = (HCURSOR)LoadImage(
        AfxGetResourceHandle(), MAKEINTRESOURCE(IDC_CURSOR_HAND), IMAGE_CURSOR, 0, 0, LR_DEFAULTSIZE);
...
void CTexViewer::Render()
{
    ...
    lpD3DDev->GetSamplerState(
        0, D3DSAMP_MAGFILTER, &dwMagFilter); // lpD3DDev->GetTextureStageState(0, D3DTSS_MAGFILTER,   &dwMagFilter);
    lpD3DDev->GetSamplerState(
        0, D3DSAMP_MINFILTER, &dwMinFilter); // lpD3DDev->GetTextureStageState(0, D3DTSS_MINFILTER,   &dwMinFilter);
    lpD3DDev->GetSamplerState(
        0, D3DSAMP_MIPFILTER, &dwMipFilter); // lpD3DDev->GetTextureStageState(0, D3DTSS_MIPFILTER,   &dwMipFilter);
    ...
    if (D3DTEXF_POINT != dwMagFilter)
        lpD3DDev->SetSamplerState(
            0, D3DSAMP_MAGFILTER, D3DTEXF_POINT); // lpD3DDev->SetTextureStageState(0, D3DTSS_MAGFILTER, D3DTEXF_POINT);
    if (D3DTEXF_POINT != dwMinFilter)
        lpD3DDev->SetSamplerState(
            0, D3DSAMP_MINFILTER, D3DTEXF_POINT); // lpD3DDev->SetTextureStageState(0, D3DTSS_MINFILTER, D3DTEXF_POINT);
    if (D3DTEXF_NONE != dwMipFilter)
        lpD3DDev->SetSamplerState(
            0, D3DSAMP_MIPFILTER, D3DTEXF_NONE); // lpD3DDev->SetTextureStageState(0, D3DTSS_MIPFILTER,   D3DTEXF_NONE);
    ...
    if (D3DTEXF_POINT != dwMagFilter)
        lpD3DDev->SetSamplerState(
            0, D3DSAMP_MAGFILTER, dwMagFilter); // lpD3DDev->SetTextureStageState(0, D3DTSS_MAGFILTER,   dwMagFilter);
    if (D3DTEXF_POINT != dwMinFilter)
        lpD3DDev->SetSamplerState(
            0, D3DSAMP_MINFILTER, dwMinFilter); // lpD3DDev->SetTextureStageState(0, D3DTSS_MINFILTER,   dwMinFilter);
    if (D3DTEXF_NONE != dwMipFilter)
        lpD3DDev->SetSamplerState(
            0, D3DSAMP_MIPFILTER, dwMipFilter); // lpD3DDev->SetTextureStageState(0, D3DTSS_MIPFILTER,   dwMipFilter);
}
    ...
    BOOL bHorizon, CString &strErrMsg) // 가로 또는 세로로 연결된 이미지 Rect들을 자동으로 같은 크기로 선택해준다.
{
    ...
    { // 수평으로 이동하며 나누기
        for (i = 0; i < m_iImageTypeCount; ++i)
        {
            ...
            m_ImageRects[i].SetRect(
                ptLeftTop.x + m_rcSelectedRect.Width() * iRow,
