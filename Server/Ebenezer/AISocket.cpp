...
CAISocket::CAISocket(SocketManager *socketManager, int zoneNum) : TcpClientSocket(socketManager)
{
    _zoneNum = zoneNum;
}
...
void CAISocket::LoginProcess(char *pBuf)
{
    ...
    else
    {
        ...
        else if (byReConnect == 1)
        {
            ...
            spdlog::info("AISocket::LoginProcess: reconnect zone={} socketCount={}", zone, _main->m_sReSocketCount);
            ...
            if (_main->m_sReSocketCount == MAX_AI_SOCKET)
            {
                ...
                {
                    spdlog::info(
                        "AISocket::LoginProcess: sockets reconnected in under a minute [sockets={}]",
                        _main->m_sReSocketCount);
                    _main->m_bServerCheckFlag = true;
                    _main->m_sReSocketCount = 0;
...
void CAISocket::RecvNpcInfoAll(char *pBuf)
{
    ...
    for (int i = 0; i < byCount; i++)
    {
        ...
        {
            spdlog::error(
                "AISocket::RecvNpcInfoAll: npc name size out of bounds [npcId={} npcName={}]", npcId, npcName);
        ...
        {
            spdlog::error(
                "AISocket::RecvNpcInfoAll: map not found for zoneIndex [serial={} npcId={} pictureId={} npcName={} "
                "zoneIndex={} x={} z={}]",
                instanceId,
                npcId,
                pictureId,
        ...
        {
            spdlog::error(
                "AISocket::RecvNpcInfoAll: invalid serial [serial={} npcId={} pictureId={} npcName={} zoneIndex={} "
                "x={} z={}]",
                instanceId,
                npcId,
                pictureId,
        ...
        {
            spdlog::error(
                "AISocket::RecvNpcInfoAll: invalid pictureId [serial={} npcId={} pictureId={} npcName={} zoneIndex={} "
                "x={} z={}]",
                instanceId,
                npcId,
                pictureId,
        ...
        {
            spdlog::error(
                "AISocket::RecvNpcInfoAll: region out of bounds [serial={} npcId={} npcName={} zoneId={} x={} z={}]",
                instanceId,
                npcId,
                npcName,
                pMap->m_nZoneNumber,
        ...
        // sPid, szName, fPosX, fPosZ, byGateOpen, byObjectType);

        if (!_main->m_NpcMap.PutData(pNpc->m_sNid, pNpc))
        {
            spdlog::error(
                "AISocket::RecvNpcInfoAll: NpcMap put failed [serial={} npcId={} npcName={} zoneId={} x={} z={}]",
                instanceId,
                npcId,
                npcName,
                pMap->m_nZoneNumber,
        ...
        {
            spdlog::error(
                "AISocket::RecvNpcInfoAll: invalid byType={} [serial={} npcId={} npcName={} zoneId={} x={} z={}]",
                byType,
                instanceId,
                npcId,
                npcName,
...
void CAISocket::RecvNpcAttack(char *pBuf)
{
    ...
    if (type == 0x01)
    {
        ...
        if (result == 0x04)
        {
            ...
            _main->Send_Region(
                pOutBuf, send_index, pNpc->m_sCurZone, pNpc->m_sRegion_X, pNpc->m_sRegion_Z, nullptr, false);
        }
        else
        {
    ...
    else if (type == 0x02)
    {
        ...
        if (tid >= USER_BAND && tid < NPC_BAND)
        {
            ...
            /*			if( sHP <= 0 && pUser->m_pUserData->m_sHp > 0 ) {
               ...
               result, sHP, pUser->m_pUserData->m_sHp); if(result == 0x02) pUser->HpChange(-1000, 1);
                        }
                        else
            ...
            //result, sHP, pUser->m_pUserData->m_sHp); TRACE(_T("RecvNpcAttack ==> sid = %d, tid = %d, result = %d\n"),
            // sid, tid, result);
            ...
            if (result == 0x02)
            {
                ...
                spdlog::debug(
                    "AISocket::RecvNpcAttack: user is dead [charId={} result={} AI_HP={} GM_HP={} x={} z={}]",
                    pUser->m_pUserData->m_id,
                    result,
                    nHP,
                    pUser->m_pUserData->m_sHp,
                ...
                if (pUser->m_pUserData->m_bFame == COMMAND_CAPTAIN)
                {
                    ...
                    _main->Send_Region(
                        pOutBuf, send_index, pUser->m_pUserData->m_bZone, pUser->m_RegionX, pUser->m_RegionZ);
                    // sungyong tw
                    pUser->Send(pOutBuf, send_index);
                    // ~sungyong tw
                    spdlog::debug(
                        "AISocket::RecvNpcAttack: Dead Captain Deprive [charId={}]", pUser->m_pUserData->m_id);
...
void CAISocket::RecvMagicAttackResult(char *pBuf)
{
    ...
    if (byCommand == MAGIC_CASTING)
    {
        ...
        _main->Send_Region(
            send_buff, send_index, pNpc->m_sCurZone, pNpc->m_sRegion_X, pNpc->m_sRegion_Z, nullptr, false);
    }
    // effecting
    else if (byCommand == MAGIC_EFFECTING)
    {
        if (sid >= USER_BAND && sid < NPC_BAND)
        {
            ...
            SetByte(send_buff, WIZ_MAGIC_PROCESS, index);
            _main->Send_Region(
                send_buff, send_index, pUser->m_pUserData->m_bZone, pUser->m_RegionX, pUser->m_RegionZ, nullptr, false);
        }
...
void CAISocket::RecvNpcInfo(char *pBuf)
{
    ...
    if (pNpc->m_NpcState == NPC_LIVE)
    {
        spdlog::get(logger::EbenezerRegion)
            ->info(
                "AISocket::RecvNpcInfo: npc regen check [state={} serial={} npcId={} npcName={} x={} z={} regionX={} "
                "regionZ={}]",
                pNpc->m_NpcState,
                instanceId,
                npcId,
                npcName,
                static_cast<int32_t>(pNpc->m_fCurX),
                static_cast<int32_t>(pNpc->m_fCurZ),
                pNpc->m_sRegion_X,
                pNpc->m_sRegion_Z);
    }
    ...
    if (pMap == nullptr)
    {
        pNpc->m_NpcState = NPC_DEAD;
        spdlog::error(
            "AISocket::RecvNpcInfo: map not found for zoneIndex [serial={} npcId={} pictureId={} npcName={} "
            "zoneIndex={} x={} z={}]",
            instanceId,
            npcId,
            pictureId,
    ...
    {
        spdlog::error(
            "AISocket::RecvNpcInfo: dead monster [serial={} npcId={} npcName={} zoneId={} x={} z={}]",
            instanceId,
            npcId,
            npcName,
            pMap->m_nZoneNumber,
...
void CAISocket::RecvUserExp(char *pBuf)
{
    ...
    {
        spdlog::error(
            "AISocket::RecvUserExp: invalid exp or loyalty amount granted [userId={} charId={} exp={} loyalty={}]",
            userId,
            pUser->m_pUserData->m_id,
            sExp,
            sLoyalty);
...
void CAISocket::RecvUserFail(char *pBuf)
{
    ...
    spdlog::trace(
        "AISocket::RecvUserFail: [npcId={} serial={} charId={}]", npcId, instanceId, pUser->m_pUserData->m_id);
...
void CAISocket::RecvCompressedData(char *pBuf)
{
    ...
    uint32_t nDecompressedLength = lzf_decompress(pCompressedBuffer, dwCompLen, &decompressedBuffer[0], dwOrgLen);
...
void CAISocket::RecvGateDestroy(char *pBuf)
{
    ...
    if (instanceId >= NPC_BAND)
    {
        ...
        spdlog::debug(
            "AISocket::RecvGateDestroy: [serial={} npcId={} npcName={} gateOpen={}]",
            pNpc->m_sNid,
            pNpc->m_sSid,
            pNpc->m_strName,
            pNpc->m_byGateOpen);
...
void CAISocket::RecvBattleEvent(char *pBuf)
{
    ...
    char strMaxUserName[MAX_ID_SIZE + 1] = {}, strKnightsName[MAX_ID_SIZE + 1] = {}, send_buff[1024] = {},
                                      udp_buff[1024] = {};
    ...
    else if (nType == BATTLE_MAP_EVENT_RESULT)
    {
        ...
        {
            spdlog::error(
                "AISocket::RecvBattleEvent: No active battle [battleOpen={} type={}]", _main->m_byBattleOpen, nType);
            return;
        }
    ...
    else if (nType == BATTLE_EVENT_RESULT)
    {
        ...
        if (nLen > 0 && nLen < MAX_ID_SIZE + 1)
        {
            ...
            if (_main->m_byBattleSave == 0)
            {
                ...
                {
                    spdlog::error(
                        "AISocket::RecvBattleEvent: WIZ_BATTLE_EVENT send fail [retValue={} type={}]", retvalue, nType);
                }
...
void CAISocket::RecvGateOpen(char *pBuf)
{
    ...
    if (pNpc == nullptr)
    {
        spdlog::error("AISocket::RecvGateOpen: Npc not found [serial={} npcId={}]", instanceId, npcId);
        return;
    }
    ...
    if (pEvent == nullptr)
    {
        spdlog::error("AISocket::RecvGateOpen: Npc ObjectEvent not found [serial={} npcId={}]", instanceId, npcId);
        return;
    }
