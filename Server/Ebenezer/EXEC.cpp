...
void EXEC::Parse(const char *line, const std::string &filename, int lineNumber)
{
    ...
    switch (opcode)
    {
    // E SAY {'up' event ID} {'ok' event ID} {talk ID 1} {talk ID 2} {talk ID 3} {talk ID 4} {talk ID 5} {talk ID 6}
    // {talk ID 7} {talk ID 8} The talk ID refers to the ID in the Quest_Talk TBL in the client.
    case "SAY"_djb2:
        m_Exec = EXEC_SAY;
        argsToParse = 10;
        break;
    ...
    // ... {menu ID 1} {event ID 1} {menu ID 2} {event ID 2} {menu ID 3} {event ID 3} {menu ID 4} {event ID 4} {menu ID
    // 5} {event ID 5}
    // ... {menu ID 6} {event ID 6} {menu ID 7} {event ID 7} {menu ID 8} {event ID 8} {menu ID 9} {event ID 9} {menu ID
    // 10} {event ID 10} The talk ID refers to the ID in the Quest_Talk TBL in the client. This is used for the dialogue
    // text. The menu ID refers to the ID in the Quest_Menu TBL in the client. This is the button text. The event ID
    // refers to the associated event ID to run on the server when this button is pressed in the client.
