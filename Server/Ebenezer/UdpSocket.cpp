...
{
    _socket.async_receive_from(asio::buffer(_recvBuff), _sender, [this](asio::error_code ec, size_t transferredBytes) {
        if (!ec)
        {
        ...
        {
            spdlog::error("UdpSocket::AsyncReceive: error [err={} ip={}]", ec.message(), _sender.address().to_string());
        }
...
int CUdpSocket::SendUDPPacket(char *strAddress, char *pBuf, int len)
{
    ...
    if (ec)
    {
        spdlog::error("UdpSocket::SendUDPPacket: invalid ip [err={} ip={}]", ec.message(), ip.to_string());
        return 0;
    }
    ...
    if (ec)
    {
        spdlog::error("UdpSocket::SendUDPPacket: send_to() failed [err={} ip={}]", ec.message(), ip.to_string());
        return 0;
    }
...
void CUdpSocket::RecvBattleEvent(char *pBuf)
{
    ...
    else if (nType == BATTLE_MAP_EVENT_RESULT)
    {
        ...
        {
            spdlog::error(
                "UdpSocket::RecvBattleEvent: No active battle [battleOpen={} type={}]", _main->m_byBattleOpen, nType);
            return;
        }
...
void CUdpSocket::RecvJoinKnights(char *pBuf, uint8_t command)
{
    ...
    if (command == KNIGHTS_JOIN)
    {
        ...
        spdlog::debug("UdpSocket::RecvJoinKnights: charId={} joined knightsId={}", charId, knightsId);
    }
    // 탈퇴..
    else
    {
