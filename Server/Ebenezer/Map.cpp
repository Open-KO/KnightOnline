...
void C3DMap::LoadObjectEvent(File &fs)
{
    ...
    for (int i = 0; i < iEventObjectCount; i++)
    {
        ...
        //		TRACE ("성래 요청 : %d , %d , %d , %d , %d \r\n", pEvent->sBelong, pEvent->sIndex, pEvent->sType,
        //pEvent->sControlNpcID, pEvent->sStatus);
    }
}
...
void C3DMap::LoadRegeneEvent(File &fs)
{
    ...
    for (int i = 0; i < iEventObjectCount; i++)
    {
        ...
        {
            spdlog::error(
                "Map::LoadObjectEvent: RegenPoint put failed [regenPoint={} zoneId={}]",
                pEvent->sRegenePoint,
                m_nZoneNumber);
            delete pEvent;
            pEvent = nullptr;
...
void C3DMap::LoadWarpList(File &fs)
{
    ...
    for (int i = 0; i < WarpCount; i++)
    {
        ...
        {
            spdlog::error(
                "Map::LoadObjectEvent: WarpArray put failed [warpId={} zoneId={}]", pWarp->sWarpID, m_nZoneNumber);
            delete pWarp;
            pWarp = nullptr;
        }
    }
...
bool C3DMap::CheckEvent(float x, float z, CUser *pUser)
{
    ...
    if (pEvent != nullptr)
    {
        ...
        if (pUser->m_pUserData->m_bNation == KARUS && pEvent->m_iExec[0] == ZONE_BATTLE)
        {
            ...
            {
                spdlog::error(
                    "Map::CheckEvent: BattleZone: karus full users [users={} charId={}]",
...
bool C3DMap::LoadEvent()
{
    ...
    loader.SetProcessFetchCallback([this](db::ModelRecordSet<ModelType> &recordset) {
        do
        {
            ...
            {
                spdlog::error(
                    "Map::LoadEvent: EventArray put failed [eventId={} zoneId={}]", pEvent->m_sIndex, m_nZoneNumber);
                delete pEvent;
            }
