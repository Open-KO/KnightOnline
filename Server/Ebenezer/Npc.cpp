...
void CNpc::NpcInOut(uint8_t Type, float fx, float fz, float fy)
{
    ...
    {
        spdlog::error(
            "Npc::NpcInOut: no map found for zoneIndex={} [serial={} npcId={} npcName={} x={} z={}]",
            m_sZoneIndex,
            m_sNid,
            m_sSid,
            m_strName,
...
void CNpc::RegisterRegion()
{
    ...
    if (m_sRegion_X != iRegX || m_sRegion_Z != iRegZ)
    {
        ...
        RemoveRegion(
            old_region_x - m_sRegion_X, old_region_z - m_sRegion_Z); // delete npc 는 계산 방향이 진행방향의 반대...
...
void CNpc::RemoveRegion(int del_x, int del_z)
{
    ...
    if (del_x != 0)
    {
        ...
        // TRACE(_T("Remove : (%d %d), (%d %d), (%d %d)\n"), m_sRegion_X+del_x*2, m_sRegion_Z+del_z-1,
        // m_sRegion_X+del_x*2, m_sRegion_Z+del_z, m_sRegion_X+del_x*2, m_sRegion_Z+del_z+1 );
    }
    ...
    if (del_z != 0)
    {
        ...
        else
        {
            ...
            // TRACE(_T("Remove : (%d %d), (%d %d), (%d %d)\n"), m_sRegion_X+del_x-1, m_sRegion_Z+del_z*2,
            // m_sRegion_X+del_x, m_sRegion_Z+del_z*2, m_sRegion_X+del_x+1, m_sRegion_Z+del_z*2 );
        }
    }
}
...
void CNpc::InsertRegion(int del_x, int del_z)
{
    ...
    if (del_x != 0)
    {
        ...
        // TRACE(_T("Insert : (%d %d), (%d %d), (%d %d)\n"), m_sRegion_X+del_x, m_sRegion_Z-1, m_sRegion_X+del_x,
        // m_sRegion_Z, m_sRegion_X+del_x, m_sRegion_Z+1 );
    }
    ...
    if (del_z != 0)
    {
        ...
        else
        {
            ...
            // TRACE(_T("Insert : (%d %d), (%d %d), (%d %d)\n"), m_sRegion_X-1, m_sRegion_Z+del_z, m_sRegion_X,
            // m_sRegion_Z+del_z, m_sRegion_X+1, m_sRegion_Z+del_z );
        }
    }
}
