      ...
      m_ItemLoggerSendQ(MAX_SMQ_SEND_QUEUE_RETRY_COUNT)
{
    ...
    _gameTimeThread = std::make_unique<TimerThread>(6s, std::bind(&EbenezerApp::GameTimeTick, this));

    _smqHeartbeatThread = std::make_unique<TimerThread>(10s, std::bind(&EbenezerApp::SendSMQHeartbeat, this));
    ...
    _marketBBSTimeThread = std::make_unique<TimerThread>(5min, std::bind(&EbenezerApp::MarketBBSTimeCheck, this));

    _packetCheckThread = std::make_unique<TimerThread>(6min, std::bind(&EbenezerApp::WritePacketLog, this));
...
bool EbenezerApp::AISocketConnect(int zone, bool flag)
{
    ...
    if (!pAISock->Create())
    {
        ...
        spdlog::error("EbenezerApp::AISocketConnect: Failed to create new AI socket for zone {}", zone);
    ...
    if (port < 0)
    {
        ...
        spdlog::error(
            "EbenezerApp::AISocketConnect: Invalid port, unsupported m_nServerNo {} (zone {})", m_nServerNo, zone);
    ...
    if (!pAISock->Connect(m_AIServerIP, port))
    {
        ...
        spdlog::error(
            "EbenezerApp::AISocketConnect: Failed to connect to AI server (zone {}) ({}:{})", zone, m_AIServerIP, port);
...
void EbenezerApp::Send_NearRegion(
    char *pBuf, int len, int zone, int region_x, int region_z, float curx, float curz, CUser *pExceptUser)
{
    C3DMap *pMap = GetMapByID(zone);
    if (pMap == nullptr)
        return;
    ...
    C3DMap *pMap, char *pBuf, int len, int x, int z, float ref_x, float ref_z, CUser *pExceptUser)
{
    ...
    for (const auto &[_, pUid] : pMap->m_ppRegion[x][z].m_RegionUserArray)
    {
        ...
        {
            double fDist =
                sqrt(pow((pUser->m_pUserData->m_curx - ref_x), 2) + pow((pUser->m_pUserData->m_curz - ref_z), 2));
            if (fDist < 32)
                pUser->RegionPacketAdd(pBuf, len);
        }
    }
...
bool EbenezerApp::LoadCoefficientTable()
{
    ...
    if (!loader.Load_ForbidEmpty())
    {
        spdlog::error("EbenezerApp::LoadCoefficientTable: load failed - {}", loader.GetError().Message);
        return false;
    }
...
bool EbenezerApp::ProcessCommandLineArgs(const argparse::ArgumentParser &parser)
{
    ...
    {
        spdlog::error("Supplied map directory (--map-dir) doesn't exist or is inaccessible: {}", _overrideMapDir);
        return false;
    }
    ...
    {
        spdlog::error(
            "Supplied quests directory (--quests-dir) doesn't exist or is inaccessible: {}", _overrideQuestsDir);
        return false;
    }
...
bool EbenezerApp::LoadConfig(CIni &iniFile)
{
    ...
    else if (!questsDir.empty())
    {
        ...
        {
            spdlog::error("Configured quests directory doesn't exist or is inaccessible: {}", questsDir);
            return false;
        }
    }
    // Fallback to the default (don't save this).
    ...
    ConnectionManager::SetDatasourceConfig(modelUtil::DbType::GAME, datasourceName, datasourceUser, datasourcePass);
    ...
    if (m_nServerGroup != 0)
    {
        ...
        if (sgroup_count < 1)
        {
            spdlog::error("EbenezerApp::LoadConfig: Invalid server group count={}, must be 1+", sgroup_count);
            return false;
        }
...
void EbenezerApp::RegionNpcInfoForMe(CUser *pSendUser, int nType)
{
    ...
    if (nType == 1)
    {
        spdlog::get(logger::EbenezerRegion)
            ->info(
                "RegionNpcInfoForMe start: charId={} x={} z={}",
                pSendUser->m_pUserData->m_id,
                pSendUser->m_RegionX,
                pSendUser->m_RegionZ);
    }
    ...
    if (nType == 1)
    {
        ...
        spdlog::get(logger::EbenezerRegion)
            ->info(
                "RegionNpcInfoForMe end: charId={} x={} z={} count={}",
                pSendUser->m_pUserData->m_id,
                pSendUser->m_RegionX,
                pSendUser->m_RegionZ,
                npcCount);
...
int EbenezerApp::GetRegionNpcList(C3DMap *pMap, int region_x, int region_z, char *nid_buff, int &t_count, int nType)
{
    ...
    if (nType == 1)
    {
        spdlog::get(logger::EbenezerRegion)->info("GetRegionNpcList: x={} z={}", region_x, region_z);
    }
    ...
    for (const auto &[_, pNid] : pMap->m_ppRegion[region_x][region_z].m_RegionNpcArray)
    {
        ...
        if (pNpc != nullptr)
        {
            ...
            if (nType == 1)
            {
                spdlog::get(logger::EbenezerRegion)->info("GetRegionNpcList: serial={}", pNpc->m_sNid);
            }
        }
        else if (nType == 1)
        {
            spdlog::get(logger::EbenezerRegion)->error("GetRegionNpcList: not found: npcId={}", npcId);
        }
    }
...
void EbenezerApp::BattleZoneOpenTimer()
{
    ...
    /*	if( m_byBattleOpen == NO_BATTLE )	{	// When Battlezone is closed, open it!
            if( nWeek == m_nBattleZoneOpenWeek && nTime == m_nBattleZoneOpenHourStart )	{	// 수요일, 20시에 전쟁존
    open TRACE(_T("전쟁 자동 시작 - week=%d, time=%d\n"), nWeek, nTime); BattleZoneOpen(BATTLEZONE_OPEN);
    //			KickOutZoneUsers(ZONE_FRONTIER);	// Kick out users in frontier zone.
            }
        }
        else {	  // When Battlezone is open, close it!
            if( nWeek == (m_nBattleZoneOpenWeek+1) && nTime == m_nBattleZoneOpenHourEnd )	{	// 목요일, 0시에 전쟁존
    close TRACE(_T("전쟁 자동 종료 - week=%d, time=%d\n"), nWeek, nTime); m_byBanishFlag = 1;
            }
        }	*/
    ...
    if (m_byBanishFlag == 1)
    {
        ...
        if (m_sBanishDelay == 3)
        {
            ...
            spdlog::debug("EbenezerApp::BattleZoneOpenTimer: War ended, stage 1: m_bVictory={}", m_bVictory);
        }
...
bool EbenezerApp::LoadStartPositionTable()
{
    ...
    if (!loader.Load_ForbidEmpty())
    {
        spdlog::error("EbenezerApp::LoadStartPositionTable: load failed - {}", loader.GetError().Message);
        return false;
    }
...
bool EbenezerApp::LoadServerResourceTable()
{
    ...
    if (!loader.Load_ForbidEmpty())
    {
        spdlog::error("EbenezerApp::LoadServerResourceTable: load failed - {}", loader.GetError().Message);
        return false;
    }
...
bool EbenezerApp::LoadAllKnights()
{
    ...
    loader.SetProcessFetchCallback([this](db::ModelRecordSet<ModelType> &recordset) {
        do
        {
            ...
            {
                spdlog::warn(
                    "EbenezerApp::LoadAllKnights: IDName too long, truncating - knightsId={}", pKnights->m_sIndex);
            }
            ...
            {
                spdlog::warn(
                    "EbenezerApp::LoadAllKnights: Chief too long, truncating - knightsId={}", pKnights->m_sIndex);
            }
            ...
            if (row.ViceChief1.has_value())
            {
                ...
                {
                    spdlog::warn(
                        "EbenezerApp::LoadAllKnights: ViceChief_1 too long, truncating - knightsId={}",
                        pKnights->m_sIndex);
                }
            }
            ...
            if (row.ViceChief2.has_value())
            {
                ...
                {
                    spdlog::warn(
                        "EbenezerApp::LoadAllKnights: ViceChief_2 too long, truncating - knightsId={}",
                        pKnights->m_sIndex);
                }
            }
            ...
            if (row.ViceChief3.has_value())
            {
                ...
                {
                    spdlog::warn(
                        "EbenezerApp::LoadAllKnights: ViceChief_3 too long, truncating - knightsId={}",
                        pKnights->m_sIndex);
                }
            }
            ...
            {
                spdlog::error(
                    "EbenezerApp::LoadAllKnights: failed to put into KnightsMap [knightsId={}]", pKnights->m_sIndex);
                delete pKnights;
            }
        } while (recordset.next());
    });
...
bool EbenezerApp::LoadAllKnightsUserData()
{
    ...
    if (!loader.Load_AllowEmpty())
    {
        spdlog::error("EbenezerApp::LoadAllKnightsUserData: load failed - {}", loader.GetError().Message);
        return false;
    }
...
bool EbenezerApp::LoadKnightsSiegeWarfareTable()
{
    ...
    if (!loader.Load_AllowEmpty())
    {
        spdlog::error("EbenezerApp::LoadKnightsSiegeWarfareTable: load failed - {}", loader.GetError().Message);
        return false;
    }
...
{
    spdlog::info(
        "EbenezerApp::WritePacketLog: send={} realsend={} recv={}",
        m_iPacketCount,
        m_iSendPacketCount,
        m_iRecvPacketCount);
}
    ...
    for (int i = 0; i < socketCount; i++)
    {
        ...
        if (pUser->GetState() == CONNECTION_STATE_GAMESTART)
        {
            if (pUser->m_sAliveCount > 3)
            {
                pUser->Close();
                spdlog::debug(
                    "EbenezerApp::CheckAliveUser: User Alive Close [userId={} charId={}]",
                    pUser->GetSocketID(),
...
void EbenezerApp::Send_CommandChat(char *pBuf, int len, int nation, CUser * /*pExceptUser*/)
{
    ...
    for (int i = 0; i < socketCount; i++)
    {
        ...
        if (pUser->GetState() == CONNECTION_STATE_GAMESTART && pUser->m_pUserData->m_bNation == nation)
            pUser->Send(pBuf, len);
    }
}
...
bool EbenezerApp::LoadKnightsRankTable()
{
    ...
    if (!loader.Load_AllowEmpty())
    {
        spdlog::error("EbenezerApp::LoadKnightsRankTable: load failed - {}", loader.GetError().Message);
        return false;
    }

    std::string strKarusCaptainName = fmt::format_db_resource(
        IDS_KARUS_CAPTAIN,
        strKarusCaptain[0],
        strKarusCaptain[1],
        strKarusCaptain[2],
    ...
    std::string strElmoCaptainName = fmt::format_db_resource(
        IDS_ELMO_CAPTAIN,
        strElmoCaptain[0],
        strElmoCaptain[1],
        strElmoCaptain[2],
...
bool EbenezerApp::LoadEventTriggerTable()
{
    ...
    loader.SetProcessFetchCallback([&](db::ModelRecordSet<ModelType> &recordset) {
        do
        {
            ...
            {
                spdlog::error(
                    "EbenezerApp::LoadEventTriggerTable: failed to insert into EventTriggerMap [NpcType={} NpcId={}]",
                    row.NpcType,
                    row.NpcId);
            }
        } while (recordset.next());
    ...
    if (!loader.Load_AllowEmpty())
    {
        spdlog::error("EbenezerApp::LoadEventTriggerTable: load failed - {}", loader.GetError().Message);
        return false;
    }
