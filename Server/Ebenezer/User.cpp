...
void CUser::SendCompressingPacket(const char *pData, int len)
{
    ...
    {
        spdlog::error(
            "CUser::SendCompressingPacket: compression failed [out_len={} pBuffSize={}]", out_len, sizeof(pBuff));
        Send((char *)pData, len);
        return;
    }
...
bool CUser::PullOutCore(char *&data, int &length)
{
    ...
    {
        spdlog::error(
            "TcpSocket::PullOutCore: {}: failed to detect header ({:2X}, {:2X})",
            _socketId,
            tmp_buffer[0],
            tmp_buffer[1]);
    ...
    {
        spdlog::error(
            "User::PullOutCore: {}: failed to detect tail ({:2X}, {:2X})",
            _socketId,
            tmp_buffer[endPos],
            tmp_buffer[endPos + 1]);
    ...
    if (_jvCryptionEnabled)
    {
        ...
        {
            spdlog::error(
                "User::PullOutCore: {}: Insufficient packet length [{}] for a decrypted packet", _socketId, length);
            Close();
            return false;
        }
        ...
        int decryptedLength =
            _jvCryption.JvDecryptionWithCRC32(length, &tmp_buffer[startPos + 2], &decryption_buffer[0]);
        ...
        {
            spdlog::error(
                "User::PullOutCore: {}: recvValue error... len={}, recvValue={}, prev={}",
                _socketId,
                length,
                recvValue,
                _recvValue);
        ...
        if (length <= 0)
        {
            spdlog::error("User::PullOutCore: {}: decrypted packet length too small... len={}", _socketId, length);
...
void CUser::CloseProcess()
{
    ...
    /* 부디 잘 작동하길 ㅠ.ㅠ
        if (!m_bZoneChangeFlag) {
            if (m_pUserData->m_bZone == ZONE_BATTLE || (m_pUserData->m_bZone != m_pUserData->m_bNation &&
       m_pUserData->m_bZone < 3) ) { model::Home* pHomeInfo = nullptr;	// Send user back home in case it was the
       battlezone. pHomeInfo = m_pMain->m_HomeTableMap.GetData(m_pUserData->m_bNation); if (!pHomeInfo) return;
...
void CUser::Parsing(int len, char *pData)
{
    ...
    spdlog::trace(
        "User::Parsing: userId={} charId={} command={:02X} len={}", GetSocketID(), m_pUserData->m_id, command, len);
...
void CUser::SelCharToAgent(char *pBuf)
{
    ...
    if (m_pMain->m_nServerNo != pMap->m_nServerNo)
    {
        ...
        if (pInfo == nullptr)
        {
            spdlog::error("User::SelCharToAgent: serverId={} not registered [zoneId={}]", pMap->m_nServerNo, zoneId);
            goto fail_return;
        }
        ...
        Send(send_buff, send_index);
        spdlog::error(
            "User::SelCharToAgent: server change [charId={} ip={} init={}]", charId, pInfo->strServerIP, bInit);
        return;
    }
    ...
    {
        spdlog::debug(
            "User::SelCharToAgent: accountId={} charId={} packetCount={}",
            m_strAccountID,
            charId,
...
void CUser::LogOut()
{
    ...
    spdlog::debug("User::LogOut: accountId={} charId={}", m_pUserData->m_Accountid, m_pUserData->m_id);
    ...
    if (count > 29)
    {
        spdlog::error("User::LogOut: send error: accountId={} charId={}", m_pUserData->m_Accountid, m_pUserData->m_id);
    }
    ...
    //	if (m_pUserData->m_bKnights > 0)
    //		m_pMain->m_KnightsManager.ModifyKnightsUser(m_pUserData->m_bKnights, m_pUserData->m_id,
    //m_pUserData->m_bFame, m_pUserData->m_bLevel, m_pUserData->m_sClass, 0);
...
void CUser::MoveProcess(char *pBuf)
{
    ...
    if (m_bResHpType == USER_DEAD || m_pUserData->m_sHp == 0)
    {
        ...
        {
            spdlog::warn(
                "User::MoveProcess: dead user is moving [charId={} socketId={} resHpType={} hp={} speed={} x={} z={}]",
                m_pUserData->m_id,
                _socketId,
                m_bResHpType,
                m_pUserData->m_sHp,
...
void CUser::Attack(char *pBuf)
{
    ...
    {
        spdlog::error(
            "User::Attack: dead user cannot attack [charId={} resHpType={} hp={}]",
            m_pUserData->m_id,
            _socketId,
            m_bResHpType,
            m_pUserData->m_sHp);
    ...
    if (tid < NPC_BAND)
    {
        ...
        if (pTUser == nullptr || pTUser->m_bResHpType == USER_DEAD || pTUser->m_bAbnormalType == ABNORMAL_BLINKING ||
        ...
        else
        {
            ...
            if (pTable != nullptr)
            {
                //				TRACE(_T("Distance : %f  , Table Distance : %f  \r\n"), distance, pTable->Range
                /// 10.0f); 				if (distance > (pTable->Range / 10.0f))
            ...
            else
            {
                ...
                // pTUser->m_pUserData->m_sHp, damage, m_sTotalHit);
                ...
                if (pTUser->m_pUserData->m_sHp == 0)
                {
                    ...
                    if (pTUser->m_pUserData->m_bFame == COMMAND_CAPTAIN)
                    {
                        ...
                        m_pMain->Send_Region(
                            send_buff, send_index, pTUser->m_pUserData->m_bZone, pTUser->m_RegionX, pTUser->m_RegionZ);
...
void CUser::SendMyInfo(int type)
{
    ...
    //	if( m_pUserData->m_bKnights > 0 )	{
    //		m_pMain->m_KnightsManager.ModifyKnightsUser( m_pUserData->m_bKnights, m_pUserData->m_id,
    //m_pUserData->m_bFame, m_pUserData->m_bLevel, m_pUserData->m_sClass, 1);
    //	}
}
...
void CUser::Chat(char *pBuf)
{
    ...
    {
    case GENERAL_CHAT:
        m_pMain->Send_NearRegion(
            send_buff,
            send_index,
            (int)m_pUserData->m_bZone,
            m_RegionX,
            m_RegionZ,
            m_pUserData->m_curx,
            m_pUserData->m_curz);
...
{
    model::Coefficient *p_TableCoefficient = m_pMain->m_CoefficientTableMap.GetData(m_pUserData->m_sClass);
    if (p_TableCoefficient == nullptr)
        return;
    ...
    {
        m_iMaxHp =
            (int16_t)(((p_TableCoefficient->HitPoint * m_pUserData->m_bLevel * m_pUserData->m_bLevel * temp_sta) +
...
void CUser::SetMaxMp()
{
    ...
    {
        m_iMaxMp =
            (int16_t)((p_TableCoefficient->ManaPoint * m_pUserData->m_bLevel * m_pUserData->m_bLevel * temp_intel) +
...
void CUser::Regene(char *pBuf, int magicid)
{
    ...
    {
        // TCHAR logstr[1024] = {};
        //_stprintf(logstr, _T("<------ User Regene ,, nid=%d, name=%hs, type=%d ******"), _socketId, m_pUserData->m_id,
        //m_bResHpType); TimeTrace(logstr);
    }
#endif
...
void CUser::ZoneChange(int zone, float x, float z)
{
    ...
    if (m_pMain->m_byBattleOpen == NATION_BATTLE)
    {
        if (m_pUserData->m_bZone == BATTLE_ZONE)
        {
            ...
            if (pMap->m_bType == 1 && m_pUserData->m_bNation != zone)
            {
                ...
                {
                    spdlog::error(
                        "User::ZoneChange: zone not open for invasion [charId={} nation={} enemyNationOpen={}]",
                        m_pUserData->m_id,
    ...
    // m_pUserData->m_bNation, zone, x, z);
    ...
    if (m_pMain->m_nServerNo != pMap->m_nServerNo)
    {
        ...
        spdlog::debug(
            "User::ZoneChange: [userId={} accountId={} charId={} zoneId={} x={} z={}]",
            _socketId,
            m_strAccountID,
            m_pUserData->m_id,
            zone,
...
void CUser::RemoveRegion(int del_x, int del_z)
{
    ...
    if (del_x != 0)
    {
        ...
        // TRACE(_T("Remove : (%d %d), (%d %d), (%d %d)\n"), m_RegionX+del_x*2, m_RegionZ+del_z-1, m_RegionX+del_x*2,
        // m_RegionZ+del_z, m_RegionX+del_x*2, m_RegionZ+del_z+1 );
    }
    ...
    if (del_z != 0)
    {
        ...
        else
        {
            ...
            // TRACE(_T("Remove : (%d %d), (%d %d), (%d %d)\n"), m_RegionX+del_x-1, m_RegionZ+del_z*2, m_RegionX+del_x,
            // m_RegionZ+del_z*2, m_RegionX+del_x+1, m_RegionZ+del_z*2 );
        }
    }
}
...
void CUser::InsertRegion(int del_x, int del_z)
{
    ...
    if (del_x != 0)
    {
        ...
        // TRACE(_T("Insert : (%d %d), (%d %d), (%d %d)\n"), m_RegionX+del_x, m_RegionZ-1, m_RegionX+del_x, m_RegionZ,
        // m_RegionX+del_x, m_RegionZ+1 );
    }
    ...
    if (del_z != 0)
    {
        ...
        else
        {
            ...
            // TRACE(_T("Insert : (%d %d), (%d %d), (%d %d)\n"), m_RegionX-1, m_RegionZ+del_z, m_RegionX,
            // m_RegionZ+del_z, m_RegionX+1, m_RegionZ+del_z );
        }
    }
}
...
void CUser::SetSlotItemValue()
{
    ...
    for (int i = 0; i < SLOT_MAX; i++)
    {
        ...
        if (i == LEFTHAND)
        {
            if ((m_pUserData->m_sClass == BERSERKER || m_pUserData->m_sClass == BLADE))
                //			m_sItemHit += item_hit * (double) (m_pUserData->m_bstrSkill[PRO_SKILL1] / 60.0);    //
                //성래씨 요청 ^^;
    ...
    model::Item *pLeftHand = m_pMain->m_ItemTableMap.GetData(m_pUserData->m_sItemArray[LEFTHAND].nNum);
    ...
    model::Item *pRightHand = m_pMain->m_ItemTableMap.GetData(m_pUserData->m_sItemArray[RIGHTHAND].nNum);
    ...
    if (magicid > 0)
    {
        ...
        else if (pTable->Type1 == 2)
        {
            ...
            if (pType2->HitType == 1
                /*|| pType2->HitType == 2*/)
                temp_hit =
                    static_cast<int16_t>(m_sTotalHit * m_bAttackAmount * (pType2->DamageMod / 100.0f) / 100); // 표시
            else
...
void CUser::SetUserAbility()
{
    ...
    */
    ...
    m_fTotalHitRate = static_cast<float>(
        ((1 + p_TableCoefficient->HitRate * m_pUserData->m_bLevel * temp_dex) * m_sItemHitrate / 100) *
...
void CUser::ItemMove(char *pBuf)
{
    ...
    {
        // 장착
        if (destpos == HEAD || destpos == BREAST || destpos == SHOULDER || destpos == LEFTHAND ||
            destpos == RIGHTHAND || destpos == LEG || destpos == GLOVE || destpos == FOOT)
    ...
    {
        // 해제
        if (srcpos == HEAD || srcpos == BREAST || srcpos == SHOULDER || srcpos == LEFTHAND || srcpos == RIGHTHAND ||
            srcpos == LEG || srcpos == GLOVE || srcpos == FOOT)
...
void CUser::ItemTrade(char *pBuf)
{
    ...
    {
        spdlog::error(
            "User::ItemTrade: dead user cannot trade [charId={} socketId={} resHpType={} hp={} x={} z={}]",
            m_pUserData->m_id,
            _socketId,
            m_bResHpType,
            m_pUserData->m_sHp,
    ...
    if (type == 0x01)
    {
        ...
        ItemLogToAgent(
            m_pUserData->m_id,
            pNpc->m_strName,
            ITEM_LOG_MERCHANT_BUY,
            m_pUserData->m_sItemArray[SLOT_MAX + pos].nSerialNum,
            itemid,
            count,
            pTable->Durability);
    }
...
void CUser::ItemGet(char *pBuf)
{
    ...
    if (pos != 0xFF)
    {
        ...
        pGetUser->ItemLogToAgent(
            pGetUser->m_pUserData->m_id,
            "MONSTER",
            ITEM_LOG_MONSTER_GET,
            pGetUser->m_pUserData->m_sItemArray[SLOT_MAX + pos].nSerialNum,
            itemid,
            count,
            pTable->Durability);
    }
...
void CUser::ExchangeReq(char *pBuf)
{
    ...
    {
        spdlog::error(
            "User::ExchangeReq: dead user cannot exchange [charId={} socketId={} resHpType={} hp={} x={} z={}]",
            m_pUserData->m_id,
            _socketId,
            m_bResHpType,
            m_pUserData->m_sHp,
...
void CUser::ExchangeDecide()
{
    ...
    else
    {
        ...
        if (bSuccess)
        {
            ...
            {
                ItemLogToAgent(
                    m_pUserData->m_id, pUser->m_pUserData->m_id, ITEM_LOG_EXCHANGE_GET, 0, ITEM_GOLD, getmoney, 0);
            }

            if (putmoney > 0)
            {
                ItemLogToAgent(
                    m_pUserData->m_id, pUser->m_pUserData->m_id, ITEM_LOG_EXCHANGE_PUT, 0, ITEM_GOLD, putmoney, 0);
            }
...
void CUser::ItemWoreOut(int type, int damage)
{
    ...
    {
        if (m_pUserData->m_sItemArray[RIGHTHAND].nNum != 0 && m_pUserData->m_sItemArray[RIGHTHAND].sDuration != 0)
        {
            pTable = m_pMain->m_ItemTableMap.GetData(m_pUserData->m_sItemArray[RIGHTHAND].nNum);
            if (pTable != nullptr
                // 2 == DEFENCE ITEM
                && pTable->Slot != 2)
            {
                ...
                ItemDurationChange(
                    RIGHTHAND, pTable->Durability, m_pUserData->m_sItemArray[RIGHTHAND].sDuration, worerate);
            }
        }

        if (m_pUserData->m_sItemArray[LEFTHAND].nNum != 0 && m_pUserData->m_sItemArray[LEFTHAND].sDuration != 0)
        {
                ...
                && pTable->Slot != 2)
            {
                ...
                ItemDurationChange(
                    LEFTHAND, pTable->Durability, m_pUserData->m_sItemArray[LEFTHAND].sDuration, worerate);
            }
        }
    }
    else if (type == DURABILITY_TYPE_DEFENCE)
    {
        if (m_pUserData->m_sItemArray[HEAD].nNum != 0 && m_pUserData->m_sItemArray[HEAD].sDuration != 0)
        {
        ...
        if (m_pUserData->m_sItemArray[BREAST].nNum != 0 && m_pUserData->m_sItemArray[BREAST].sDuration != 0)
        {
        ...
        if (m_pUserData->m_sItemArray[LEG].nNum != 0 && m_pUserData->m_sItemArray[LEG].sDuration != 0)
        {
        ...
        if (m_pUserData->m_sItemArray[GLOVE].nNum != 0 && m_pUserData->m_sItemArray[GLOVE].sDuration != 0)
        {
        ...
        if (m_pUserData->m_sItemArray[FOOT].nNum != 0 && m_pUserData->m_sItemArray[FOOT].sDuration != 0)
        {
...
void CUser::HPTimeChange(double currentTime)
{
    ...
    // char logstr[128] = {};
    // wsprintf(logstr, "HPTimeChange ,, nid=%d, name=%hs, hp=%d, type=%d ******", _socketId, m_pUserData->m_id,
    // m_pUserData->m_sHp, m_bResHpType); TimeTrace(logstr);
...
void CUser::ItemRepair(char *pBuf)
{
    ...
    money = static_cast<int>(
        (((pTable->BuyPrice - 10) / 10000.0f) + pow(pTable->BuyPrice, 0.75)) * quantity / (double)durability);
...
uint8_t CUser::ItemCountChange(int itemid, int type, int amount)
{
    ...
    for (int i = SLOT_MAX * type; i < SLOT_MAX + HAVE_MAX * type; i++)
    {
        ...
        if (pTable->RequiredStamina > (m_pUserData->m_bSta + m_sItemSta + m_sStaAmount) && pTable->RequiredStamina != 0)
            return result;
...
void CUser::ItemRemove(char *pBuf)
{
    ...
    ItemLogToAgent(m_pUserData->m_id, "DESTROY", ITEM_LOG_DESTROY, serial, itemid, count, durability);
    return;
...
void CUser::WarehouseProcess(char *pBuf)
{
    ...
    {
        spdlog::error(
            "User::WarehouseProcess: dead user cannot use warehouse [charId={} socketId={} resHpType={} hp={} x={} "
            "z={}]",
            m_pUserData->m_id,
            _socketId,
            m_bResHpType,
    ...
    case WAREHOUSE_INPUT:
        ...
        if (m_pUserData->m_sWarehouseArray[reference_pos + destpos].nNum && pTable->Countable == 0)
            goto fail_return;
        ...
        if (pTable->Countable == 0 && m_pUserData->m_sWarehouseArray[reference_pos + destpos].nSerialNum == 0)
    ...
    case WAREHOUSE_OUTPUT:
        ...
        ItemLogToAgent(
            ...
            count,
            m_pUserData->m_sItemArray[SLOT_MAX + destpos].sDuration);
        // TRACE(_T("WARE OUTPUT : %hs %hs %d %d %d %d %d"), m_pUserData->m_id, m_pUserData->m_Accountid,
        // ITEM_WAREHOUSE_GET, 0, itemid, count, m_pUserData->m_sItemArray[SLOT_MAX+destpos].sDuration );
        break;
...
void CUser::MarketBBSDelete(char *pBuf)
{
    ...
    if (buysell_index == MARKET_BBS_BUY)
    {
        if (m_pMain->m_sBuyID[delete_id] != _socketId && m_pUserData->m_bAuthority != AUTHORITY_MANAGER)
            goto fail_return;
    ...
    {
        if (m_pMain->m_sSellID[delete_id] != _socketId && m_pUserData->m_bAuthority != AUTHORITY_MANAGER)
            goto fail_return;
...
void CUser::MarketBBSReport(char *pBuf, uint8_t type)
{
    ...
    int16_t page_index = 0, start_counter = 0, valid_counter = 0, BBS_Counter = 0, title_length = 0, message_length = 0;
    uint8_t result = 0, sub_result = 1, buysell_index = 0;
    char send_buff[8192] = {};
...
void CUser::SetLogInInfoToDB(uint8_t bInit)
{
    ...
    if (pInfo == nullptr)
    {
        spdlog::error("User::SetLogInInfoToDB: invalid serverId={}, closing user", m_pMain->m_nServerNo);
        Close();
        return;
    }
...
bool CUser::CheckEventLogic(const EVENT_DATA *pEventData)
{
    ...
    for (LOGIC_ELSE *pLE : pEventData->m_arLogicElse)
    {
        ...
        case LOGIC_CHECK_CLASS:
            if (CheckClass(
                    pLE->m_LogicElseInt[0],
                    pLE->m_LogicElseInt[1],
                    pLE->m_LogicElseInt[2],
                    pLE->m_LogicElseInt[3],
                    pLE->m_LogicElseInt[4],
        ...
        case LOGIC_CHECK_MONSTER_CHALLENGE_TIME:
            if (m_pMain->_monsterChallengeActiveType == pLE->m_LogicElseInt[0] && m_pMain->_monsterChallengeState != 0)
                bExact = true; // NOTE: officially this returns true, ending check processing immediately
            break;
        ...
        case LOGIC_CHECK_NO_CASTLE:
            if (m_pUserData->m_bKnights != m_pMain->m_KnightsSiegeWar._masterKnights ||
                m_pMain->m_KnightsSiegeWar._masterKnights == 0 || m_pUserData->m_bFame != CHIEF)
                bExact = true; // NOTE: officially this returns true, ending check processing immediately
            break;
...
void CUser::ItemLogToAgent(
    const char *srcid, const char *tarid, int type, int64_t serial, int itemid, int count, int durability)
{
    int send_index = 0, retvalue = 0;
    char send_buff[1024] = {};
...
void CUser::GoldGain(int gold)
{
    ...
    {
        spdlog::error(
            "User::GoldGain: user has negative gold [charId={} existingGold={}]",
            m_pUserData->m_id,
            m_pUserData->m_iGold);
        return;
    }
...
bool CUser::GoldLose(int gold)
{
    ...
    {
        spdlog::error(
            "User::GoldLose: user has negative gold [charId={} existingGold={}]",
            m_pUserData->m_id,
            m_pUserData->m_iGold);
        return false;
    }
...
bool CUser::CheckClass(
    int16_t class1, int16_t class2, int16_t class3, int16_t class4, int16_t class5, int16_t class6) const
{
...
bool CUser::JobGroupCheck(int16_t jobgroupid) const
{
    ...
    if (jobgroupid < 100)
    {
        ...
        case JOB_GROUP_WARRIOR:
            if (m_pUserData->m_sClass == CLASS_KA_WARRIOR || m_pUserData->m_sClass == CLASS_KA_BERSERKER ||
                m_pUserData->m_sClass == CLASS_KA_GUARDIAN || m_pUserData->m_sClass == CLASS_EL_WARRIOR ||
                m_pUserData->m_sClass == CLASS_EL_BLADE || m_pUserData->m_sClass == CLASS_EL_PROTECTOR)
                return true;
            break;

        case JOB_GROUP_ROGUE:
            if (m_pUserData->m_sClass == CLASS_KA_ROGUE || m_pUserData->m_sClass == CLASS_KA_HUNTER ||
                m_pUserData->m_sClass == CLASS_KA_PENETRATOR || m_pUserData->m_sClass == CLASS_EL_ROGUE ||
                m_pUserData->m_sClass == CLASS_EL_RANGER || m_pUserData->m_sClass == CLASS_EL_ASSASIN)
                return true;
            break;

        case JOB_GROUP_MAGE:
            if (m_pUserData->m_sClass == CLASS_KA_WIZARD || m_pUserData->m_sClass == CLASS_KA_SORCERER ||
                m_pUserData->m_sClass == CLASS_KA_NECROMANCER || m_pUserData->m_sClass == CLASS_EL_WIZARD ||
                m_pUserData->m_sClass == CLASS_EL_MAGE || m_pUserData->m_sClass == CLASS_EL_ENCHANTER)
                return true;
            break;

        case JOB_GROUP_CLERIC:
            if (m_pUserData->m_sClass == CLASS_KA_PRIEST || m_pUserData->m_sClass == CLASS_KA_SHAMAN ||
                m_pUserData->m_sClass == CLASS_KA_DARKPRIEST || m_pUserData->m_sClass == CLASS_EL_PRIEST ||
                m_pUserData->m_sClass == CLASS_EL_CLERIC || m_pUserData->m_sClass == CLASS_EL_DRUID)
                return true;
            break;
        ...
        case JOB_GROUP_DEFENSE_WARRIOR:
            if (m_pUserData->m_sClass == CLASS_KA_GUARDIAN || m_pUserData->m_sClass == CLASS_EL_PROTECTOR)
                return true;
            break;
        ...
        case JOB_GROUP_ASSASSIN:
            if (m_pUserData->m_sClass == CLASS_KA_PENETRATOR || m_pUserData->m_sClass == CLASS_EL_ASSASIN)
                return true;
            break;
        ...
        case JOB_GROUP_PET_MAGE:
            if (m_pUserData->m_sClass == CLASS_KA_NECROMANCER || m_pUserData->m_sClass == CLASS_EL_ENCHANTER)
                return true;
            break;
        ...
        case JOB_GROUP_CURSE_CLERIC:
            if (m_pUserData->m_sClass == CLASS_KA_DARKPRIEST || m_pUserData->m_sClass == CLASS_EL_DRUID)
                return true;
            break;
        }
    }
...
void CUser::KickOutZoneUser(bool home)
{
    ...
    if (home)
    {
        ...
        {
            spdlog::error(
                "User::KickOutZoneUser: no regene event found [charId={} regeneEventId={} zoneIndex={}]",
                m_pUserData->m_id,
                regene_event,
                zoneindex);
            KickOutZoneUser();
...
void CUser::GameStart(char *pBuf)
{
    ...
    if (opcode == 1)
    {
        ...
        spdlog::debug("User::GameStart: loading [charId={} socketId={}]", m_pUserData->m_id, _socketId);
    ...
    else if (opcode == 2)
    {
        ...
        spdlog::debug("User::GameStart: in game [charId={} socketId={}]", m_pUserData->m_id, _socketId);
