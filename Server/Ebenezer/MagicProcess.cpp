...
void CMagicProcess::MagicPacket(char *pBuf)
{
    ...
    else if (m_pMain->IsValidUserId(sid))
    {
        ...
        if (pSrcUser->m_bResHpType == USER_DEAD || pSrcUser->m_pUserData->m_sHp == 0)
        {
            spdlog::error(
                ...
                pSrcUser->m_pUserData->m_id,
                pSrcUser->GetSocketID(),
                pSrcUser->m_bResHpType,
    ...
    if (pTUser != nullptr)
    {
        ...
        if (pMagic->Type1 == 4)
        {
            if (pMagic->Moral < 5)
            {
                model::MagicType4 *pType4 =
                    m_pMain->m_MagicType4TableMap.GetData(magicid); // Get magic skill table type 4.
                if (pType4 == nullptr)
                    return;
                ...
                if (pTUser->m_bType4Buff[pType4->BuffType - 1] > 0)
                {
                    ...
                    if (m_bMagicState == MAGIC_STATE_CASTING)
                    {
                        m_pMain->Send_Region(
                            send_buff,
                            send_index,
                            m_pSrcUser->m_pUserData->m_bZone,
                            m_pSrcUser->m_RegionX,
                            m_pSrcUser->m_RegionZ,
                            nullptr,
                            false);
                    }
                    else
        ...
        else if (pMagic->Type1 == 3)
        {
            if (pMagic->Moral < 5)
            {
                model::MagicType3 *pType3 =
                    m_pMain->m_MagicType3TableMap.GetData(magicid); // Get magic skill table type 4.
                if (pType3 == nullptr)
                    return;
        ...
        if (m_pMain->IsValidUserId(sid))
        {
                ...
                (tid == -1 && (pMagic->Moral == MORAL_AREA_ENEMY || pMagic->Moral == MORAL_SELF_AREA)))
            {
                ...
                if (m_pSrcUser->m_pUserData->m_sItemArray[RIGHTHAND].nNum != 0)
                {
                    ...
                    if (pRightHand != nullptr && m_pSrcUser->m_pUserData->m_sItemArray[LEFTHAND].nNum == 0 &&
                        (pRightHand->Kind / 10) == WEAPON_STAFF)
                    {
                        ...
                        if (pMagic->Type1 == 3)
                        {
                            ...
                            if (m_pSrcUser->m_bMagicTypeRightHand == pType3->Attribute)
                            {
                                ...
                                total_magic_damage += static_cast<int>(
                                    ((pRightHand->Damage * 0.8f) +
                                     (pRightHand->Damage * m_pSrcUser->m_pUserData->m_bLevel) / 30));
                            }
                            //
...
return_echo:
    ...
    {
        m_pMain->Send_Region(
            send_buff, send_index, pMon->m_sCurZone, pMon->m_sRegion_X, pMon->m_sRegion_Z, nullptr, false);
    }
}
    ...
    int magicid, int tid, int sid, uint8_t type, int data1, int /*data2*/, int /*data3*/)
{
    ...
    if (m_pMain->IsValidUserId(tid))
    {
        ...
        if (pTable->Type1 != 5)
        {
            if (pUser == nullptr || pUser->m_bResHpType == USER_DEAD || pUser->m_bAbnormalType == ABNORMAL_BLINKING)
                goto fail_return;
        }
    ...
    case MORAL_CLAN: // #14
        ...
        if (pUser != nullptr && pUser->m_pUserData->m_bKnights != m_pSrcUser->m_pUserData->m_bKnights)
            goto fail_return;
        break;
...
uint8_t CMagicProcess::ExecuteType1(
    int magicid, int sid, int tid, int data1, int data2, int data3) // Applied to an attack skill using a weapon.
{
                    ...
                    if (!bFlag)
                    {
                            ...
                            m_pMain->m_byBattleOpen == SNOW_BATTLE)
                        {
                            ...
                            spdlog::get(logger::EbenezerEvent)
                                ->info("{} killed {}", m_pSrcUser->m_pUserData->m_id, pTUser->m_pUserData->m_id);
...
void CMagicProcess::ExecuteType8(
    int magicid, int sid, int tid, int data1, int /*data2*/, int data3) // Warp, resurrection, and summon spells.
{
    int send_index = 0, result = 1; // Variable initialization. result == 1 : success, 0 : fail
    char send_buff[128] = {};
...
*/
            ...
            else if (pTUser->m_pUserData->m_bNation != pTUser->m_pUserData->m_bZone && pTUser->m_pUserData->m_bZone < 3)
            {
                // Land of Karus
        ...
        case 13:
            ...
            pTUser->ZoneChange(
                m_pSrcUser->m_pUserData->m_bZone, m_pSrcUser->m_pUserData->m_curx, m_pSrcUser->m_pUserData->m_curz);
            // pTUser->UserInOut( USER_IN );
            pTUser->UserInOut(USER_REGENE);
            // TRACE(_T(" Summon ,, name=%hs, x=%.2f, z=%.2f\n"), pTUser->m_pUserData->m_id,
            // pTUser->m_pUserData->m_curx, pTUser->m_pUserData->m_curz);
            break;
        ...
        else
        {
            ...
            if (attribute != 4)
            {
                damage = static_cast<int16_t>(
                    damage - ((righthand_damage * 0.8f) + (righthand_damage * m_pSrcUser->m_pUserData->m_bLevel) / 60) -
...
final_test:
    ...
    if (!bFlag)
    {
        ...
        {
            // Region Check!
            if (pTUser->m_RegionX == m_pSrcUser->m_RegionX && pTUser->m_RegionZ == m_pSrcUser->m_RegionZ)
            {
...
void CMagicProcess::Type4Cancel(int magicid, int tid)
{
    ...
    case 7:
        if ((pTUser->m_sStrAmount + pTUser->m_sStaAmount + pTUser->m_sDexAmount + pTUser->m_sIntelAmount +
             pTUser->m_sChaAmount) > 0)
        {
            pTUser->m_sDuration7 = 0;
            pTUser->m_fStartTime7 = 0.0;
