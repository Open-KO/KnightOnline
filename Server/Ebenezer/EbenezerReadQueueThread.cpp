...
EbenezerReadQueueThread::EbenezerReadQueueThread() : ReadQueueThread(EbenezerApp::instance()->m_LoggerRecvQueue)
{
}
...
void EbenezerReadQueueThread::process_packet(const char *buffer, int len)
{
    ...
    case WIZ_LOGOUT:
        ...
        {
            spdlog::debug("EbenezerApp::ReadQueueThread: WIZ_LOGOUT [charId={}]", pUser->m_pUserData->m_id);
            pUser->Close();
        }
        break;
