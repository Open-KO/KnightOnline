...
void MAP::RegionNpcAdd(int rx, int rz, int nid)
{
    ...
    *pInt = nid;
    ...
    // region->m_RegionNpcArray.GetSize());
}
...
void MAP::LoadObjectEvent(File &fs)
{
    ...
    for (int i = 0; i < iEventObjectCount; i++)
    {
        ...
        // TRACE(_T("Object - belong=%d, index=%d, type=%d, con=%d, sta=%d\n"), pEvent->sBelong, pEvent->sIndex,
        // pEvent->sType, pEvent->sControlNpcID, pEvent->sStatus);
...
bool MAP::LoadRoomEvent(int zone_number, const std::filesystem::path &eventDir)
{
    ...
    while (count < length)
    {
        ...
        if ((char)byte == '\n' || count == length)
        {
            ...
            {
                spdlog::warn(
                    "MAP::LoadRoomEvent({}): unhandled opcode '{}' ({}:{})", zone_number, first, filename, lineNumber);
            }
    ...
    for (int i = 1; i < nSize + 1; i++)
    {
        ...
        if (bFlag_1 && bFlag_2)
        {
            ...
            if (pRoom->m_byStatus == 1)
            {
                ...
                room_number = i;
                spdlog::trace("Map::IsRoomCheck: [roomEventId={} zoneId={} x={} z={}]", i, m_nZoneNumber, nX, nZ);
                // wsprintf(notify, "** 알림 : [%d Zone][%d] 방에 들어오신것을 환영합니다 **", m_nZoneNumber,
                // pRoom->m_sRoomNumber); m_pMain->SendSystemMsg( notify, m_nZoneNumber, PUBLIC_CHAT, SEND_ALL);
            }
            // 진행중인 상태
            else if (pRoom->m_byStatus == 2)
            {
                pRoom->m_byStatus = 3; // 클리어 상태로
                // wsprintf(notify, "** 알림 : [%d Zone][%d] 목표지점까지 도착해서 클리어 됩니다ㅇ **", m_nZoneNumber,
                // pRoom->m_sRoomNumber); m_pMain->SendSystemMsg( notify, m_nZoneNumber, PUBLIC_CHAT, SEND_ALL);
            }
...
CRoomEvent *MAP::SetRoomEvent(int number)
{
    ...
    {
        spdlog::error(
            "Map::SetRoomEvent: RoomEvent duplicate definition [roomEventId={} zoneId={}]", number, m_nZoneNumber);
        return nullptr;
    }
...
bool MAP::IsRoomStatusCheck()
{
    ...
    for (int i = 1; i < nTotalRoom; i++)
    {
        ...
        if (pRoom == nullptr)
        {
            spdlog::warn("Map::IsRoomStatusCheck: RoomEvent null [roomEventId={} zoneId={}", i, m_nZoneNumber);
            continue;
            // return nullptr;
        }
        ...
        if (m_byRoomStatus == 1)
        {
            ...
            if (m_byRoomType == 0)
            {
                ...
                if (nTotalRoom == nClearRoom)
                {
                    m_byRoomStatus = 2;
                    spdlog::trace(
                        "Map::IsRoomStatusCheck: all rooms cleared [zoneId={} roomType={} roomStatus={}]",
                        m_nZoneNumber,
                        m_byRoomType,
                        m_byRoomStatus);
                    return true;
        ...
        else if (m_byRoomStatus == 2)
        {
            if (m_byInitRoomCount >= 10)
            {
                ...
                if (nTotalRoom == nClearRoom)
                {
                    m_byRoomStatus = 3;
                    spdlog::trace(
                        "Map::IsRoomStatusCheck: room initialized [zoneId={} roomType={} roomStatus={}]",
                        m_nZoneNumber,
                        m_byRoomType,
                        m_byRoomStatus);
                    return true;
...
void MAP::InitializeRoom()
{
    ...
    for (int i = 1; i < nTotalRoom; i++)
    {
        ...
        if (pRoom == nullptr)
        {
            spdlog::error("Map::InitializeRoom: RoomEvent null [roomEventId={} zoneId={}]", i, m_nZoneNumber);
            continue;
        }
