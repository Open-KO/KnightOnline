...
AIServerApp::AIServerApp(AIServerLogger &logger) : AppThread(logger)
{
    ...
    _checkAliveThread = std::make_unique<TimerThread>(10s, std::bind(&AIServerApp::CheckAliveTest, this));
}
...
bool AIServerApp::ListenByServerZoneType()
{
    ...
    {
        spdlog::error(
            "AIServerApp::ListenByServerZoneType: failed to associate listen port for zone {}", _serverZoneType);
        return false;
    }
...
bool AIServerApp::GetMagicTableData()
{
    ...
    {
        spdlog::error("AIServerApp::GetMagicTableData: load failed - {}", loader.GetError().Message);
        return false;
    }
...
bool AIServerApp::GetMakeWeaponItemTableData()
{
    ...
    if (!loader.Load_ForbidEmpty())
    {
        spdlog::error("AIServerApp::GetMakeWeaponItemTableData: load failed - {}", loader.GetError().Message);
        return false;
    }
...
{
    recordset_loader::STLMap<MakeWeaponTableMap, model::MakeDefensive> loader(_makeDefensiveTableMap);
    if (!loader.Load_ForbidEmpty())
    {
        spdlog::error("AIServerApp::GetMakeDefensiveItemTableData: load failed - {}", loader.GetError().Message);
        return false;
    }
...
bool AIServerApp::GetMakeGradeItemTableData()
{
    ...
    if (!loader.Load_ForbidEmpty())
    {
        spdlog::error("AIServerApp::GetMakeGradeItemTableData: load failed - {}", loader.GetError().Message);
        return false;
    }
...
bool AIServerApp::GetMakeRareItemTableData()
{
    ...
    if (!loader.Load_ForbidEmpty())
    {
        spdlog::error("AIServerApp::GetMakeRareItemTableData: load failed - {}", loader.GetError().Message);
        return false;
    }
...
bool AIServerApp::GetMakeItemGroupTableData()
{
    ...
    if (!loader.Load_ForbidEmpty())
    {
        spdlog::error("AIServerApp::GetMakeItemGroupTableData: load failed - {}", loader.GetError().Message);
        return false;
    }
...
bool AIServerApp::GetZoneInfoTableData()
{
    ...
    if (!loader.Load_ForbidEmpty())
    {
        spdlog::error("AIServerApp::GetZoneInfoTableData: load failed - {}", loader.GetError().Message);
        return false;
    }
...
bool AIServerApp::GetMonsterTableData()
{
    ...
    if (!loader.Load_ForbidEmpty())
    {
        spdlog::error("AIServerApp::GetMonsterTableData: load failed - {}", loader.GetError().Message);
        return false;
    }
        ...
        do
        {
            ...
            if (_serverZoneType == nServerNum || _serverZoneType == UNIFY_ZONE)
            {
                for (int j = 0; j < nMonsterNumber; j++)
                {
                    ...
                    {
                        spdlog::warn(
                            "AIServerApp::LoadNpcPosTable: RegTime below minimum value of 15s [npcId={}, serial={}, "
                            "npcName={}, RegTime={}]",
                            pNpc->m_sSid,
                    ...
                    if (pNpc->m_byMoveType == 2 || pNpc->m_byMoveType == 3)
                    {
                        ...
                        {
                            spdlog::error(
                                "AIServerApp::LoadNpcPosTable: NPC expects path to be set [zoneId={} serial={}, "
                                "npcId={}, npcName={}, moveType={}, pathCount={}]",
                                row->ZoneId,
                                pNpc->m_sNid + NPC_BAND,
                                pNpc->m_sSid,
                                pNpc->m_strName,
                    ...
                    if (row->PathPointCount != 0 && row->Path.has_value())
                    {
                        ...
                        {
                            spdlog::error(
                                "LoadNpcPosTable: NPC expects a larger path for this PathPointCount [zoneId={} "
                                "serial={} npcId={} npcName={} moveType={}, pathCount={}]",
                                row->ZoneId,
                                row->PathPointCount,
                                pNpc->m_sNid + NPC_BAND,
                                pNpc->m_sSid,
                        ...
                        for (int l = 0; l < row->PathPointCount; l++)
                        {
                            ...
                            //	TRACE(_T(" l=%d, x=%d, z=%d\n"), l, pNpc->m_PathList.pPattenPos[l].x,
                            //pNpc->m_PathList.pPattenPos[l].z);
                        }
                    }
                    ...
                    {
                        spdlog::error(
                            "AIServerApp::LoadNpcPosTable: NPC invalid zone [npcId:{}, npcZoneId:{}]",
                            pNpc->m_sSid,
                            pNpc->m_sCurZone);
                        return false;
                    }
                    ...
                    {
                        spdlog::error("AIServerApp::LoadNpcPosTable: Npc PutData Fail [serial={}]", pNpc->m_sNid);
                        delete pNpc;
                        pNpc = nullptr;
                    }
                    ...
                    if (pNpc != nullptr && pMap->m_byRoomEvent > 0 && pNpc->m_byDungeonFamily > 0)
                    {
                        ...
                        {
                            spdlog::error(
                                "AIServerApp::LoadNpcPosTable: No RoomEvent for NPC dungeonFamily: serial={}, "
                                "npcId={}, npcName={}, dungeonFamily={}, zoneId={}",
                                pNpc->m_sNid + NPC_BAND,
                                pNpc->m_sSid,
                                pNpc->m_strName,
                                pNpc->m_byDungeonFamily,
                                pNpc->m_ZoneIndex);
                            return false;
                        }
                        ...
                        *pInt = pNpc->m_sNid;
                        if (!pRoom->m_mapRoomNpcArray.PutData(pNpc->m_sNid, pInt))
                        {
                            delete pInt;
                            spdlog::error(
                                "AIServerApp::LoadNpcPosTable: MapRoomNpcArray.PutData failed for NPC: [serial={}, "
                                "npcId={}]",
                                pNpc->m_sNid,
                                pNpc->m_sSid);
                        }
...
bool AIServerApp::MapFileLoad()
{
    ...
    loader.SetProcessFetchCallback([&](db::ModelRecordSet<ModelType> &recordset) {
        do
        {
            ...
            {
                spdlog::error("AIServerApp::MapFileLoad: Failed to load map file: {}", filename);
                delete pMap;
                return;
            }
            ...
            if (row.RoomEvent > 0)
            {
                ...
                {
                    spdlog::error("AIServerApp::MapFileLoad: LoadRoomEvent failed: {}", filename);
                    delete pMap;
                    return;
                }
    ...
    if (!loader.Load_ForbidEmpty())
    {
        spdlog::error("AIServerApp::MapFileLoad: load failed (ZONE_INFO) - {}", loader.GetError().Message);
        return false;
    }
    ...
    for (MAP *pMap : _zones)
    {
        ...
        for (int i = 0; i < size; i++)
        {
            ...
            if (count == NPC_NUM)
            {
                ...
                count = 0;
                send_tot++;
                spdlog::trace("AIServerApp::AllNpcInfo: send_count={}, count={}, zone={}", send_tot, count, nZone);
                memset(send_buff, 0, sizeof(send_buff));
...
void AIServerApp::SendCompressedData(int nZone)
{
    if (_compressedPacketCount <= 0 || _compressedPacketIndex <= 0)
    {
        ...
        _compressedPacketIndex = 0;
        spdlog::error(
            "AIServerApp::SendCompressData: count={}, index={}", _compressedPacketCount, _compressedPacketIndex);
        return;
    }
...
bool AIServerApp::GetMagicType1Data()
{
    ...
    if (!loader.Load_ForbidEmpty())
    {
        spdlog::error("AIServerApp::GetMagicType1Data: load failed - {}", loader.GetError().Message);
        return false;
    }
...
bool AIServerApp::GetMagicType2Data()
{
    ...
    if (!loader.Load_ForbidEmpty())
    {
        spdlog::error("AIServerApp::GetMagicType2Data: load failed - {}", loader.GetError().Message);
        return false;
    }
...
bool AIServerApp::GetMagicType3Data()
{
    ...
    if (!loader.Load_ForbidEmpty())
    {
        spdlog::error("AIServerApp::GetMagicType3Data: load failed - {}", loader.GetError().Message);
        return false;
    }
...
bool AIServerApp::GetMagicType4Data()
{
    ...
    if (!loader.Load_ForbidEmpty())
    {
        spdlog::error("AIServerApp::GetMagicType4Data: load failed - {}", loader.GetError().Message);
        return false;
    }
...
bool AIServerApp::GetMagicType7Data()
{
    ...
    if (!loader.Load_ForbidEmpty())
    {
        spdlog::error("AIServerApp::GetMagicType7Data: load failed - {}", loader.GetError().Message);
        return false;
    }
...
bool AIServerApp::AddObjectEventNpc(_OBJECT_EVENT *pEvent, int zone_number)
{
    ...
    if (pNpcTable == nullptr)
    {
        spdlog::error("AIServerApp::AddObjectEventNpc error: eventId={} zoneId={}", pEvent->sIndex, zone_number);
        return false;
    }
...
bool AIServerApp::ProcessCommandLineArgs(const argparse::ArgumentParser &parser)
{
    ...
    {
        spdlog::error("Supplied EVT directory (--event-dir) doesn't exist or is inaccessible: {}", _overrideEventDir);
        return false;
    }
...
bool AIServerApp::LoadConfig(CIni &iniFile)
{
    ...
    else if (!eventDir.empty())
    {
        ...
        {
            spdlog::error("Configured EVT directory doesn't exist or is inaccessible: {}", eventDir);
            return false;
        }
    }
    // Fallback to the default (don't save this).
...
void AIServerApp::SendSystemMsg(const std::string_view msg, int zone, int type, int who)
{
    ...
    spdlog::info("AIServerApp::SendSystemMsg: zoneId={} type={} who={} msg={}", zone, type, who, msg);
}
