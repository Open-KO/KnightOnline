...
void CMagicProcess::ExecuteType3(
    int magicid, int tid, int data1, int data2, int data3, int moral, int dexpoint, int righthand_damage)
{
...
void CMagicProcess::ExecuteType4(int magicid, int sid, int tid, int data1, int data2, int data3, int moral)
{
    ...
    case 6: // 이동 속도 올리기..
        ...
        // pNpc->m_fOldSpeed_2, pType->bSpeed, pNpc->m_fSpeed_1, pNpc->m_fSpeed_2);
        //			}
        break;
...
int16_t CMagicProcess::AreaAttack(
    int magictype, int magicid, int moral, int data1, int data2, int data3, int dexpoint, int righthand_damage)
{
    ...
    if (radius <= 0)
    {
        spdlog::error("MagicProcess::AreaAttack: Invalid radius [magicId={} radius={}", magicid, radius);
        return 0;
    }
    ...
    {
        spdlog::error(
            "MagicProcess::AreaAttack: No map found: [charId={} zoneIndex={}]",
            m_pSrcUser->m_strUserID,
            m_pSrcUser->m_sZoneIndex);
        return 0;
    }
    ...
    for (int i = 0; i < search_x; i++)
    {
        for (int j = 0; j < search_z; j++)
            AreaAttackDamage(
                magictype, min_x + i, min_z + j, magicid, moral, data1, data2, data3, dexpoint, righthand_damage);
    }
...
void CMagicProcess::AreaAttackDamage(
    int magictype,
    int rx,
    int rz,
    int magicid,
    ...
    int righthand_damage)
{
    ...
    {
        spdlog::error(
            "MagicProcess::AreaAttackDamage: No map found: [charId={} zoneIndex={}]",
            m_pSrcUser->m_strUserID,
            m_pSrcUser->m_sZoneIndex);
        return;
    }
    ...
    if (fRadius <= 0)
    {
        spdlog::error("MagicProcess::AreaAttackDamage: invalid radius [magicId={} radius={}]", magicid, fRadius);
        return;
    }
    ...
    for (int i = 0; i < total_mon; i++)
    {
        ...
        if (pNpc != nullptr && pNpc->m_NpcState != NPC_DEAD)
        {
            ...
            if (magictype == 3)
            {
                ...
                spdlog::trace(
                    "MagicProcess::AreaAttackDamage: magic damage calculated [magicId={} damage={}]", magicid, damage);
                if (damage >= 0)
                {
                    result = pNpc->SetHMagicDamage(damage);
                }
    ...
    /*
    for( Iter = pMap->m_ppRegion[rx][rz].m_RegionNpcArray.m_UserTypeMap.begin(); Iter !=
    pMap->m_ppRegion[rx][rz].m_RegionNpcArray.m_UserTypeMap.end(); ) { if( bDead ) { Iter =
    pMap->m_ppRegion[rx][rz].m_RegionNpcArray.DeleteData( Iter ); continue;
        }
        Iter++;
    }	*/
}
