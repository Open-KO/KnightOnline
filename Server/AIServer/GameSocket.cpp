...
void CGameSocket::RecvServerConnect(char *pBuf)
{
    ...
    if (byReConnect == 1)
    {
        ...
        spdlog::info(
            "GameSocket::RecvServerConnect: Ebenezer reconnect: zone={} sockets={}",
            byZoneNumber,
            m_pMain->_reconnectSocketCount);
        ...
        {
            spdlog::info(
                "GameSocket::RecvServerConnect: Ebenezer sockets reconnected in under 2 minutes [sockets={}]",
                m_pMain->_reconnectSocketCount);
        ...
        if (m_pMain->_reconnectSocketCount == MAX_AI_SOCKET)
        {
            ...
            {
                spdlog::info(
                    "GameSocket::RecvServerConnect: Ebenezer sockets reconnected within a minute [sockets={}]",
                    m_pMain->_reconnectSocketCount);
    ...
    else
    {
        ...
        spdlog::info(
            "GameSocket::RecvServerConnect: Ebenezer connected [zone={}, sockets={}]",
            byZoneNumber,
            m_pMain->_socketCount);
        if (m_pMain->_socketCount == MAX_AI_SOCKET)
        {
            spdlog::info(
                "GameSocket::RecvServerConnect: Ebenezer sockets all connected [sockets={}]", m_pMain->_socketCount);
            m_pMain->_firstServerFlag = true;
...
void CGameSocket::RecvUserInfo(char *pBuf)
{
    ...
    spdlog::get(logger::AIServerUser)->info("Login: level={}, charId={}", pUser->m_byLevel, pUser->m_strUserID);
}
...
void CGameSocket::RecvUserInOut(char *pBuf)
{
    ...
    if (pUser != nullptr)
    {
        ...
        {
            spdlog::error(
                "GameSocket::RecvUserInOut: Character position out of bounds [charId={} x1=%d, z1=%d]",
                pUser->m_strUserID,
                x1,
                z1);
            return;
        ...
        if (bType == 2)
        {
            ...
            // TRACE(_T("^^& RecvUserInOut()-> User(%hs, %d)를 Region에서 삭제..,, zone=%d, index=%d, region_x=%d,
            // y=%d\n"), pUser->m_strUserID, pUser->m_iUserId, pUser->m_curZone, pUser->m_sZoneIndex, region_x,
            // region_z);
        }
        // region in
        else
        {
            if (pUser->m_sRegionX != region_x || pUser->m_sRegionZ != region_z)
            {
                ...
                // TRACE(_T("^^& RecvUserInOut()-> User(%hs, %d)를 Region에 등록,, zone=%d, index=%d, region_x=%d,
                // y=%d\n"), pUser->m_strUserID, pUser->m_iUserId, pUser->m_curZone, pUser->m_sZoneIndex, region_x,
                // region_z);
            }
        }
...
bool CGameSocket::SetUid(float x, float z, int id, int speed)
{
    ...
    {
        spdlog::error(
            "GameSocket::SetUid: map not found [charId=%hs zoneIndex={}]", pUser->m_strUserID, pUser->m_sZoneIndex);
        return false;
    }
    ...
    {
        spdlog::error(
            "GameSocket::SetUid: character position out of bounds [userId=%d, charId=%hs x1=%d z1=%d]",
            id,
            pUser->m_strUserID,
            x1,
            z1);
    ...
    {
        spdlog::error(
            "GameSocket::SetUid: region bounds exceeded [userId={} charId={} nRX={} nRZ={}]",
            id,
            pUser->m_strUserID,
            nRX,
            nRZ);
    ...
    if (pUser != nullptr)
    {
        if (pUser->m_bLive == USER_DEAD || pUser->m_sHP <= 0)
        {
            if (pUser->m_sHP > 0)
            {
                pUser->m_bLive = USER_LIVE;
                spdlog::debug(
                    "GameSocket::SetUid: user healed [charId={} isAlive={} hp={}]",
                    pUser->m_strUserID,
                    pUser->m_bLive,
                    pUser->m_sHP);
            }
            ...
            {
                spdlog::error(
                    "GameSocket::SetUid: user is dead [charId={} isAive={} hp={}]",
                    pUser->m_strUserID,
                    pUser->m_bLive,
                    pUser->m_sHP);
                // Send_UserError(id);
        ...
        if (pUser->m_sRegionX != nRX || pUser->m_sRegionZ != nRZ)
        {
            ...
            // pUser->m_strUserID, pUser->m_iUserId, pUser->m_curZone, pUser->m_sZoneIndex, pUser->m_sRegionX,
            // pUser->m_sRegionZ);
...
void CGameSocket::RecvUserLogOut(char *pBuf)
{
    ...
    {
        spdlog::warn(
            "GameSocket::RecvUserLogOut: character name length out of bounds [userId={} charId={} len={}]",
            uid,
            strName,
            len);
        // return;
    ...
    // UserLogFile write
    spdlog::get(logger::AIServerUser)->info("Logout: level={}, charId={}", pUser->m_byLevel, pUser->m_strUserID);
...
void CGameSocket::RecvUserRegene(char *pBuf)
{
    ...
    spdlog::debug(
        "GameSocket::RecvUserRegene: processed [userId={} charId={} hp={}]",
        pUser->m_strUserID,
        pUser->m_iUserId,
        pUser->m_sHP);
    // TRACE(_T("**** RecvUserRegene -- uid = (%hs,%d), HP = %d\n"), pUser->m_strUserID, pUser->m_iUserId,
    // pUser->m_sHP);
}
...
void CGameSocket::RecvUserUpdate(char *pBuf)
{
    ...
    if (pUser->m_byLevel < byLevel) // level up
    {
        ...
        spdlog::get(logger::AIServerUser)->info("LevelUp: level={}, charId={}", byLevel, pUser->m_strUserID);
    }
    ...
    //
    // TCHAR buff[256] = {};
    // wsprintf(buff, _T("**** RecvUserUpdate -- uid = (%hs,%d), HP = %d, level=%d->%d"), pUser->m_strUserID,
    // pUser->m_iUserId, pUser->m_sHP, byLevel, pUser->m_sLevel); TimeTrace(buff); TRACE(_T("**** RecvUserUpdate -- uid
...
void CGameSocket::RecvZoneChange(char *pBuf)
{
    ...
    spdlog::trace(
        "GameSocket::RecvZoneChange: [charId={} userId={} zoneId={}]",
        pUser->m_strUserID,
        pUser->m_iUserId,
        byZoneNumber);
}
...
void CGameSocket::RecvUserInfoAllData(char *pBuf)
{
    ...
    for (int i = 0; i < byCount; i++)
    {
        ...
        {
            spdlog::error(
                "GameSocket::RecvUserInfoAllData: character name length is out of bounds [userId={} charId={} len={}]",
                uid,
                strName,
                len);
            continue;
