...
constexpr int MAX_ITEM_GRADECODE_GRADE =
    sizeof(model::MakeItemGradeCode::Grade) / sizeof(model::MakeItemGradeCode::Grade[0]);
...
inline bool CNpc::SetUid(float x, float z, int id)
{
    ...
    if (pMap == nullptr)
    {
        spdlog::error("Npc::SetUid: map not found [zoneIndex={} npcId={} npcName={}]", m_ZoneIndex, m_sSid, m_strName);
        return false;
    }
    ...
    {
        spdlog::error(
            "Npc::SetUid: out of map bounds [serial={} npcId={} x={} z={}]", m_sNid + NPC_BAND, m_sSid, x1, z1);
        return false;
    }
    ...
    {
        spdlog::error(
            "Npc::SetUid: out of region bounds [serial={} npcId={} x={} z={}]", m_sNid + NPC_BAND, m_sSid, nRX, nRZ);
        return false;
    }
...
void CNpc::Init()
{
    ...
    if (pMap == nullptr)
    {
        spdlog::error("Npc::Init: map not found [zoneIndex={} npcId={} npcName={}]", m_strName, m_sSid, m_ZoneIndex);
        return;
    }
...
void CNpc::NpcTracing()
{
    ...
    if (m_sStepCount != 0)
    {
        ...
        {
            spdlog::error(
                "Npc::NpcTracing: previous coordinates invalid [serial={} npcName={} prevX={} prevZ={}]",
                m_sNid + NPC_BAND,
                m_strName,
                m_fPrevX,
                m_fPrevZ);
    ...
    */
    ...
    if (m_byResetFlag == 1)
    {
        ...
        {
            spdlog::error(
                "Npc::NpcTracing: pathfinding failed, set state to NPC_STANDING [serial={} npcId={} npcName={}]",
                m_sNid + NPC_BAND,
                m_sSid,
                m_strName);
            InitTarget();
    ...
    if (!m_bPathFlag)
    {
        ...
        if (!StepMove(1))
        {
            ...
            m_fDelayTime = TimeGet();
            spdlog::error(
                "Npc::NpcTracing: StepMove failed [serial={} npcId={} npcName={}]",
                m_sNid + NPC_BAND,
                m_sSid,
                m_strName);
            return;
    ...
    else // if (m_bPathFlag == true)
    {
        ...
        if (!StepNoPathMove(1)) // 한칸 움직임(걷는동작, 달릴때는 2칸)
        {
            ...
            m_fDelayTime = TimeGet();
            spdlog::error(
                "Npc::NpcTracing: StepNoPathMove failed [serial={} npcId={} npcName={}]",
                m_sNid + NPC_BAND,
                m_sSid,
                m_strName);
            return;
    ...
    if (IsMovingEnd())
    {
        ...
        // TRACE(_T("Npc TRACE end --> nid = %d, cur=[x=%.2f, y=%.2f, metor=%d], prev=[x=%.2f, z=%.2f], frame=%d, speed
    ...
    else
    {
        ...
        // TRACE(_T("Npc Tracing --> nid = %d, cur=[x=%.2f, z=%.2f], prev=[x=%.2f, z=%.2f, metor = %.2f], frame=%d,
        // speed = %d \n"), m_sNid, m_fCurX, m_fCurZ, m_fPrevX, m_fPrevZ, m_fSecForRealMoveMetor, m_sStepCount,
        // m_sSpeed);
        SendAll(pBuf, index); // thread 에서 send
    }
...
void CNpc::NpcAttacking()
{
    ...
    // if(m_tNpcType == NPCTYPE_DOOR || m_tNpcType == NPCTYPE_ARTIFACT || m_tNpcType == NPCTYPE_PHOENIX_GATE ||
    // m_tNpcType == NPCTYPE_GATE_LEVER)		return;		// 성문 NPC는 공격처리 안하게
...
void CNpc::NpcMoving()
{
    ...
    if (m_sStepCount != 0)
    {
        ...
        {
            spdlog::error(
                "Npc::NpcMoving: previous coordinates invalid [serial={} npcName={} prevX={} prevZ={}]",
                m_sNid + NPC_BAND,
                m_strName,
                m_fPrevX,
                m_fPrevZ);
    ...
    if (IsMovingEnd())
    {
        ...
        SetFloat(pBuf, m_fPrevY, index);
        SetFloat(pBuf, 0, index);
        // TRACE(_T("Npc Move end --> nid = %d, cur=[x=%.2f, y=%.2f, metor=%d], prev=[x=%.2f, z=%.2f], frame=%d, speed =
    ...
    else
    {
        ...
        SetFloat(pBuf, fMoveSpeed, index);
        // SetFloat(pBuf, m_fSecForRealMoveMetor, index);
        // TRACE(_T("Npc Move --> nid = %d, cur=[x=%.2f, z=%.2f], prev=[x=%.2f, z=%.2f, metor = %.2f], frame=%d, speed =
        // %d \n"), m_sNid+NPC_BAND, m_fCurX, m_fCurZ, m_fPrevX, m_fPrevZ, m_fSecForRealMoveMetor, m_sStepCount,
        // m_sSpeed);
        SendAll(pBuf, index); // thread 에서 send
    }
...
void CNpc::NpcStanding()
{
    ...
    {
        spdlog::error(
            "Npc::NpcStanding: map not found [zoneIndex={} npcId={} npcName={}]", m_strName, m_sSid, m_ZoneIndex);
        return;
    }
...
void CNpc::NpcBack()
{
    ...
    if (m_sStepCount != 0)
    {
        ...
        {
            spdlog::error(
                "Npc::NpcBack: previous coordinates invalid [serial={} npcName={} prevX={} prevZ={}]",
                m_sNid + NPC_BAND,
                m_strName,
                m_fPrevX,
                m_fPrevZ);
    ...
    if (IsMovingEnd())
    {
        ...
        SetFloat(pBuf, m_fCurY, index);
        SetFloat(pBuf, 0, index);
        //		TRACE(_T("NpcBack end --> nid = %d, cur=[x=%.2f, y=%.2f, metor=%d], prev=[x=%.2f, z=%.2f], frame=%d,
        //speed = %d \n"), m_sNid, m_fCurX, m_fCurZ, 0, m_fPrevX, m_fPrevZ, m_sStepCount, 0);
        SendAll(pBuf, index); // thread 에서 send
...
bool CNpc::SetLive()
{
    ...
    if (pMap == nullptr)
    {
        spdlog::error("Npc::SetLive: map not found [zoneIndex={} npcId={} npcName={}]", m_strName, m_sSid, m_ZoneIndex);
        return false;
    }
    ...
    {
        spdlog::error(
            "Npc::SetLive: coordinates invalid [serial={} npcId={} npcName={} x={} z={}]",
            m_sNid + NPC_BAND,
            m_sSid,
            m_strName,
            m_fCurX,
    ...
    else
    {
        ...
        while (1)
        {
            ...
            {
                spdlog::error(
                    "Npc::SetLive: tile coordinates invalid [serial={} npcId={} npcName={} tileX={} tileZ={}]",
                    m_sNid + NPC_BAND,
                    m_sSid,
                    m_strName,
                    nTileX,
            ...
            if (pMap->m_pMap[nTileX][nTileZ].m_sEvent <= 0)
            {
                if (retryCount >= maxRetry)
                {
                    ...
                    m_nInitZ = m_fPrevZ = m_fCurZ;
                    spdlog::error(
                        "Npc::SetLive: failed to spawn NPC, max retries exceeded [npcId={} serial={} zoneId={} "
                        "retryCount={} x={} z={}]",
                        m_sSid,
                        m_sNid + NPC_BAND,
                        m_sCurZone,
            ...
            {
                spdlog::error(
                    "Npc::SetLive: post-move coordinates invalid [serial={} npcId={} npcName={} x={} z={}]",
                    m_sNid + NPC_BAND,
                    m_sSid,
                    m_strName,
                    m_fCurX,
    ...
    m_byDeadType = 0;
    spdlog::trace(
        "Npc::SetLive: NPC initialized [serial={} npcId={} threadNumber={} npcName={} x={} z={} gateOpen={} "
        "deadType={}]",
        m_sNid + NPC_BAND,
        m_sSid,
        m_sThreadNumber,
...
bool CNpc::RandomMove()
{
    ...
    else if (m_byMoveType == 2)
    {
        ...
        if (m_sPathCount != 0 && !IsInPathRange())
        {
            ...
            {
                spdlog::debug(
                    "Npc::RandomMove: unreachable path point, returning to beginning of path. [serial={} npcId={} "
        ...
        // TRACE(_T("RandomMove 길따라 이동 : [nid=%d, sid=%d, name=%hs], path=%d/%d, nPathCount=%d, curx=%.2f, z=%.2f
        // -> dest_X=%.2f, z=%.2f\n"), m_sNid+NPC_BAND, m_sSid, m_strName, m_sPathCount, m_sMaxPathCount, nPathCount,
        // m_fCurX, m_fCurZ, fDestX, fDestZ);
        m_sPathCount++;
    }
    // PathLine을 따라서 움직이는 NPC
    ...
    {
        spdlog::error(
            "Npc::RandomMove: coordinates invalid [serial={} npcName={} x={} z={} destX={} destZ={} mapBounds=[x:{} "
            "z:{}]]",
            m_sNid + NPC_BAND,
            m_strName,
            m_fCurX,
    ...
    {
        spdlog::error(
            "Npc::RandomMove: destination coordinates invalid [serial={} npcName={} x={} z={} destX={} destZ={} "
            "mapBounds=[x:{} z:{}]]",
            m_sNid + NPC_BAND,
            m_strName,
            m_fCurX,
    ...
    if (fDis > NPC_MAX_MOVE_RANGE)
    {
        ...
        spdlog::error(
            "Npc::RandomMove: tried to move further than max move distance [serial={} npcId={} npcName={} distance={}]",
            m_sNid + NPC_BAND,
            m_sSid,
            m_strName,
            fDis);
...
bool CNpc::RandomBackMove()
{
    ...
    {
        spdlog::error(
            "Npc::RandomBackMove: map not found [zoneIndex={} npcId={} npcName={}]", m_ZoneIndex, m_sSid, m_strName);
        return false;
    }
...
int CNpc::GetNearPathPoint()
{
    ...
    if (m_sPathCount + 1 >= m_sMaxPathCount)
    {
        ...
        {
            vEnd.Set(
                (float)m_PathList.pPattenPos[m_sPathCount - 1].x + m_fBattlePos_x,
                0,
        ...
        {
            vEnd.Set(
                (float)m_PathList.pPattenPos[0].x + m_fBattlePos_x,
                0,
...
void CNpc::Dead(int iDeadType)
{
    ...
    if (pMap == nullptr)
    {
        spdlog::error("Npc::Dead: map not found [zoneIndex={} npcId={} npcName={}]", m_strName, m_sSid, m_ZoneIndex);
        return;
    }
    ...
    {
        spdlog::error(
            "Npc::Dead: out of region bounds [serial={} npcId={} x={} z={}]",
            m_sNid + NPC_BAND,
            m_sSid,
            m_iRegion_X,
            m_iRegion_Z);
    ...
    // m_iRegion_X, m_iRegion_Z); TRACE(_T("****** (%hs,%d) Dead regentime = %d , m_byDeadType=%d, dungeonfam=%d,
    ...
    if (m_bySpecialType == 1 || m_bySpecialType == 4)
    {
        ...
        else if (m_byChangeType == 2)
        {
            ...
            {
                spdlog::error(
                    "Npc::Dead: dungeonFamily out of range [serial={} npcId={} npcName={} dungeonFamily={}]",
                    m_sNid + NPC_BAND,
                    m_sSid,
                    m_strName,
                    m_byDungeonFamily);
    ...
    /*
       ...
       m_sNid+NPC_BAND, m_strName, m_byDungeonFamily); return;
        }
...
bool CNpc::FindEnemy()
{
    ...
    {
        spdlog::error(
            "Npc::FindEnemy: map not found [zoneIndex={} npcId={} npcName={}]", m_strName, m_sSid, m_ZoneIndex);
        return false;
    }
    ...
    if (m_iRegion_X > pMap->GetXRegionMax() || m_iRegion_Z > pMap->GetZRegionMax() || m_iRegion_X < 0 ||
        m_iRegion_Z < 0)
    {
        // TRACE(_T("#### Npc-FindEnemy() Fail : [nid=%d, sid=%d, name=%hs, th_num=%d, cur_x=%.2f, cur_z=%.2f], nRX=%d,
        // nRZ=%d #####\n"), m_sNid+NPC_BAND, m_sSid, m_strName, m_sThreadNumber, m_fCurX, m_fCurZ, m_iRegion_X,
        // m_iRegion_Z);
        return false;
    }
...
float CNpc::FindEnemyExpand(int nRX, int nRZ, float fCompDis, int nType)
{
    ...
    {
        spdlog::error(
            "Npc::FindEnemyExpand: map not found [zoneIndex={} npcId={} npcName={}]", m_strName, m_sSid, m_ZoneIndex);
        return 0.0f;
    }
...
bool CNpc::IsMovable(float x, float z)
{
    ...
    {
        spdlog::error(
            "Npc::IsMovable: map not found [zoneIndex={} npcId={} npcName={}]", m_strName, m_sSid, m_ZoneIndex);
        return false;
    }
...
bool CNpc::StepMove(int /*nStep*/)
{
    ...
    if (m_pPoint[m_iAniFrameCount].fXPos < 0 || m_pPoint[m_iAniFrameCount].fZPos < 0)
    {
        ...
        spdlog::error(
            "Npc::StepMove: aniFrameCount out of bounds [serial={} npcId={} npcName={} frameCount={} frameIndex={}]",
            m_sNid + NPC_BAND,
            m_sSid,
            m_strName,
            m_iAniFrameCount,
...
bool CNpc::StepNoPathMove(int /*nStep*/)
{
    ...
    {
        spdlog::error(
            "Npc::StepNoPathMove: stepCount out of bounds [serial={} npcId={} npcName={} stepCount={} frameIndex={}]",
            m_sNid + NPC_BAND,
            m_sSid,
            m_strName,
            m_sStepCount,
    ...
    {
        spdlog::error(
            "Npc::StepNoPathMove: previous coordinates invalid [serial={} npcId={} npcName={} prevX={} prevZ={}]",
            m_sNid + NPC_BAND,
            m_sSid,
            m_strName,
            m_fPrevX,
    ...
    if (++m_sStepCount > 1)
    {
        ...
        {
            spdlog::error(
                "Npc::StepNoPathMove: old previous coordinates invalid [serial={} npcId={} npcName={} oldCurX={} "
                "oldCurZ={}]",
                m_sNid + NPC_BAND,
                m_sSid,
                m_strName,
...
int CNpc::GetTargetPath(int option)
{
    ...
    {
        spdlog::error(
            "Npc::GetTargetPath: map not found [zoneIndex={} npcId={} npcName={}]", m_strName, m_sSid, m_ZoneIndex);
        return -1;
    }
    ...
    if (targetUser != nullptr)
    {
        ...
        _POINT pt = {
            static_cast<int>(targetUser->m_curx / TILE_SIZE), static_cast<int>(targetUser->m_curz / TILE_SIZE)};

        if (!IsPointInRect(pt, r))
        {
            spdlog::debug(
                "Npc::GetTargetPath: user outside of search range [serial={} npcId={} npcName={} charId={} "
                "attackPos={}]",
                m_sNid + NPC_BAND,
                m_sSid,
                m_strName,
                targetUser->m_strUserID,
                m_byAttackPos);
            return -1;
        }
        ...
        if (m_byAttackPos > 0 && m_byAttackPos < 9)
        {
            ...
            // m_fEndPoint_X = vUser.x + surround_fx[m_byAttackPos-1];	m_fEndPoint_Y = vUser.z +
            // surround_fz[m_byAttackPos-1];
    ...
    else if (npcTarget != nullptr)
    {
        ...
        if (!IsPointInRect(
                _POINT(
                    static_cast<int>(npcTarget->m_fCurX / TILE_SIZE), static_cast<int>(npcTarget->m_fCurZ / TILE_SIZE)),
                r))
        {
            spdlog::debug(
                "Npc::GetTargetPath: target outside of search range [serial={} npcId={} npcName={} targetSerial={} "
                "targetId={} targetName={} attackPos={}]",
                m_sNid + NPC_BAND,
                m_sSid,
                m_strName,
                npcTarget->m_sNid + NPC_BAND,
                npcTarget->m_sSid,
                npcTarget->m_strName,
                m_byAttackPos);
            return -1;
    ...
    {
        spdlog::debug(
            "Npc::GetTargetPath: target outside of chase range [serial={} npcId={} npcName={} destDist={} "
            "chaseRange={}]",
            m_sNid + NPC_BAND,
            m_sSid,
            m_strName,
...
int CNpc::GetFinalDamage(CUser *pUser, int /*type*/)
{
    ...
    {
        spdlog::trace(
            "Npc::GetFinalDamage: damage exceeded maximum, clamped to max. [serial={} npcId={} npcName={} damage={} "
            "maxDamage={}]",
            m_sNid + NPC_BAND,
            m_sSid,
            m_strName,
...
bool CNpc::SetDamage(int nAttackType, int nDamage, const char *sourceName, int uid)
{
    ...
    {
        spdlog::error(
            "Npc::SetDamage: map not found [zoneIndex={} npcId={} npcName={}]", m_strName, m_sSid, m_ZoneIndex);
        return true;
    }
    ...
    for (i = 0; i < NPC_HAVE_USER_LIST; i++)
    {
        if (m_DamagedUserList[i].iUid == -1)
        {
            if (m_DamagedUserList[i].nDamage <= 0)
            {
                ...
                {
                    spdlog::error(
                        "Npc::SetDamage: sourceName length out of bounds [serial={} npcId={} npcName={} len={} "
                        "sourceName={}]",
                        m_sNid + NPC_BAND,
                        m_sSid,
                        m_strName,
...
bool CNpc::SetHMagicDamage(int nDamage)
{
    ...
    spdlog::trace(
        "Npc::SetHMagicDamage: [serial={} npcId={} npcName={} oldHp={} newHp={}]",
        m_sNid + NPC_BAND,
        m_sSid,
        m_strName,
        oldHP,
...
void CNpc::SendExpToUserList()
{
    ...
    {
        spdlog::error(
            "Npc::SendExpToUserList: map not found [zoneIndex={} npcId={} npcName={}]", m_strName, m_sSid, m_ZoneIndex);
        return;
    }
    ...
    for (i = 0; i < NPC_HAVE_USER_LIST; i++)
    {
                ...
                if (bFlag)
                {
                    ...
                    if (pParty != nullptr)
                    {
                        ...
                        for (int k = 0; k < 8; k++)
                        {
                            ...
                            TempValue = (nPartyExp * (1 + 0.3 * (nTotalMan - 1))) * (double)pPartyUser->m_byLevel /
                                        (double)nTotalLevel;
                            ...
                            // *\n"), pPartyUser->m_strUserID, (int)totalDamage, m_TotalDamage, nExp, nLoyalty,
                            // pPartyUser->m_sLevel, nTotalLevel); pPartyUser->SetExp(nExp, nLoyalty, m_sLevel);
            ...
            else if (i == 0)
            {
                ...
                if (pParty != nullptr)
                {
                    ...
                    for (int k = 0; k < 8; k++)
                    {
                        ...
                        {
                            TempValue = (nPartyLoyalty * (1 + 0.2 * (nTotalMan - 1))) * (double)pPartyUser->m_byLevel /
                                        (double)nTotalLevel;
                            nLoyalty = (int)TempValue;
                            if (TempValue > nLoyalty)
                                ++nLoyalty;
        ...
        else
        {
            ...
            else
            {
                ...
                // (int)totalDamage, m_TotalDamage, nExp, nLoyalty);
...
int CNpc::FindFriend(int type)
{
    ...
    {
        spdlog::error(
            "Npc::FindFriend: map not found [zoneIndex={} npcId={} npcName={}]", m_strName, m_sSid, m_ZoneIndex);
        return 0;
    }
...
// void CNpc::FindFriendRegion(int x, int z, MAP* pMap, int type)
{
    ...
    {
        spdlog::error(
            "Npc::FindFriendRegion: out of region bounds [serial={} npcId={} x={} z={}]",
            m_sNid + NPC_BAND,
            m_sSid,
            x,
            z);
...
void CNpc::NpcTrace(std::string_view msg)
{
    ...
    {
        spdlog::trace(
            "NPCTrace: {} [serial={} npcId={} npcName={} x={} z={}]",
            msg,
            m_sNid + NPC_BAND,
            m_sSid,
            m_strName,
...
bool CNpc::GetUserInView()
{
    ...
    {
        spdlog::error(
            "Npc::GetUserInView: map not found [zoneIndex={} npcId={} npcName={}]", m_strName, m_sSid, m_ZoneIndex);
        return false;
    }
...
bool CNpc::GetUserInViewRange(int x, int z)
{
    ...
    {
        spdlog::error(
            "Npc::GetUserInViewRange: map not found [zoneIndex={} npcId={} npcName={}]",
            m_strName,
            m_sSid,
            m_ZoneIndex);
        return false;
    ...
    {
        spdlog::error(
            "Npc::GetUserInViewRange: out of map bounds [serial={} npcId={} x={} z={}]",
            m_sNid + NPC_BAND,
            m_sSid,
            x,
            z);
...
void CNpc::SendAttackSuccess(uint8_t byResult, int tuid, int16_t sDamage, int nHP, uint8_t byFlag, uint8_t byAttackType)
{
    int send_index = 0;
    int sid = -1, tid = -1;
    uint8_t type;
    ...
    // sDamage, sHP); SetShort( buff, sMaxHP, send_index );
...
bool CNpc::IsPathFindCheck(float fDistance)
{
    ...
    {
        spdlog::error(
            "Npc::IsPathFindCheck: map not found [zoneIndex={} npcId={} npcName={}]", m_strName, m_sSid, m_ZoneIndex);
        return false;
    }
...
void CNpc::IsNoPathFind(float fDistance)
{
    ...
    if (fDis > NPC_MAX_MOVE_RANGE)
    {
        ClearPathFindData();
        spdlog::error(
            "Npc::IsNoPathFind: tried to move further than max move distance [serial={} npcId={} npcName={} "
            "distance={}]",
            m_sNid + NPC_BAND,
            m_sSid,
            m_strName,
    ...
    if (pMap == nullptr)
    {
        ClearPathFindData();
        spdlog::error(
            "Npc::IsNoPathFind: map not found [zoneIndex={} npcId={} npcName={}]", m_strName, m_sSid, m_ZoneIndex);
        return;
    }
...
void CNpc::GiveNpcHaveItem()
{
        ...
        for (int j = 1; j < m_pMain->_npcItem.m_nField; j += 2)
        {
            ...
            if (iRandom <= iPer)
            {
                ...
                else
                {
                    ...
                    // 화살이라면
                    if (COMPARE(GiveItemList[nCount].sSid, ARROW_MIN, ARROW_MAX))
    ...
    for (i = 0; i < nCount; i++)
    {
        ...
        if (GiveItemList[i].sSid != TYPE_MONEY_SID)
            spdlog::get(logger::AIServerItem)->info(GiveItemList[i].sSid);
...
bool CNpc::CheckFindEnemy()
{
    ...
    {
        spdlog::error(
            "Npc::CheckFindEnemy: map not found [zoneIndex={} npcId={} npcName={}]", m_strName, m_sSid, m_ZoneIndex);
        return false;
    }
    ...
    {
        spdlog::error(
            "Npc::CheckFindEnemy: out of region bounds [serial={} npcId={} x={} z={}]",
            m_sNid + NPC_BAND,
            m_sSid,
            m_iRegion_X,
            m_iRegion_Z);
...
int CNpc::ItemProdution(int item_number) const
{
    ...
    if (COMPARE(iRandom, 1, 4001))
    {
        ...
        // TRACE(_T("ItemProdution : Weapon Success item_number = %d, default=%d, itemcode=%d, iRand2=%d, iRand3=%d,
        // iRand4=%d, iRand5, iItemGrade=%d\n"), iItemNumber, iDefault, iItemCode, iRand2, iRand3, iRand4, iRand5,
        // iItemGrade);
    }
    // 방어구 아이템
    else if (COMPARE(iRandom, 4001, 8001))
    {
        ...
        // TRACE(_T("ItemProdution : Defensive Success item_number = %d, default=%d, iRand2=%d, itemcode=%d, iRand3=%d,
    ...
    else if (COMPARE(iRandom, 8001, 10001))
    {
        ...
        // TRACE(_T("ItemProdution : Accessary Success item_number = %d, default=%d, iRand2=%d, iRand3=%d, iRand4=%d,
        // iItemGrade=%d\n"), iItemNumber, iDefault, iRand2, iRand3, iRand4, iItemGrade);
    }
...
void CNpc::DurationMagic_4(double currentTime)
{
    ...
    {
        spdlog::error(
            "Npc::DurationMagic_4: map not found [zoneIndex={} npcId={} npcName={}]", m_strName, m_sSid, m_ZoneIndex);
        return;
    }
    ...
    if (m_byDungeonFamily > 0)
    {
        ...
        {
            spdlog::error(
                "Npc::DurationMagic_4: dungeonFamily out of range [serial={} npcId={} npcName={} dungeonFamily={}]",
                m_sNid + NPC_BAND,
                m_sSid,
                m_strName,
                m_byDungeonFamily);
        ...
        else
        {
            ...
            {
                spdlog::error(
                    "Npc::DurationMagic_4: RoomEvent not found for dungeonFamily [serial={} npcId={} npcName={} "
                    "dungeonFamily={}]",
                    m_sNid + NPC_BAND,
                    m_sSid,
                    m_strName,
...
void CNpc::ChangeMonsterInfo(int iChangeType)
{
    ...
    if (m_byInitMoveType >= 0 && m_byInitMoveType < 100)
    {
        ...
        {
            spdlog::error(
                "Npc::ChangeMonsterInfo: changeNpcId not found [serial={} npcId={} npcName={} changeNpcId={}]",
                m_sNid + NPC_BAND,
                m_sSid,
                m_strName,
                m_sChangeSid);
...
void CNpc::DurationMagic_3(double currentTime)
{
    ...
    for (int i = 0; i < MAX_MAGIC_TYPE3; i++)
    {
        ...
        // m_MagicType3[i].fStartTime, m_sNid+NPC_BAND, m_MagicType3[i].sHPAmount);
        ...
        else
        {
            ...
            if (!SetDamage(0, duration_damage, "**duration**", m_MagicType3[i].sHPAttackUserID))
            {
                ...
                SendDead();
                SendAttackSuccess(
                    MAGIC_ATTACK_TARGET_DEAD,
                    m_MagicType3[i].sHPAttackUserID,
                    duration_damage,
                    m_iHP,
                    1,
                    DURATION_ATTACK);
                // TRACE(_T("&&&& Duration Magic attack .. pNpc->m_byHPInterval[%d] = %d &&&& \n"), i,
                // m_MagicType3[i].byHPInterval);
                m_MagicType3[i].fStartTime = 0.0;
                m_MagicType3[i].byHPDuration = 0;
                m_MagicType3[i].byHPInterval = 2;
                m_MagicType3[i].sHPAmount = 0;
            ...
            {
                SendAttackSuccess(
                    ATTACK_SUCCESS, m_MagicType3[i].sHPAttackUserID, duration_damage, m_iHP, 1, DURATION_ATTACK);
                // TRACE(_T("&&&& Duration Magic attack .. pNpc->m_byHPInterval[%d] = %d &&&& \n"), i,
                // m_MagicType3[i].byHPInterval);
            }
        }
...
void CNpc::ChangeAbility(int iChangeType)
{
    ...
    {
        spdlog::error(
            "Npc::ChangeAbility: invalid initMoveType [serial={} npcId={} npcName={} initMoveType={}]",
            m_sNid + NPC_BAND,
            m_sSid,
            m_strName,
            m_byInitMoveType);
    ...
    else if (m_byInitMoveType >= 100)
    {
        ...
        {
            spdlog::error(
                "Npc::ChangeAbility: invalid npcId [serial={} npcId={} npcName={}]",
                m_sNid + NPC_BAND,
                m_sSid,
                m_strName);
            return;
    ...
    if (iChangeType == BATTLEZONE_OPEN)
    {
        ...
        // pNpcTable->m_iMaxHP, nHP, pNpcTable->m_sDamage, nDamage);
    }
    // 능력치 회복
    else if (iChangeType == BATTLEZONE_CLOSE)
    {
        ...
        // m_iHP, m_iMaxHP, pNpcTable->m_sDamage, nDamage);
...
bool CNpc::Teleport()
{
    ...
    while (1)
    {
        ...
        {
            spdlog::error(
                "Npc::Teleport: tile coordinates invalid [serial={} npcId={} npcName={} tileX={} tileZ={}]",
                m_sNid + NPC_BAND,
                m_sSid,
                m_strName,
                nTileX,
        ...
        if (pMap->m_pMap[nTileX][nTileZ].m_sEvent <= 0)
        {
            ...
            {
                spdlog::error(
                    "Npc::Teleport: max retries exceeded [npcId={} serial={} zoneId={} retryCount={} x={} z={}]",
                    m_sSid,
                    m_sNid + NPC_BAND,
                    m_sCurZone,
                    retryCount,
