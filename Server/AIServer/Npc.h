...
class CNpc
{
  public:
    ...
    uint8_t
        m_bySpecialType; // 몬스터의 형태가 변하는지를 판단(0:변하지 않음, 1:변하는 몬스터,
  ...
  public:
    ...
    bool SetDamage(int nAttackType, int nDamage, const char *sourceName, int uid); // Npc의 데미지 계산..
    ...
    void SendAttackSuccess(
        uint8_t byResult, int tuid, int16_t sDamage, int nHP = 0, uint8_t byFlag = 0, uint8_t byAttackType = 1);
