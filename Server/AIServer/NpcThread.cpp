...
void CNpcThread::thread_loop()
{
    ...
    while (_canTick)
    {
        ...
        for (int i = 0; i < NPC_NUM; i++)
        {
            ...
            // if((pNpc->m_tNpcType == NPCTYPE_DOOR || pNpc->m_tNpcType == NPCTYPE_ARTIFACT || pNpc->m_tNpcType ==
            // NPCTYPE_PHOENIX_GATE || pNpc->m_tNpcType == NPCTYPE_GATE_LEVER) && !pNpc->m_bFirstLive) continue; if(
            // pNpc->m_bFirstLive ) continue;
            ...
            // if(i==0)
            // TRACE(_T("thread time = %.2f, %.2f, %.2f, delay=%d, state=%d, nid=%d\n"), pNpc->m_fDelayTime, fTime2,
            // fTime3, dwTickTime, pNpc->m_NpcState, pNpc->m_sNid+NPC_BAND);
