...
bool CRoomEvent::CheckEvent(int event_num, double currentTime)
{
    ...
    case 1:
        ...
        {
            spdlog::error(
                "RoomEvent::CheckEvent: missing NPC definition [npcId={} logicNumber={}]", nOption_1, m_byLogicNumber);
        }
    ...
    case 3:
        ...
        {
            spdlog::debug(
                "RoomEvent::CheckEvent: Time limit met, survival success [currTime={} delayTime={}]",
                currentTime,
                m_fDelayTime);
            return true;
        }
    ...
    case 5:
        ...
        if (bRetValue)
        {
            spdlog::debug("RoomEvent::CheckEvent: killed ({}/{}) monsters.", nOption_1, nOption_2);
            return true;
        }
        break;
...
bool CRoomEvent::RunEvent(int event_num)
{
    ...
    case 100:
        ...
        spdlog::debug(
            "RoomEvent::RunEvent: spawned {} of npcId({}) in roomNumber={}", nOption_2, nOption_1, m_sRoomNumber);
        if (nOption_1 != 0)
...
bool CRoomEvent::CheckMonsterCount(int sid, int count, int type)
{
    ...
    for (int i = 0; i < nMonster; i++)
    {
        ...
        {
            // Determine whether a certain number of specific monsters have been killed || 특정 몬스터가 마리수 만큼
            // 죽었는지를 판단
            if (type == 1)
            {
                if (pNpc->m_byChangeType == 100)
