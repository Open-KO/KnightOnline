...
bool SharedMemoryQueue::OpenOrCreate(const char *name)
{
    ...
    {
        spdlog::error(
            "SharedMemoryQueue::OpenOrCreate: failed to open or create shared memory. name='{}' ex='{}'",
            name,
            ex.what());
        _queue.reset();
    }
...
bool SharedMemoryQueue::Open(const char *name)
{
    ...
    catch (const interprocess_exception &ex)
    {
        if (ex.get_error_code() != not_found_error)
            spdlog::error(
                "SharedMemoryQueue::Open: failed to open existing shared memory. name='{}' ex='{}'", name, ex.what());
