...
TcpSocket::TcpSocket(SocketManager *socketManager)
    : _socketManager(socketManager), _socket(*socketManager->GetWorkerPool()),
      _recvCircularBuffer(socketManager->GetRecvBufferSize()), _sendCircularBuffer(socketManager->GetSendBufferSize())
{
...
bool TcpSocket::AsyncSend(bool fromAsyncChain)
{
    ...
    try
    {
        if (span.Buffer2 != nullptr && span.Length2 > 0)
        {
            ...
            _socket.async_write_some(
                buffers,
                std::bind(
                    &SocketManager::OnPostSend, _socketManager, std::placeholders::_1, std::placeholders::_2, this));
        }
        else
        {
            _socket.async_write_some(
                asio::buffer(span.Buffer1, span.Length1),
                std::bind(
                    &SocketManager::OnPostSend, _socketManager, std::placeholders::_1, std::placeholders::_2, this));
        }
    ...
    catch (const asio::system_error &ex)
    {
        ...
        spdlog::error("TcpSocket::AsyncSend: failed to post send for socketId={}: {}", _socketId, ex.what());
        Close();
        return false;
    }
...
void TcpSocket::AsyncReceive()
{
    ...
    try
    {
        _socket.async_read_some(
            ...
            std::bind(
                &SocketManager::OnPostReceive, _socketManager, std::placeholders::_1, std::placeholders::_2, this));
    }
    catch (const asio::system_error &ex)
    {
        spdlog::error("TcpSocket::Receive: failed to post receive for socketId={}: {}", _socketId, ex.what());
        Close();
    }
}
...
void TcpSocket::CloseProcess()
{
    ...
    if (_socket.is_open())
    {
        ...
        if (ec)
        {
            spdlog::error("TcpSocket::CloseProcess: shutdown() failed for socketId={}: {}", _socketId, ec.message());
        }

        _socket.close(ec);
        if (ec)
        {
            spdlog::error("TcpSocket::CloseProcess: close() failed for socketId={}: {}", _socketId, ec.message());
        }
    }
