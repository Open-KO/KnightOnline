...
bool SocketManager::Listen(int port)
{
    try
    {
        ...
        if (ec)
        {
            spdlog::error("SocketManager::Listen: bind() failed on 0.0.0.0:{}: {}", port, ec.message());
            return false;
        }
...
void SocketManager::OnPostReceive(const asio::error_code &ec, size_t bytesTransferred, TcpSocket *tcpSocket)
{
    if (ec)
    {
        ...
        {
            spdlog::debug(
                "SocketManager::OnPostReceive: peer closed connection. socketId={}", tcpSocket->GetSocketID());
        }
        else
        {
            spdlog::debug("SocketManager::OnPostReceive: socketId={} error={}", tcpSocket->GetSocketID(), ec.message());
    ...
    {
        spdlog::debug("SocketManager::OnPostReceive: closed by 0 byte notify. socketId={}", tcpSocket->GetSocketID());
        ProcessClose(tcpSocket);
        return;
    }
...
void SocketManager::OnPostSend(const asio::error_code &ec, size_t /*bytesTransferred*/, TcpSocket *tcpSocket)
{
    if (ec)
    {
        spdlog::error("SocketManager::OnPostSend: socketId={} failed: {}", tcpSocket->GetSocketID(), ec.message());
...
void SocketManager::OnPostServerSocketClose(TcpSocket *tcpSocket)
{
    ...
    spdlog::debug(
        "SocketManager::OnPostServerSocketClose: socket closed by Close() socketId={}", tcpSocket->GetSocketID());
}
...
void SocketManager::OnPostClientSocketClose(TcpClientSocket *tcpSocket)
{
    ...
    spdlog::debug(
        "SocketManager::OnPostClientSocketClose: socket closed by Close() socketId={}", tcpSocket->GetSocketID());
}
