...
class TcpSocket
{
  ...
  protected:
    ...
    // In the event there's too much data in the circular buffer to send, we allocate our own contiguous buffer here for
    // it, and submit that instead. This buffer is considered owned (by the send queue), so the buffer will be freed
