...
bool TcpClientSocket::Connect(const char *remoteAddress, uint16_t remotePort)
{
    ...
    if (ec)
    {
        spdlog::error("TcpClientSocket::Connect: invalid address {}: {}", remoteAddress, ec.message());
        return false;
    }
...
void TcpClientSocket::Close()
{
    ...
    {
        spdlog::error("TcpClientSocket::Close: failed to post close for socketId={}: {}", _socketId, ex.what());
    }
}
