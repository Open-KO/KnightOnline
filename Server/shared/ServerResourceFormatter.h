...
namespace fmt
{
	...
	{
		std::string get_from_db(uint32_t resourceId);

		inline std::string get_from_win32(uint32_t resourceId, const int outputCodePage = 949)
		{
			static wchar_t buffer[512];
			buffer[0] = '\0';

			::LoadStringW(nullptr, resourceId, buffer, _countof(buffer));

			return std::string(CW2A(buffer, outputCodePage));
		}
	}
	...
	{
		auto fmtStr = resource_helper::get_from_win32(resourceId);
		try
		{
