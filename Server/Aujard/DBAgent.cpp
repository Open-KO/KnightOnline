...
bool CDBAgent::LoadUserData(const char *accountId, const char *charId, int userId)
{
    ...
    if (user == nullptr)
    {
        spdlog::error("DBAgent::LoadUserData: UserData[{}] not found for charId={}", userId, charId);
        return false;
    }

    if (user->m_bLogout)
    {
        spdlog::error("DBAgent::LoadUserData: logout error: charId={}, logout={}", charId, user->m_bLogout);
        return false;
    }
    ...
    spdlog::debug(
        "DBAgent::LoadUserData: name={}, nation={}, zone={}, level={}, exp={}, money={}",
        charId,
        Nation,
        Zone,
        Level,
    ...
    for (int i = 0; i < HAVE_MAX + SLOT_MAX; i++)
    {
        ...
        if (pTable != nullptr)
        {
            ...
            spdlog::debug(
                "DBAgent::LoadUserData: {} : {} slot ({} : {})",
                user->m_id,
                i,
                user->m_sItemArray[i].nNum,
                user->m_sItemArray[i].nSerialNum);
        }
        else
        {
            user->m_sItemArray[i].nNum = 0;
    ...
    int cha)
{
    ...
    try
    {
        ...
        proc.execute(&retCode, accountId, index, charId, race, Class, hair, face, str, sta, dex, intel, cha);
    }
    catch (const nanodbc::database_error &dbErr)
    {
...
bool CDBAgent::LoadCharInfo(char *charId_, char *buff, int &buffIndex)
{
    ...
    for (int i = 0; i < SLOT_MAX; i++)
    {
        ...
        if (i == HEAD || i == BREAST || i == SHOULDER || i == LEG || i == GLOVE || i == FOOT || i == LEFTHAND ||
            i == RIGHTHAND)
        {
...
bool CDBAgent::GetAllCharID(const char *accountId, char *charId1_, char *charId2_, char *charId3_)
{
    ...
    {
        spdlog::error(
            "DBAgent::GetAllCharID: failed to write charId1(len: {}, val: {}) to charId1_", charId1.length(), charId1);
        return false;
    }
    ...
    {
        spdlog::error(
            "DBAgent::GetAllCharID: failed to write charId2(len: {}, val: {}) to charId2_", charId2.length(), charId2);
        return false;
    }
    ...
    {
        spdlog::error(
            "DBAgent::GetAllCharID: failed to write charId3(len: {}, val: {}) to charId3_", charId3.length(), charId3);
        return false;
    }
...
int CDBAgent::CreateKnights(int knightsId, int nation, char *name, char *chief, int flag)
{
    ...
    {
        spdlog::error(
            "DBAgent::CreateKnights: database error creating knights (knightsId={}, nation={}, name={}, chief={}, "
            "flag={}",
            knightsId,
            nation,
            name,
...
bool CDBAgent::LoadWarehouseData(const char *accountId, int userId)
{
    ...
    for (int i = 0; i < WAREHOUSE_MAX; i++)
    {
        ...
        if (itemData != nullptr)
        {
            ...
            spdlog::trace(
                "DBAgent::LoadWarehouseData: {}: {} ware slot ({} : {})",
                user->m_id,
                i,
                user->m_sWarehouseArray[i].nNum,
                user->m_sWarehouseArray[i].nSerialNum);
        }
        else
        {
            ...
            if (itemId > 0)
            {
                spdlog::error("DBAgent::LoadWarehouseData: item dropped itemId={} accountId={}", itemId, accountId);
            }
        }
    }
...
bool CDBAgent::UpdateWarehouseData(const char *accountId, int userId, int updateType)
{
    ...
    {
        spdlog::error("DBAgent::UpdateWarehouseData: called with inactive userId={} accountId={}", userId, accountId);
        return false;
    }
    ...
    try
    {
        ...
        auto weak_result = proc.execute(accountId, pUser->m_iBank, pUser->m_dwTime, items.storage(), serials.storage());
...
bool CDBAgent::LoadKnightsInfo(int knightsId, char *buffOut, int &buffIndex)
{
    ...
    try
    {
        ...
        {
            spdlog::error(
                "DBAgent::LoadKnightsInfo: knights.Name(len: {}, val: {}) exceeds length",
                knights.Name.length(),
                knights.Name);
            return false;
        }
...
bool CDBAgent::SetLogInInfo(
    const char *accountId, const char *charId, const char *serverIp, int serverId, const char *clientIp, uint8_t init)
{
    ...
    {
        query = fmt::format(
            "INSERT INTO [CURRENTUSER] ([strAccountID], [strCharID], [nServerNo], [strServerIP], [strClientIP]) VALUES "
            "(\'{}\',\'{}\',{},\'{}\',\'{}\')",
            accountId,
            charId,
            serverId,
    ...
    {
        query = fmt::format(
            "UPDATE [CURRENTUSER] SET [nServerNo]={}, [strServerIP]=\'{}\' WHERE [strAccountID] = \'{}\'",
            serverId,
            serverIp,
            accountId);
    }
    ...
    {
        spdlog::error(
            "DBAgent::SetLogInInfo: invalid init code specified (init: {}) for accountId={}", init, accountId);
        return false;
    }
    ...
    const char *accountId, const char *charId, int checkType, int userUpdateTime, int compareData)
{
    ...
    {
        spdlog::error(
            "DBAgent::CheckUserData: data mismatch dbTime(expected: {}, actual: {}) dbData(expected: {}, actual: {})",
            userUpdateTime,
            dbTime,
            compareData,
            dbData);
...
bool CDBAgent::CheckCouponEvent(const char* accountId)
{
    ...
    else
    {
        if (DisplayErrorMsg(hstmt) == -1)
        {
            ...
            {
                ReconnectIfDisconnected(&accountConn1, _main->m_strAccountDSN, _main->m_strAccountUID,
_main->m_strAccountPWD); return false;
            }
        }
...
bool CDBAgent::UpdateCouponEvent(const char* accountId, char* charId, char* cpid, int itemId, int count)
{
    ...
    wsprintf(szSQL, TEXT("{call UPDATE_COUPON_EVENT (\'%hs\', \'%hs\', \'%hs\', %d, %d)}"), accountId, charId, cpid,
itemId, count);
...
bool CDBAgent::DeleteChar(int index, char* id, char* charId, char* socno)
{
    ...
    if (retcode == SQL_SUCCESS)
    {
        retcode = SQLBindParameter(hstmt, 1, SQL_PARAM_OUTPUT, SQL_C_SSHORT, SQL_INTEGER, 0, 0, &sParmRet, 0,
&cbParmRet); if (retcode == SQL_SUCCESS)
        {
            retcode = SQLExecDirect(hstmt, (SQLTCHAR*) szSQL, SQL_NTS);
            if (retcode == SQL_ERROR)
            {
                if (DisplayErrorMsg(hstmt) == -1)
                {
                    ...
                    if (!gameConn1.IsOpen())
                    {
                        ReconnectIfDisconnected(&gameConn1, _main->m_strGameDSN, _main->m_strGameUID,
_main->m_strGamePWD); return false;
                    }
                }
