...
AujardApp::AujardApp(logger::Logger &logger) : AppThread(logger), LoggerSendQueue(MAX_SMQ_SEND_QUEUE_RETRY_COUNT)
{
    _sendPacketCount = 0;
    _packetCount = 0;
    _recvPacketCount = 0;
    ...
    _heartbeatCheckThread = std::make_unique<TimerThread>(40s, std::bind(&AujardApp::CheckHeartbeat, this));

    _concurrentCheckThread = std::make_unique<TimerThread>(5min, std::bind(&AujardApp::ConCurrentUserCount, this));

    _packetCheckThread = std::make_unique<TimerThread>(2min, std::bind(&AujardApp::WritePacketLog, this));

    _readQueueThread = std::make_unique<AujardReadQueueThread>();

    _smqOpenThread = std::make_unique<TimerThread>(5s, std::bind(&AujardApp::AttemptOpenSharedMemoryThreadTick, this));
}
...
{
    int index = 0, userId = -1, sendIndex = 0, idLen1 = 0, idLen2 = 0, tempUserId = -1, packetIndex = 0;
    uint8_t init = 0x01;
    ...
    spdlog::debug("AujardApp::SelectCharacter: acctId={}, charId={}, index={}", accountId, charId, packetIndex);
    ...
    {
        spdlog::error(
            "AujardApp::SelectCharacter: accountId too long (len: {}, val: {})", std::strlen(accountId), accountId);
...
bool AujardApp::HandleUserLogout(int userId, uint8_t saveType, bool forceLogout)
{
    ...
    spdlog::debug(
        "AujardApp::HandleUserLogout: Logout: {}, {} (UserData: {}, Logout: {})",
        pUser->m_Accountid,
        pUser->m_id,
        userdataSuccess,
        logoutResult);
...
bool AujardApp::HandleUserUpdate(int userId, const _USER_DATA &user, uint8_t saveType)
{
    ...
    for (retryCount = 0; !updateWarehouseResult || !updateUserResult; retryCount++)
    {
        ...
        {
            spdlog::error(
                "AujardApp::HandleUserUpdate: UserData Save Error: [accountId={} charId={} (W:{},U:{})]",
                user.m_Accountid,
                user.m_id,
        ...
        // only retry the calls that fail - they're both updating using UserData[userId]->dwTime, so they should sync
        // fine
        if (!updateUserResult)
        {
            updateUserResult = _dbAgent.UpdateUser(user.m_id, userId, saveType);
    ...
    for (retryCount = 0; !updateWarehouseResult || !updateUserResult; retryCount++)
    {
        if (retryCount >= maxRetry)
        {
            ...
            {
                spdlog::error(
                    "AujardApp::HandleUserUpdate: Warehouse Save Check Error [accountId={} charId={} (W:{})]",
                    user.m_Accountid,
                    user.m_id,
                    updateWarehouseResult);
            }
            ...
            {
                spdlog::error(
                    "AujardApp::HandleUserUpdate: UserData Save Check Error: [accountId={} charId={} (U:{})]",
                    user.m_Accountid,
                    user.m_id,
                    updateUserResult);
            }
...
{
    int index = 0, userId = -1, accountIdLen = 0, charIdLen = 0, sendIndex = 0, result = 0, charIndex = 0, race = 0,
        Class = 0, hair = 0, face = 0, str = 0, sta = 0, dex = 0, intel = 0, cha = 0;
...
{
    int index = 0, userId = -1, accountIdLen = 0, charIdLen = 0, sendIndex = 0, result = 0, charIndex = 0, socNoLen = 0;
    char accountId[MAX_ID_SIZE + 1] = {}, charId[MAX_ID_SIZE + 1] = {}, socNo[15] = {}, sendBuff[256] = {};
...
void AujardApp::AllCharInfoReq(const char *buffer)
{
    ...
    char accountId[MAX_ID_SIZE + 1] = {}, sendBuff[1024] = {}, charBuff[1024] = {}, charId1[MAX_ID_SIZE + 1] = {},
                                 charId2[MAX_ID_SIZE + 1] = {}, charId3[MAX_ID_SIZE + 1] = {};
...
void AujardApp::ConCurrentUserCount()
{
    ...
    spdlog::trace(
        "AujardApp::ConCurrentUserCount: [serverId={} zoneId={} userCount={}]", _serverId, _zoneId, usercount);
...
void AujardApp::UserDataSave(const char *buffer)
{
    ...
    {
        spdlog::error(
            "AujardApp::UserDataSave: failed for UserData[{}] [accountId={} charId={}]", userId, accountId, charId);
    }
}
...
void AujardApp::KnightsPacket(const char *buffer)
{
    ...
    default:
        spdlog::error("AujardApp::KnightsPacket: Invalid WIZ_KNIGHTS_PROCESS command code received: {:X}", command);
    }
}
...
{
    int index = 0, sendIndex = 0, nameLen = 0, chiefNameLen = 0, knightsId = 0, userId = -1, nation = 0, result = 0,
        community = 0;
    ...
    spdlog::trace("AujardApp::CreateKnights: userId={}, knightsId={}, result={}", userId, knightsId, result);
...
void AujardApp::JoinKnights(const char *buffer)
{
    ...
    spdlog::trace(
        "AujardApp::JoinKnights: userId={}, charId={}, knightsId={}, result={}",
        userId,
        pUser->m_id,
        knightsId,
        result);
...
void AujardApp::WithdrawKnights(const char *buffer)
{
    ...
    spdlog::trace("AujardApp::WithdrawKnights: userId={}, knightsId={}, result={}", userId, knightsId, result);
...
void AujardApp::ModifyKnightsMember(const char *buffer, uint8_t command)
{
    ...
    spdlog::trace(
        "AujardApp::ModifyKnights: command={}, userId={}, knightsId={}, result={}", command, userId, knightsId, result);
...
void AujardApp::DestroyKnights(const char *buffer)
{
    ...
    spdlog::trace("AujardApp::DestroyKnights: userId={}, knightsId={}, result={}", userId, knightsId, result);
...
{
    int index = 0, accountIdLen = 0, charIdLen = 0, serverId = 0, serverIpLen = 0, clientIpLen = 0, userId = -1,
        sendIndex = 0;
    ...
    if (!_dbAgent.SetLogInInfo(accountId, charId, serverIp, serverId, clientIp, init))
    {
        ...
        {
            spdlog::error(
                "AujardApp::SetLoginInfo: Packet Drop: WIZ_LOGIN_INFO [accountId={}, charId={}, init={}]",
                accountId,
                charId,
                init);
        }
...
{
    spdlog::info(
        "AujardApp::WritePacketLog: Packet Count: recv={}, send={}, realsend={}",
        _recvPacketCount,
        _packetCount,
        _sendPacketCount);
}
...
void AujardApp::BattleEventResult(const char *data)
{
    ...
    if (charIdLen > 0 && charIdLen < MAX_ID_SIZE + 1)
    {
        ...
        spdlog::info(
            "AujardApp::BattleEventResult : The user who killed the enemy commander is {}, _type={}, nation={}",
            charId,
            _type,
            result);
...
void AujardApp::CouponEvent(const char *data)
{
    ...
    char strAccountName[MAX_ID_SIZE + 1] = {}, strCharName[MAX_ID_SIZE + 1] = {}, strCouponID[MAX_ID_SIZE + 1] = {},
                                      sendBuff[256] = {};
