...
void CMainFrame::OnToolConvertFilesAutomaticaly()
{
    ...
    CFileDialog dlg(
        TRUE, "dxt", nullptr, dwFlags, "Generic Image Files(*.bmp, *.tga, *.jpg)|*.bmp;*.tga;*.jpg||", nullptr);
    char szCurPath[256];
...
void CMainFrame::OnToolConvertFilesManually()
{
    ...
    CFileDialog dlg(
        ...
        nullptr,
        dwFlags,
        "Generic Image Files(*.dxt, *.bmp, *.tga, *.jpg)|*.dxt;*.bmp;*.tga;*.jpg|BitmapFile File(*.bmp)|*.bmp|Targa "
        "File(*.tga)|*.bmp|DXT File(*.dxt)|*.dxt||",
        nullptr);
    char szCurPath[256];
    GetCurrentDirectory(256, szCurPath);
    dlg.m_ofn.lpstrInitialDir = szCurPath;
...
BOOL CMainFrame::BMPCutter(LPCTSTR lpszFileName, int iWidth, int iHeight, bool bSaveToDXT, D3DFORMAT fmtDXT)
{
    ...
    //	pSrcImageBit = (LPVOID)((BYTE*)pSrcDIB + (bmfHeader.bfOffBits - sizeof(bmfHeader)));
        ...
        HANDLE hFile = CreateFile(szFNameDest, GENERIC_WRITE, 0, nullptr, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL,
    nullptr); if(INVALID_HANDLE_VALUE != hFile)
        {
        ...
        for (j=0; j<iCY; ++j)
        {
            for (i=0; i<iCX; ++i)
            {
                ...
                for(y=0; y<iHeight; ++y)
                {
                    ...
                    if (i == (iCX-1))
                    {	// 맨 오른쪽 끝은 짤릴 가능성이 있다.
                        file.Read(pTmpBitDest + iRealWidthDest*(iHeight-1-y), bmInfoHeader.biWidth*3 -
    iRealWidthDest*(iCX-1));
                    }
                    else
                    {
                ...
                hFile = CreateFile(szFNameDest, GENERIC_WRITE, 0, nullptr, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL,
    nullptr); if(INVALID_HANDLE_VALUE != hFile)
                {
