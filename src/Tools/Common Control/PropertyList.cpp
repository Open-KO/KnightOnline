...
void CPropertyList::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
	...
	if (nIndex != (UINT) -1)
	{
		...
		dc.DrawText(pItem->m_propName,
			CRect(rect2.left + 3, rect2.top + 3, rect2.right - 3, rect2.bottom + 3),
		...
		dc.DrawText(pItem->m_curValue,
			CRect(rect.left + 3, rect.top + 3, rect.right + 3, rect.bottom + 3),
			DT_LEFT | DT_SINGLELINE);
	}
	dc.Detach();
}
...
int CPropertyList::AddPropItem(const char* szPropName, const char* szVal, int nItemType,
	const char* szCmbItems, COLORREF crColor)
{
...
void CPropertyList::OnSelchange()
{
	...
	if (pItem->m_nItemType == PIT_COMBO)
	{
		...
		else
		{
			rect.bottom += 300;
			m_ComboBox.Create(
				CBS_DROPDOWNLIST | CBS_NOINTEGRALHEIGHT | WS_VISIBLE | WS_CHILD | WS_BORDER, rect,
				this, IDC_PROPCMBBOX);
		...
		int nCmbC = static_cast<int>(pItem->m_szCBItemsOrFilter.GetSize());
	...
	else if (pItem->m_nItemType == PIT_EDIT)
	{
		...
		{
			m_EditBox.Create(ES_LEFT | ES_AUTOHSCROLL | WS_VISIBLE | WS_CHILD | WS_BORDER, rect,
				this, IDC_PROPEDITBOX);
...
void CPropertyList::DisplayPushButton(CRect region)
{
	...
	{
		m_ButtonPush.Create(
			"...", BS_PUSHBUTTON | WS_VISIBLE | WS_CHILD, region, this, IDC_PROP_BUTTON);
...
void CPropertyList::OnSelchangeCmbBox()
{
	...
	if (m_ComboBox)
	{
		...
		if (pWnd != nullptr)
			pWnd->SendMessage(
				WM_NOTIFY, (DWORD_PTR) this, (DWORD_PTR) pItem); // 부모 윈도우에 메시지 보내기..
	}
}
...
void CPropertyList::OnButton()
{
	...
	else if (pItem->m_nItemType == PIT_FILE_MULTI)
	{
		...
		if (IDOK == dlg.DoModal())
		{
			...
			int iSC           = static_cast<int>(szFNs.GetSize());
			if (iSC > 0)
			{
				pItem->m_curValue += szFNs[iSC - 1];
				pItem->m_curValue += '\n';

				// 1 부터 시작하는 이유는 파일 대화상자에서 여러 파일을 부르면 첨과 끝 파일 이름이 바뀌기 때문이다.
				...
				CWnd* pWnd = GetParent();
				if (pWnd != nullptr)
					pWnd->SendMessage(WM_NOTIFY, (DWORD_PTR) this,
						(DWORD_PTR) pItem); // 부모 윈도우에 메시지 보내기..
			}
		}
	}
...
{
	m_crColor = (((DWORD) (color.r * 255.0f)) << 0) | (((DWORD) (color.g * 255.0f)) << 8)
