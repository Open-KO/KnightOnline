...
CViewProperty::CViewProperty() : CFormView(CViewProperty::IDD)
{
...
{
	if (m_LPTransform.GetSafeHwnd() == nullptr)
		return;
	...
	if (pBase->Type() & OBJ_SHAPE)
	{
		...
		if (pItem)
			pItem->m_curValue.Format(
				"%d", pS->m_iNPC_Status); // NPC 로 쓰는 오브젝트일 경우 NPC Type
		...
		if (nPart >= 0 && nPart < nPartCount)
		{
			...
			if (pItem)
			{
				if (pPMesh)
					pItem->m_curValue.Format(
						"%4d, %4d", pPMesh->GetMaxNumIndices() / 3, pPMesh->GetMinNumVertices());
				else
					pItem->m_curValue = "";
			}
	...
	if (pBase->Type() & OBJ_CHARACTER)
	{
		...
		m_CBChrLOD.SetWindowPos(
			nullptr, pt.x + (nW / 2) + 2, pt.y, (nW / 2) - 2, rc.Height(), SWP_NOZORDER);
	...
	if (pBase != nullptr)
	{
		...
		if ((void*) wParam == &m_LPCamera && (pBase->Type() & OBJ_CAMERA))
		{
			...
			if (pItem->m_propName == "Eye")
				pC->EyePosSet(pItem->VectorGet());
			else if (pItem->m_propName == "At")
				pC->AtPosSet(pItem->VectorGet());
			else if (pItem->m_propName == "Up")
				pC->UpVectorSet(pItem->VectorGet());
		...
		if ((void*) wParam == &m_LPShape && (pBase->Type() & OBJ_SHAPE))
		{
			...
			else if (pItem->m_propName == "Collision Mesh File"
					 && pItem->m_curValue.GetLength() > 0)
		...
		if ((void*) wParam == &m_LPCPlug && (pBase->Type() & OBJ_CHARACTER))
		{
			...
			if (nPlug >= 0 && nPlug < nPlugCount)
			{
				...
				else if (pItem->m_propName == "Plug Joint")
				{
					...
					pPlug->PositionSet(pItem->VectorGet());
				else if (pItem->m_propName == "Plug Rotation")
				{
					...
					pPlug->ScaleSet(pItem->VectorGet());
				...
				else if (pItem->m_propName == "Plug Texture File"
						 && pItem->m_curValue.GetLength() > 0)
				{
					CN3BaseFileAccess tmp;
					tmp.FileNameSet(std::string(pItem->m_curValue));
...
void CViewProperty::OnInitialUpdate()
{
	...
	D3DTOP_BUMPENVMAP|D3DTOP_BUMPENVMAPLUMINANCE|D3DTOP_DOTPRODUCT|D3DTOP_MULTIPLYADD|D3DTOP_LERP|";
		...
		m_LPShape.AddPropItem(
			"Mesh File", "", PIT_FILE, "N3 Progressive Mesh File(*.N3PMesh)|*.N3PMesh||");
		m_LPShape.AddPropItem("Make Smooth", "", PIT_BUTTON, "");
		m_LPShape.AddPropItem("Make Sharp", "", PIT_BUTTON, "");
		m_LPShape.AddPropItem(
			"Collision Mesh File", "", PIT_FILE, "N3 Vector Mesh(*.N3VMesh)|*.N3VMesh||");
		...
		m_LPCPart.AddPropItem(
			"Part File", "", PIT_FILE, "N3 Character part file(*.N3CPart)|*.N3CPart||");
		...
		m_LPCPlug.AddPropItem(
			"Plug Mesh File", "", PIT_FILE, "N3 Progressive Mesh File(*.N3PMesh)|*.N3PMesh||");
