...
BOOL CMainFrame::OnCreateClient(LPCREATESTRUCT /*lpcs*/, CCreateContext* pContext)
{
	...
	if (m_wndSplitter.CreateView(
			0, 1, RUNTIME_CLASS(CN3ViewerView), CSize(rc.Width() - nW1, rc.Height()), pContext)
		== FALSE)
		return FALSE;

	if (m_wndSplitterLeft.CreateStatic(&m_wndSplitter, 2, 1) == FALSE)
		return FALSE;
	if (m_wndSplitterLeft.CreateView(
			0, 0, RUNTIME_CLASS(CViewSceneTree), CSize(nW1, rc.Height() / 2), pContext)
		== FALSE)
		return FALSE;
	if (m_wndSplitterLeft.CreateView(
			1, 0, RUNTIME_CLASS(CViewProperty), CSize(nW1, rc.Height() / 2), pContext)
		== FALSE)
		return FALSE;
...
void CMainFrame::OnToolFixProgressiveMesh()
{
	...
	MessageBox("프로그레시브 처리가 안된 파일리스트는 \"프로그레시브 메쉬 처리 안된 리스트.txt\" "
			   "파일에 기록되었습니다.");
	file.Close();
