...
CDlgEditScript::CDlgEditScript(CWnd* pParent /*=nullptr*/) : CDialog(CDlgEditScript::IDD, pParent)
{
...
void CDlgEditScript::OnBtnSaveAs()
{
	...
	if (dlg.DoModal() == IDOK)
	{
		...
		sprintf(
			szGameFileName, "%s%s%s%s", szDrive, szDir, dlg.m_strNewFileName.GetString(), szExt);
