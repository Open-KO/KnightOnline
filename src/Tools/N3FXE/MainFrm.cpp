...
bool CMainFrame::IsDuplicated(CString& PathName)
{
	...
	std::list<CDlgEditPartBillBoard*>::iterator itPartBillBoard;
	for (itPartBillBoard = m_pEditPartBillBoard.begin();
		itPartBillBoard != m_pEditPartBillBoard.end(); itPartBillBoard++)
	{
	...
	std::list<CDlgEditPartParticle*>::iterator itPartParticle;
	for (itPartParticle = m_pEditPartParticle.begin(); itPartParticle != m_pEditPartParticle.end();
		itPartParticle++)
	{
	...
	std::list<CDlgEditPartGround*>::iterator itPartGround;
	for (itPartGround = m_pEditPartGround.begin(); itPartGround != m_pEditPartGround.end();
		itPartGround++)
	{
...
void CMainFrame::OnFileLoadPart()
{
	...
	CFileDialog dlg(
		TRUE, "N3FXPart", nullptr, dwFlags, "N3FXPart File(*.n3fxpart)|*.n3fxpart||", nullptr);
...
void CMainFrame::OnFileLoadBundle()
{
	DWORD dwFlags = OFN_EXPLORER | OFN_FILEMUSTEXIST | OFN_LONGNAMES | OFN_HIDEREADONLY
					| OFN_ALLOWMULTISELECT;
	CFileDialog dlg(TRUE, "N3FXBundle", nullptr, dwFlags,
		"N3FXBundle File(*.n3fxbundle)|*.n3fxbundle||", nullptr);
...
void CMainFrame::OnFileLoadGroup()
{
	DWORD dwFlags = OFN_EXPLORER | OFN_FILEMUSTEXIST | OFN_LONGNAMES | OFN_HIDEREADONLY
					| OFN_ALLOWMULTISELECT;
	CFileDialog dlg(
		TRUE, "N3FXGroup", nullptr, dwFlags, "N3FXGroup File(*.n3fxgroup)|*.n3fxgroup||", nullptr);
