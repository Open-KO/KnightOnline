...
BOOL CMainFrame::OnCreateClient(LPCREATESTRUCT /*lpcs*/, CCreateContext* pContext)
{
	...
	if (m_wndSplitter.CreateView(
			0, 0, RUNTIME_CLASS(CFormViewProperty), CSize(nW1, rc.Height()), pContext)
		== FALSE)
		return FALSE;
	if (m_wndSplitter.CreateView(
			0, 1, RUNTIME_CLASS(CSkyViewerView), CSize(rc.Width() - nW1, rc.Height()), pContext)
		== FALSE)
		return FALSE;
...
void CMainFrame::OnImportObject()
{
	...
	CFileDialog dlg(
		TRUE, ".N3Shape", nullptr, dwFlags, "Shape Data(*.N3Shape)|*.N3Shape||", nullptr);
