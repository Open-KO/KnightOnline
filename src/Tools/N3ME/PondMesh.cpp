...
void CPondMesh::Render()
{
	...
	if (m_pTexture && m_iVC > 2 && m_iIC > 0)
	{
		...
		// render
		s_lpD3DDev->DrawIndexedPrimitiveUP(D3DPT_TRIANGLELIST, 0, m_iVC, m_iIC, m_pdwIndex,
			D3DFMT_INDEX16, m_pViewVts //m_pVertices
			,
	...
	else
	{
		...
		s_lpD3DDev->DrawPrimitiveUP(
			D3DPT_LINESTRIP, m_iWaterScaleWidth, m_pViewVts, sizeof(__VertexXyzT2));
...
void CPondMesh::UpdateDrawPos()
{
	m_iRectVC      = 0;
	//	위,아래점들 세팅
	m_fWaterScaleX = SettingDrawPos(
		m_vDrawBox[0], m_vDrawBox[1], m_vDrawBox[3], m_vDrawBox[2], m_iWaterScaleWidth);
	//	좌우점들 세팅
	m_fWaterScaleZ = SettingDrawPos(
		m_vDrawBox[0], m_vDrawBox[3], m_vDrawBox[1], m_vDrawBox[2], m_iWaterScaleHeight);
}

float CPondMesh::SettingDrawPos(
	__Vector3 vPos1, __Vector3 vPos2, __Vector3 vPos3, __Vector3 vPos4, int iLinePosNum)
{
	if (iLinePosNum == 0)
		return 0.0f;
...
void CPondMesh::MakePondPos()
{
	...
	if (m_pdwIndex == nullptr
		|| (m_iBackUpWidht != m_iWaterScaleWidth || m_iBackUpHeight != m_iWaterScaleHeight))
	{
		delete[] m_pdwIndex;
	...
	ASSERT(MAX_PONDMESH_VERTEX
		   > m_iWaterScaleWidth
				 * m_iWaterScaleHeight); //	앞으로 만들어질 점의숫자가 max에 달하는지 확인
	int ix, iz, itemp;
...
void CPondMesh::InputSelectPos(float fX, float fY, float fZ, int iVC)
{
	...
	if (iVC > -1) //	전체 선택이 아닌 일부선택시
	{
		...
		{
			if (pSelpo != nullptr && pSelpo->ix == iWidth && pSelpo->iz == iHeight)
			{
				pSelpo = nullptr;
				return;
			}
...
void CPondMesh::SetAllPos(int iSx, int iSy, int iEx, int iEy)
{
	...
	vCenter = *(
		m_pVertices + m_iWaterScaleWidth * iSy + iSx); //	현재 지정된 점(나중에 여러개 지정되게)
	vNowCenter = *(m_pViewVts + m_iWaterScaleWidth * iSy
				   + iSx); //	백업된 전의 좌표(참조하여 새로운 좌표 계산)
...
bool CPondMesh::Load1000(File& file)
{
	...
	for (i = 0; i < m_iVC; ++i)
	{
		m_pViewVts[i].x = m_pVertices[i].x, m_pViewVts[i].y = m_pVertices[i].y,
		m_pViewVts[i].z = m_pVertices[i].z;
	...
	m_vDrawBox[0].x += 1.0f, m_vDrawBox[3].x += 1.0f, m_vDrawBox[1].x -= 1.0f,
		m_vDrawBox[2].x -= 1.0f;
	m_vDrawBox[0].z -= 1.0f, m_vDrawBox[3].z -= 1.0f, m_vDrawBox[1].z += 1.0f,
		m_vDrawBox[2].z += 1.0f;
