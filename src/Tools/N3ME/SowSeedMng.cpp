...
BOOL CSowSeedMng::MouseMessage(LPMSG pMsg)
{
	...
	switch (pMsg->message)
	{
		case WM_MOUSEMOVE:
		{
			...
			{
				// 브러시 업데이트
				if (pFrame->GetMapMng()->GetTerrain()->Pick(
						point.x, point.y, nullptr, &ptSelHeightMapPos))
					...
					//				if( (pFrame->m_pDlgSowSeed->Sow_Array[i][j] == 2) || (pFrame->m_pDlgSowSeed->Sow_Array[i][j] == 1) )
					{
						if (Grass != nullptr)
						{
							...
							if (Grass[Num].Sub_Grass != nullptr)
							{
								for (int k = 0; k < Grass[Num].Sub_Grass_Count; k++)
								{
									Grass[Num].Sub_Grass[k].Tile_x = ptSelHeightMapPos.x
																	 - BRUSH_CENTER + i;
									Grass[Num].Sub_Grass[k].Tile_z = ptSelHeightMapPos.y
																	 - BRUSH_CENTER + j;
		...
		for (i = 0; i < static_cast<int>(Grass_Group.size()); i++, it++)
		{
			...
			if (Select_Group_Id != i)
			{
				for (j = 0; j < static_cast<int>(group->grass.size()); j++, it_grass++)
				{
					...
					CN3Shape* pShape = (CN3Shape*) pFrame->GetMapMng()
										   ->m_pDlgSourceList->m_ListShape.GetItemDataPtr(
											   group->Obj_ID);
					if ((CN3Shape*) pShape)
					{
...
void CSowSeedMng::Add_Grass(void)
{
	if (Grass != nullptr)
	{
		...
		CN3Shape* pShape = (CN3Shape*) pFrame->GetMapMng()
							   ->m_pDlgSourceList->m_ListShape.GetItemDataPtr(temp->Obj_ID);
		...
		for (i = 0, listCount = 0; i < Grass_Count; i++)
		{
			...
			if ((add_list_group == true) && (grass->Tile_x > 0) && (grass->Tile_z > 0))
			{
				...
				char text[256];
				sprintf(text, "SubID:%d, x :%f, y :%f, z :%f", listCount, grass->Pos.x,
					grass->Pos.y, grass->Pos.z);
			...
			if (Grass[i].Sub_Grass != nullptr)
			{
				for (int j = 0; j < Grass[i].Sub_Grass_Count; j++)
				{
					...
					for (int ii = 0; ii < static_cast<int>(Grass_Group.size()); ii++, it++)
					{
						LPGRASS_GROUP group_list = *it;
						it_Grass it_grass        = group_list->grass.begin();
						for (int jj = 0; jj < static_cast<int>(group_list->grass.size());
							jj++, it_grass++)
						{
							LPGRASS grass_list = *it_grass;
					...
					if ((add_list == true) && (grass_sub->Tile_x > 0) && (grass_sub->Tile_z > 0))
					{
						...
						char text[256];
						sprintf(text, "SubID:%d, x :%f, y :%f, z :%f", listCount, grass_sub->Pos.x,
							grass_sub->Pos.y, grass_sub->Pos.z);
						pFrame->m_pDlgSowSeed->m_CB_Seed.AddString(text);
						listCount++;
					}
				}
...
{
	WORD m_Index[26] = { 0, 1, 1, 4, 4, 5, 5, 0, 3, 2, 2, 7, 7, 6, 6, 3, 1, 2, 0, 3, 1, 2, 4, 7, 5,
		6 };
	DWORD dwLight;
	...
	lpD3DDevice->DrawIndexedPrimitiveUP(
		D3DPT_LINELIST, 0, 8, 13, m_Index, D3DFMT_INDEX16, m_pVertices, sizeof(__VertexColor));
