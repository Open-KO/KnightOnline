...
CDlgSetDTex::CDlgSetDTex(CWnd* pParent /*=nullptr*/) : CDialog(CDlgSetDTex::IDD, pParent)
{
...
BOOL CDlgSetDTex::OnInitDialog()
{
	...
	pView->SetWindowPos(nullptr, 0, 0, (int) m_fTexSurfaceSize, (int) m_fTexSurfaceSize,
		SWP_DRAWFRAME | SWP_NOMOVE);

	//	texture 그리는 버퍼..
	pFrm->m_pEng->s_lpD3DDev->CreateVertexBuffer(
		4 * sizeof(__VertexTransformed), 0, FVF_TRANSFORMED, D3DPOOL_MANAGED, &m_pTexVB, nullptr);
	...
	pVertices[1].Set(m_fTexSurfaceSize - 1.0f, 0.0f, 0.1f, 0.5f, 0x00000000, DTexSize / DTexSize,
		1.0f / DTexSize);
	pVertices[2].Set(m_fTexSurfaceSize - 1.0f, m_fTexSurfaceSize - 1.0f, 0.1f, 0.5f, 0x00000000,
		DTexSize / DTexSize, DTexSize / DTexSize);
	pVertices[3].Set(0.0f, m_fTexSurfaceSize - 1.0f, 0.1f, 0.5f, 0x00000000, 1.0f / DTexSize,
		DTexSize / DTexSize);
	m_pTexVB->Unlock();

	//	Grid 그리는 버퍼..
	pFrm->m_pEng->s_lpD3DDev->CreateVertexBuffer(
		((NUM_DTEXTILE - 1) << 2) * sizeof(__VertexTransformedColor), 0, FVF_TRANSFORMEDCOLOR,
		D3DPOOL_MANAGED, &m_pGridVB, nullptr);
		...
		if (dlg.DoModal() == IDOK)
		{
			...
			for (int i = 0; i < NUM_DTEXTILE; i++)
			{
				...
				if (pDTex)
				{
					...
					pDTexGroupMng->SetTile(
						pDTex->m_Attr[i][iy].Group, pDTex->m_Attr[i][iy].Attr, tile);
				}
			}
		}
	}
		...
		if (pRefTerrain)
		{
			...
			for (ix = 0; ix < pRefTerrain->m_iHeightMapSize; ix++)
			{
				for (iz = 0; iz < pRefTerrain->m_iHeightMapSize; iz++)
				{
					...
					if (TargetID == DelGroupID)
					{
						pRefTerrain->m_ppMapData[ix][iz]
							.DTexInfo1 = pRefTerrain->m_ppMapData[ix][iz].DTexInfo2;
