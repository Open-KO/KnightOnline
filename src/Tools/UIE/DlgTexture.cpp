...
CDlgTexture::CDlgTexture(CWnd* pParent /*=nullptr*/) : CDialog(CDlgTexture::IDD, pParent)
{
...
int CDlgTexture::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	if (FALSE
		== m_pTexViewer->Create(
			nullptr, "TexViewer", WS_CHILD | WS_VISIBLE, CRect(0, 0, 0, 0), this, 1000))
		return -1;
...
LRESULT CDlgTexture::OnUpdateInfo(WPARAM wParam, LPARAM lParam)
{
	...
	if (-1 == rcSelected.left)
		...
		strRect.Format("Left : %d\nTop : %d\nRight : %d\nBottom : %d\nWidth : %d\nHeight : %d\n",
			rcSelected.left, rcSelected.top, rcSelected.right, rcSelected.bottom,
			rcSelected.Width(), rcSelected.Height());
