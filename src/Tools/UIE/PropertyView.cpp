...
void CPropertyView::OnInitialUpdate()
{
	...
	if (FALSE == bInitCtrl)
	{
		...
		m_UIString.AddPropItem(
			"H align", "", PIT_COMBO, "LEFT|CENTER|RIGHT|");      // style(horizontal align)
		m_UIString.AddPropItem(
			"V align", "", PIT_COMBO, "TOP|VCENTER|BOTTOM|");     // style(vertical align)
		...
		// progress
		m_UIProgress.AddPropItem(
			"Style", "", PIT_COMBO, "오른쪽 증가|왼쪽 증가|아래로 증가|위로 증가|");
		...
		m_UIProgress.AddPropItem(
			"Delete BkGnd", "click", PIT_BUTTON, ""); // 배경 이미지 지우는 버튼
		...
		// area
		m_UIArea.AddPropItem(
			"Type", "", PIT_COMBO, "area_none|area_slot|area_inv|area_trade_npc|area_per_trade_my|\
...
BOOL CPropertyView::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT* pResult)
{
	...
	if ((void*) wParam == (&m_UIImage) && UI_TYPE_IMAGE == pUI->UIType())
	{
		...
		if (pItem->m_propName == "Texture")
		{
			...
			tmpBase.FileNameSet(
				(LPCTSTR) pItem->m_curValue); // Base경로에 대해서 상대적 경로를 넘겨준다.
		...
		{ // Animation image로 만들기
			...
			while (1)
			{
				...
				{ // 반드시 부모는 image가 아니어야 한다.
					pFrm->MessageBox(
						"Animate image의 자식 image는 animate image로 만들 수 없습니다.");
					break;
				}
				CDlgAnimate dlgAnim;
				...
				for (i = 0; i < dlgAnim.m_iCount; ++i)
				{
					...
					if (nullptr == pChildImage)
						continue;
					pChildImage->SetTex(szTexFName);                       // texture설정
					pChildImage->SetUVRect(
						frcUV.left, frcUV.top, frcUV.right, frcUV.bottom); // uv 설정
				}
				// 위치 설정
	...
	if ((void*) wParam == (&m_UIList) && UI_TYPE_LIST == pUI->UIType())
	{
		...
		if (pItem->m_propName == "Font" || pItem->m_propName == "Font size"
			|| pItem->m_propName == "Font style")
		{
			// font 정하는 루틴
			LOGFONT logfont;
	...
	if ((void*) wParam == (&m_UIButton) && UI_TYPE_BUTTON == pUI->UIType())
	{
		...
		if (pItem->m_propName == "Style")
		{
			...
			pBtn->SetStyle(dwStyle);
				 ...
				 || pItem->m_propName == "ClickRect right"
				 || pItem->m_propName == "ClickRect bottom")
		{
			// 클릭 영역 지정하는 코드 넣기
	...
	// static
	if ((void*) wParam == (&m_UIStatic)
		&& (UI_TYPE_STATIC == pUI->UIType() || UI_TYPE_EDIT == pUI->UIType()))
	{
...
{
	if (m_UIBase.GetSafeHwnd() == nullptr)
		return;
...
void CPropertyView::SetPropertyListPos(CPropertyList* pList, CPoint& ptWndPos)
{
	...
	pList->SetWindowPos(
		nullptr, ptWndPos.x, ptWndPos.y, rc.Width(), iHeight, SWP_NOZORDER | SWP_SHOWWINDOW);
