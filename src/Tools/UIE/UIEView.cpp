	...
	else if (UIEMODE_EDIT == m_eMode)
	{
		...
		{                                     // m_RootUI의 자식중에서 point에 위치한 ui 선택하기
			if (!(nFlags & MK_CONTROL))
				pDoc->SetSelectedUI(nullptr); // 컨트롤 키를 누르지 않으면 멀티 셀렉트 해제후..

			CN3UIBase* pRootUI     = GetDocument()->GetRootUI();
			CN3UIBase* pUISelected = nullptr;
			for (UIListItor itor = pRootUI->m_Children.begin(); pRootUI->m_Children.end() != itor;
				++itor)
			{
				CN3UIBase* pUI = (*itor);
			...
			if (nullptr == pUISelected && pRootUI->IsIn(point.x, point.y))
...
void CUIEView::UpdateStatusBarText()
{
	...
	else if (UIEMODE_EDIT == m_eMode)
	{
		...
		{
			strText.Format(
				"%s  (left=%04d top=%04d right=%04d bottom=%04d)  (Width=%04d Height=%04d)",
				strRect, m_rcSelectedRect.left, m_rcSelectedRect.top, m_rcSelectedRect.right,
				m_rcSelectedRect.bottom, m_rcSelectedRect.Width(), m_rcSelectedRect.Height());
		}
		else
		{
			CPoint pt;
...
{
	CN3UIBase* pSelectedUI = GetDocument()->GetSelectedUI();
	if (pSelectedUI && UIEMODE_EDIT == m_eMode && RT_NONE != m_eSelectedRectType
		&& -1000 != m_rcSelectedRect.left)
	{
		...
		switch (m_eSelectedRectType)
		{
			case RT_REGION:
			{
				...
				for (int i = 0; i < iUIC; i++)
				{
					...
					{
						pSelectedUI->GetParent()
							->ResizeAutomaticalyByChild(); // 자동으로 영역 다시 계산..
					}
				}
			}
			break;
...
void CUIEView::ChangeBkgndColor()
{
	...
	m_BkgndColor = 0xff000000 | ((clr & 0x00ff0000) >> 16) | (clr & 0x0000ff00)
				   | ((clr & 0x000000ff) << 16);
...
void CUIEView::OnDropFiles(HDROP hDropInfo)
{
	...
	if (len >= 3 && 0 == lstrcmpi(szExt, "uif") && pDoc)
	{
