...
bool TcpClientSocket::Create()
{
	...
	if (_socket == nullptr)
	{
		spdlog::error("{}::Create: failed to allocate socket: out of memory", GetSocketClass());
		return false;
	}
	...
	if (ec)
	{
		spdlog::error("::Create: failed to open socket: {}", GetSocketClass(), ec.message());
		return false;
	}
	...
	{
		spdlog::error(
			"{}::Create: failed to set linger option: {}", GetSocketClass(), ec.message());
		return false;
	}
	...
	{
		spdlog::error(
			"{}::Create: failed to set receive buffer size: {}", GetSocketClass(), ec.message());
		return false;
	}
	...
	{
		spdlog::error(
			"{}::Create: failed to set send buffer size: {}", GetSocketClass(), ec.message());
		return false;
	}
...
bool TcpClientSocket::Connect(const char* remoteAddress, uint16_t remotePort)
{
	...
	if (!Create())
	{
		spdlog::error("{}::Connect: failed to create new socket [socketId={}]", GetSocketClass(),
			GetSocketID());
		return false;
	}
	...
	if (ec)
	{
		spdlog::error("{}::Connect: invalid address {}: {} [socketId={}]", GetSocketClass(),
	...
	if (ec)
	{
		spdlog::error("{}::Connect: failed to connect: {} [socketId={}]", GetSocketClass(),
			ec.message(), GetSocketID());
		_socket->close();
		return false;
	}
