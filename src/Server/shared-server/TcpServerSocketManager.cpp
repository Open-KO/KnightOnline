...
TcpServerSocketManager::~TcpServerSocketManager()
{
	...
	{
		spdlog::error(
			"TcpServerSocketManager::~TcpServerSocketManager: exception occurred - {}", ex.what());
	}
}
...
bool TcpServerSocketManager::Listen(int port)
{
	try
	{
		...
		if (ec)
		{
			spdlog::error("TcpServerSocketManager::Listen: open() failed: {}", ec.message());
			return false;
		}
		...
		if (ec)
		{
			spdlog::error("TcpServerSocketManager::Listen: bind() failed on 0.0.0.0:{}: {}", port,
				ec.message());
			return false;
		}
		...
		if (ec)
		{
			spdlog::error("TcpServerSocketManager::Listen: set_option(reuse_address) failed: {}",
				ec.message());
			return false;
		}
		...
		if (ec)
		{
			spdlog::error("TcpServerSocketManager::Listen: set_option(send_buffer_size) failed: {}",
				ec.message());
			return false;
		}
		...
		if (ec)
		{
			spdlog::error("TcpServerSocketManager::Listen: listen() failed: {}", ec.message());
			return false;
		}
	}
	...
	{
		spdlog::error(
			"TcpServerSocketManager::Listen: failed to bind on 0.0.0.0:{}: {}", port, ex.what());
		return false;
	}

	spdlog::info("TcpServerSocketManager::Listen: initialized port={:05}", port);
	return true;
}
...
void TcpServerSocketManager::StopAccept()
{
	...
	if (_acceptor != nullptr && _acceptor->is_open())
	{
		...
		if (ec)
			spdlog::error("TcpServerSocketManager::StopAccept: cancel() failed: {}", ec.message());
	}
}
...
void TcpServerSocketManager::AsyncAccept()
{
		...
		_acceptor->async_accept(
			[this](const asio::error_code& ec, asio::ip::tcp::socket rawSocket)
			{
				...
				else
				{
					...
					{
						spdlog::error(
							"TcpServerSocketManager::AsyncAccept: accept failed: {}", ec.message());
					}
				}
	...
	{
		spdlog::error("TcpServerSocketManager::AsyncAccept: async_accept() failed: {}", ex.what());
	}
}
...
void TcpServerSocketManager::OnAccept(asio::ip::tcp::socket& rawSocket)
{
	...
	if (socketId == -1)
	{
		spdlog::error("TcpServerSocketManager::OnAccept: socketId list is empty");
		return;
	}
	...
	{
		spdlog::error("TcpServerSocketManager::OnAccept: null socket [socketId:{}]", socketId);
		return;
	}
	...
	{
		spdlog::error(
			"TcpServerSocketManager::OnAccept: no raw socket allocated [socketId:{}]", socketId);
		return;
	}
