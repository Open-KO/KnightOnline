...
void CN3EngTool::RenderGrid(const __Matrix44& mtxWorld)
{
	...
	if (m_pVGrids)              // ±×¸®µå ±×¸®±â..
	{
		s_lpD3DDev->DrawPrimitiveUP(
			D3DPT_LINELIST, m_nGridLineCount, m_pVGrids, sizeof(__VertexColor)); // X
	}
...
void CN3EngTool::RenderTexturePreview(CN3Texture* pTex, HWND hWndDiffuse, RECT* pRCSrc)
{
	...
	if (pTex == nullptr)
	{
		...
		s_lpD3DDev->Clear(1, &rcClear, D3DCLEAR_ZBUFFER | D3DCLEAR_TARGET,
			D3DCOLOR_ARGB(255, 64, 64, 64), 1.0f, 0);
		...
		m_VPreview[0].Set(
			(float) rcTex.left, (float) rcTex.top, 0.5f, 1.0f, 0xff000000, fU_Left, fV_Top);
		m_VPreview[1].Set(
			(float) rcTex.right, (float) rcTex.top, 0.5f, 1.0f, 0xff000000, fU_Right, fV_Top);
		m_VPreview[2].Set(
			(float) rcTex.right, (float) rcTex.bottom, 0.5f, 1.0f, 0xff000000, fU_Right, fV_Bottom);
		m_VPreview[3] = m_VPreview[0];
		m_VPreview[4] = m_VPreview[2];
		m_VPreview[5].Set(
			(float) rcTex.left, (float) rcTex.bottom, 0.5f, 1.0f, 0xff000000, fU_Left, fV_Bottom);
		...
		s_lpD3DDev->DrawPrimitiveUP(
			D3DPT_TRIANGLELIST, 2, &m_VPreview, sizeof(__VertexTransformed));
	...
	//	if(hWndDiffuse != nullptr)
	{
		...
		D3DRECT rcClear = { rcPreview.left, rcPreview.top, rcPreview.right, rcPreview.bottom };
		hr              = s_lpD3DDev->Clear(1, &rcClear, D3DCLEAR_ZBUFFER | D3DCLEAR_TARGET,
						 D3DCOLOR_ARGB(255, 64, 64, 64), 1.0f, 0);
		...
		if (pTex->Get() != nullptr)
		{
			...
			m_VPreview[0].Set(
				(float) rcTex.left, (float) rcTex.top, 0.5f, 1.0f, 0xffffffff, fU_Left, fV_Top);
			m_VPreview[1].Set(
				(float) rcTex.right, (float) rcTex.top, 0.5f, 1.0f, 0xffffffff, fU_Right, fV_Top);
			m_VPreview[2].Set((float) rcTex.right, (float) rcTex.bottom, 0.5f, 1.0f, 0xffffffff,
				fU_Right, fV_Bottom);
			m_VPreview[3] = m_VPreview[0];
			m_VPreview[4] = m_VPreview[2];
			m_VPreview[5].Set((float) rcTex.left, (float) rcTex.bottom, 0.5f, 1.0f, 0xffffffff,
				fU_Left, fV_Bottom);
