...
//
#ifdef _N3TOOL
bool CN3FXPartBillBoard::ParseScript(
	char* szCommand, char* szBuff0, char* szBuff1, char* szBuff2, char* szBuff3)
{
	...
	if (lstrcmpi(szCommand, "<billboard_Rotation>") == 0)
	{
		...
		m_mtxRot.Identity();
		m_mtxRot.Rotation(DegreesToRadians(m_fRotBillBoardX), DegreesToRadians(m_fRotBillBoardY),
...
bool CN3FXPartBillBoard::Save(File& file)
{
	...
	if (m_iVersion >= 3)
		file.Write(&m_bRoateOnlyY, sizeof(bool));
