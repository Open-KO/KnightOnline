...
public:
	CN3ColorChange();
	~CN3ColorChange() override;

public:
	D3DCOLOR GetCurColor() const
	{
		return m_CurColor;
	}

	D3DCOLOR GetNextColor() const
	{
		return m_NextColor;
	}
	...
	CN3ColorChange& operator=(const CN3ColorChange& rhs);
...
public:
	CN3DeltaChange();
	~CN3DeltaChange() override;

public:
	float GetCurDelta() const
	{
		return m_fCurDelta;
	}

	float GetNextDelta() const
	{
		return m_fNextDelta;
	}
	...
	void Tick();
	void SetPercentage(float fPercentage);
	CN3DeltaChange& operator=(const CN3DeltaChange& rhs);
