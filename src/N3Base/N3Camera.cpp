...
BOOL CN3Camera::MoveByWindowMessage(MSG* pMsg)
{
	...
	else if (pMsg->message == WM_KEYDOWN)
	{
		switch (pMsg->wParam)
		{
			...
			case '9':
			{
				static __Vector3 vEyes[10] = { m_vPos, m_vPos, m_vPos, m_vPos, m_vPos, m_vPos,
					m_vPos, m_vPos, m_vPos, m_vPos };
				static __Vector3 vAts[10]  = { m_vAt, m_vAt, m_vAt, m_vAt, m_vAt, m_vAt, m_vAt,
					 m_vAt, m_vAt, m_vAt };
...
void CN3Camera::Rotate(float fRadianX, float fRadianY)
{
	...
	__Vector3 v1 = m_vAt - m_vPos,
			  v2; // Rotation 은 LookAt Position 처럼, Scale 은 UpVector 처럼 쓴다..
	v1       *= mtx;
	m_vScale *= mtx;
...
#ifdef _N3TOOL
void CN3Camera::LookAround(
	float fRadianX, float fRadianY) //At Postion을 중심으로 카메라가 돈다..고로 위치가 바뀐다..
{
	//static __Matrix44 mtx;
...
{
	__Vector3 vDir = m_vAt
					 - m_vPos; // Rotation 은 LookAt Position 처럼, Scale 은 UpVector 처럼 쓴다..
	vDir.Normalize();
...
void CN3Camera::Tick(float fFrm)
{
	...
	if (m_bOrtho)
	{
		...
		m_Data.mtxProjection.OrthoLH(fL, fL / m_Data.fAspect, m_Data.fNP * (1.0f + fL / 1000.0f),
	...
	{
		m_Data.mtxProjection.PerspectiveFovLH(
			m_Data.fFOV, m_Data.fAspect, m_Data.fNP, m_Data.fFP); // Projection Matrix Setting
	}
	...
	float t        = sqrt(frustum[0][0] * frustum[0][0] + frustum[0][1] * frustum[0][1]
