...
enum e_SubPacket_Trade : uint8_t
{
...
// Sub Packet
enum e_SubPacket_Party : uint8_t
{
	// Send - s1(ID)	| Recv b1(YesNoErr)
	N3_SP_PARTY_OR_FORCE_CREATE        = 0x01,

	// Send - b1(YesNo) | Recv - s1(ID)
	N3_SP_PARTY_OR_FORCE_PERMIT        = 0x02,

	// Send - s1(ID) | Recv - s3(ID, HPMax, HP) b2(Level, Class) - 문자열은 ID 로 알아낸다.. ID 가 -1 이면.. 파티에 들어오는것을 상대방이 거절한거다..
	N3_SP_PARTY_OR_FORCE_INSERT        = 0x03,

	// Send - s1(ID) | Recv - s1(ID) - 자기 자신이면 파티를 깨야 한다..
	N3_SP_PARTY_OR_FORCE_REMOVE        = 0x04,

	// Send
	N3_SP_PARTY_OR_FORCE_DESTROY       = 0x05,

	// Recv - s3(ID, HPMax, HP, iMPMax, MP)
	N3_SP_PARTY_OR_FORCE_HP_CHANGE     = 0x06,

	// Recv - s1(ID), b1(Level)
	N3_SP_PARTY_OR_FORCE_LEVEL_CHANGE  = 0x07,

	// Recv - s1(ID), b1(Class)드물지만 전직할때...
	N3_SP_PARTY_OR_FORCE_CLASS_CHANGE  = 0x08,
...
// Sub Packet
enum e_SubPacket_PerTrade : uint8_t
{
...
// Sub Packet
enum e_SubPacket_Magic : uint8_t
{
...
enum e_SubPacket_KNights_Create : uint8_t
{
...
enum e_SubPacket_KNights_Common : uint8_t
{
...
enum e_SubPacket_KnightsList : uint8_t
{
...
enum e_SubPacket_WareHouse : uint8_t
{
