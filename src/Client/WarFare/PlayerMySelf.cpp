...
void CPlayerMySelf::Tick()
{
	...
	if (PSM_WALK == m_eStateMove || PSM_WALK_BACKWARD == m_eStateMove || PSM_RUN == m_eStateMove) // 앞뒤로 걸어가거나 달려가면.
	{
...
float CPlayerMySelf::AttackableDistance(CPlayerBase* pTarget)
{
	...
	if (m_pItemPlugBasics[0])
		fDistLimit += m_pItemPlugBasics[0]->siAttackRange / 10.0f;           // 아이템을 들고 있으면..
			 ...
			 && ITEM_POS_TWOHANDLEFT == m_pItemPlugBasics[1]->byAttachPoint) // 왼손에 투핸드레프트(활) 아이템을 들고 있으면..
