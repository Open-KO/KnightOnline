...
//////////////////////////////////////////////////////////////////////
TCHAR CZipArchive::m_gszCopyright[] = {
	_T("Zip archive creation and modification Copyright 2000 Tadeusz Dracz")
};
...
bool CZipArchive::OpenNewFile(CZipFileHeader& header, int iLevel, LPCTSTR lpszFilePath)
{
	...
	if (lpszFilePath)
	{
		...
		else
		{
			...
			header.m_uExternalAttr = ::GetFileAttributes(
				lpszFilePath); // mfc bug: m_attribute is 1-byte
	...
	if (CurrentFile()->m_uMethod == Z_DEFLATED)
	{
		...
		int err                = deflateInit2(
            &m_info.m_stream, iLevel, Z_DEFLATED, -MAX_WBITS, DEF_MEM_LEVEL, Z_DEFAULT_STRATEGY);
	...
	ZIPCALLBACKFUN pCallback, void* pUserData, DWORD nBufSize)
{
	...
	szFile +=
		_T("\\")
		+ (bFullPath
				? GetFileDirAndName(szFileName)
				: GetFileName(
					  szFileName)); // just in case in the archive there are file names with drives
	...
	for (WORD i = 0; i < static_cast<WORD>(GetNoEntries()); i++)
	{
		...
		for (int j = 0; j < aNames.GetSize(); j++)
		{
			bool bEqual = (bCaseSensitive ? aNames[j].Collate(szFileName)
										  : aNames[j].CollateNoCase(szFileName))
						  == 0;
			if (bEqual)
			{
				indexes.Add(i);
...
void CZipArchive::DeleteInternal(WORD uIndex)
{
	...
	if (uIndex == GetNoEntries() - 1) // last entry or the only one entry
		...
		uOtherOffsetChanged = RemovePackedFile(
			pfh->m_uOffset, m_centralDir.m_headers[uIndex + 1]->m_uOffset);
...
//
BOOL CZipArchive::AddNewFile(
	LPCTSTR lpszFilePath, LPCTSTR strPath, int iLevel, unsigned long nBufSize)
{
	CString strFileName;
	CString strDirPath;
...
bool CZipArchive::AddNewFile(LPCTSTR lpszFilePath, int iLevel, bool bFullPath,
	ZIPCALLBACKFUN pCallback, void* pUserData, unsigned long nBufSize)
{
	if (!nBufSize)
		return false;
...
int CZipArchive::WideToSingle(LPCTSTR lpWide, CZipAutoBuffer& szSingle)
{
	...
	int iLen = WideCharToMultiByte(CP_ACP, 0, lpWide, wideLen, szSingle, 0, nullptr, nullptr);
	if (iLen > 0)
	{
		szSingle.Allocate(iLen, true);
		iLen = WideCharToMultiByte(CP_ACP, 0, lpWide, wideLen, szSingle, iLen, nullptr, nullptr);
		ASSERT(iLen != 0);
	}
...
int CZipArchive::SingleToWide(CZipAutoBuffer& szSingle, CString& szWide)
{
	...
	if (iLen > 0)
	{
		iLen = MultiByteToWideChar(
			CP_ACP, MB_PRECOMPOSED, szSingle, singleLen, szWide.GetBuffer(iLen), iLen);
