...
public:
	...
	BOOL AddNewFile(
		LPCTSTR lpszFilePath, LPCTSTR strPath, int iLevel = -1, unsigned long nBufSize = 65535 * 2);
	...
	bool TestFile(WORD uIndex, ZIPCALLBACKFUN pCallback = nullptr, void* pUserData = nullptr,
		DWORD uBufSize = 65535);
	...
	void SetAdvanced(
		int iWriteBuffer = 65535, int iExtractBuffer = 16384, int iSearchBuffer = 32768);
...
*/
	bool AddNewFile(LPCTSTR lpszFilePath, int iLevel = -1, bool bFullPath = true,
		ZIPCALLBACKFUN pCallback = nullptr, void* pUserData = nullptr,
	...
	bool OpenNewFile(
		CZipFileHeader& header, int iLevel = Z_DEFAULT_COMPRESSION, LPCTSTR lpszFilePath = nullptr);
	...
	bool ExtractFile(WORD uIndex, LPCTSTR lpszPath, bool bFullPath = true,
		LPCTSTR lpszNewName = nullptr, ZIPCALLBACKFUN pCallback = nullptr,
		void* pUserData = nullptr, DWORD nBufSize = 65535);
