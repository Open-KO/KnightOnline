...
struct CZipAutoHandle
{
	...
	bool CreateMapping(HANDLE hFile)
	{
		m_hFileMap = CreateFileMapping(
			hFile, nullptr, PAGE_READWRITE, 0, 0, _T("ZipArchive Mapping File"));
		if (!m_hFileMap)
			return false;
...
void CZipCentralDir::Read()
{
	...
	ASSERT(
		(!m_uThisDisk && (m_uEntriesNumber == m_uDiskEntriesNo) && !m_uDiskWithCD) || m_uThisDisk);
...
void CZipCentralDir::AddNewFile(CZipFileHeader& header)
{
	...
	if (m_bFindFastEnabled)
		InsertFindFastElement(pHeader,
			WORD(m_headers.GetSize() - 1)); // GetSize IS > 0, 'cos we've just added a header
	RemoveFromDisk();
	...
	if (m_pStorage->IsSpanMode() && m_pStorage->GetCurrentDisk() == 0)
	{
		...
		// lets make sure it will be one-disk archive
		{
				...
				(m_pStorage->GetFreeInBuffer() >= uToGrow)) // is the buffer big enough?
			{
				...
				bDontAllowDiskChange =
					true; // if the disk change occurs somehow, we'll throw an error later
			}
			else
			{
				m_pStorage->Flush();
...
void CZipCentralDir::InsertFindFastElement(CZipFileHeader* pHeader, WORD uIndex)
{
	...
	m_findarray.InsertAt(
		start, CZipFindFast(pHeader, WORD(uIndex == -1 ? iSize : uIndex /* just in case */)));
}
