...
struct WriteBuffer : std::vector<uint8_t>
{
    ...
    {
        insert(end(), reinterpret_cast<const uint8_t *>(value), reinterpret_cast<const uint8_t *>(value) + length);
    }
};
...
bool CTblEditorBase::LoadFile(const CString &path, CString &errorMsg)
{
    if (path.IsEmpty())
    {
        TRACE("Path is empty");
    ...
    if (encryptedFileSize == 0)
    {
        ...
        encryptedFile.Close();
        std::filesystem::remove(TemporaryPath.GetString(), ec);
...
bool CTblEditorBase::LoadRowData(File &file)
{
    ...
    for (int iRowNo = 0; iRowNo < iRowCount; iRowNo++)
    {
        ...
        for (int iColNo = 0; iColNo < iDataTypeCount; iColNo++)
        {
            ...
            case DT_BYTE: {
                uint8_t val = 0;
                if (!file.Read(&val, sizeof(uint8_t), &bytesRead) || bytesRead != sizeof(uint8_t))
                {
            ...
            case DT_SHORT: {
                int16_t val = 0;
                if (!file.Read(&val, sizeof(int16_t), &bytesRead) || bytesRead != sizeof(int16_t))
                {
            ...
            case DT_WORD: {
                uint16_t val = 0;
                if (!file.Read(&val, sizeof(uint16_t), &bytesRead) || bytesRead != sizeof(uint16_t))
                {
            ...
            case DT_INT: {
                int32_t val = 0;
                if (!file.Read(&val, sizeof(int32_t), &bytesRead) || bytesRead != sizeof(int32_t))
                {
            ...
            case DT_DWORD: {
                uint32_t val = 0;
                if (!file.Read(&val, sizeof(uint32_t), &bytesRead) || bytesRead != sizeof(uint32_t))
                {
            ...
            case DT_STRING: {
                ...
                int32_t iStrLen = 0;
                if (!file.Read(&iStrLen, sizeof(int32_t), &bytesRead) || bytesRead != sizeof(int32_t))
                {
                ...
                {
                    if (!file.Read(szValue.GetBuffer(iStrLen), iStrLen, &bytesRead) || bytesRead != iStrLen)
                    {
...
CString CTblEditorBase::GetColumnName(int iColNo) const
{
    DATA_TYPE dataType = m_DataTypes[iColNo];
    switch (dataType)
    {
...
DATA_TYPE CTblEditorBase::GetColumnType(int iColNo) const
{
