...
bool CIni::Load(const std::filesystem::path &path)
{
    ...
    while (!file.eof())
    {
        ...
        if (sectionStart == std::string::npos || sectionEnd == std::string::npos || sectionStart > sectionEnd)
        {
            /* invalid section */
    ...
    size_t nBufferLength)
{
    ...
    snprintf(szOutBuffer, nBufferLength, "%.*s", static_cast<int>(svDefault.length()), svDefault.data());
}
