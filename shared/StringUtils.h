...
// Copies src into dst (up to srcLen), ensuring it's always null-terminated, even if it were to overflow the dst buffer
// (bufferSize). Returns the number of characters truncated (should be 0 if everything is written correctly)
size_t strcpy_safe(char *dst, const char *src, size_t dstBufferSize, size_t srcLen);

// Copies src into dst (up to src.length()), ensuring it's always null-terminated, even if it were to overflow the dst
// buffer (bufferSize). Returns the number of characters truncated (should be 0 if everything is written correctly)
size_t strcpy_safe(char *dst, const std::string_view src, size_t dstBufferSize);

// Copies src into dst (up to src.length()), ensuring it's always null-terminated, even if it were to overflow the dst
// buffer (BufferSize). Returns the number of characters truncated (should be 0 if everything is written correctly)
template <std::size_t BufferSize> size_t strcpy_safe(char (&dst)[BufferSize], const std::string_view src)
{
