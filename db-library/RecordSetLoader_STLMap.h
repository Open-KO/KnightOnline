...
class STLMap : public Base<ModelType, BoundModelType>
{
  public:
    ...
    STLMap(ContainerType &targetContainer) : _targetContainer(targetContainer)
    {
        this->SetProcessFetchCallback(std::bind(&STLMap::ProcessFetch, this, std::placeholders::_1));
    }

  protected:
    void ProcessFetch(RecordSetType &recordset)
    {
        using ContainerKeyType = std::remove_const_t<std::remove_reference_t<typename ContainerType::KeyType>>;

        using ModelKeyType = std::remove_const_t<std::remove_reference_t<decltype(std::declval<ModelType>().MapKey())>>;
