...
template <typename ModelType, typename BoundModelType = ModelType> class ModelRecordSet
{
  public:
    ModelRecordSet(bool fetchRowCount = false) : _fetchRowCount(fetchRowCount)
    {
    }
    ...
    void execute() noexcept(false)
    {
        ...
        Model::IndexColumnNameBindings<BoundModelType>(_result, _columnCount, _bindingIndex);
    }
