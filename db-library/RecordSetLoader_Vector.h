...
template <typename ModelType, typename BoundModelType = ModelType> class Vector : public Base<ModelType, BoundModelType>
{
  public:
    using ContainerType = std::vector<BoundModelType *>;
    using RecordSetType = Base<ModelType, BoundModelType>::RecordSetType;

    Vector(ContainerType &targetContainer) : _targetContainer(targetContainer)
    {
        this->SetProcessFetchCallback(std::bind(&Vector::ProcessFetch, this, std::placeholders::_1));
    }
