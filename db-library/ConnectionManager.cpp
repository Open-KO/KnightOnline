...
{
    GetInstance().SetDatasourceConfigImpl(dbType, datasourceName, datasourceUserName, datasourcePassword);
}

std::shared_ptr<const DatasourceConfig> ConnectionManager::GetDatasourceConfig(modelUtil::DbType dbType)
{
    ...
    const std::string_view datasourcePassword)
{
    ...
    {
        auto config = std::make_shared<const DatasourceConfig>(datasourceName, datasourceUserName, datasourcePassword);
...
std::shared_ptr<const DatasourceConfig> ConnectionManager::GetDatasourceConfigImpl(modelUtil::DbType dbType) const
{
    std::lock_guard<std::mutex> lock(_configLock);
...
std::shared_ptr<Connection> ConnectionManager::CreateConnectionImpl(modelUtil::DbType dbType, long timeout) noexcept(
    false)
{
    auto config = GetDatasourceConfig(dbType);
    if (config == nullptr)
